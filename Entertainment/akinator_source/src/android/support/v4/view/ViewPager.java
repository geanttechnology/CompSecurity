...
_L3:
        ...
        if (mIsBeingDragged)
        {
            ...
            int j1 = getScrollX();
            obj = infoForCurrentScrollPosition();
            setCurrentItemInternal(determineTargetPage(((ItemInfo) (obj)).position, ((float)j1 / (float)i1 - ((ItemInfo) (obj)).offset) / ((ItemInfo) (obj)).widthFactor, k, (int)(MotionEventCompat.getX(motionevent, MotionEventCompat.findPointerIndex(motionevent, mActivePointerId)) - mInitialMotionX)), true, true, k);
            flag = resetTouch();
        }
        continue; /* Loop/switch isn't completed */
_L5:
...
_L3:
    ...
    // Unreferenced inner class android/support/v4/view/ViewPager$SimpleOnPageChangeListener
    /* block-local class not found */
    class SimpleOnPageChangeListener {}


    // Unreferenced inner class android/support/v4/view/ViewPager$ViewPositionComparator
    /* block-local class not found */
