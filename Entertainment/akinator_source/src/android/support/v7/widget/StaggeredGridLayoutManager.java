    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutstate, RecyclerView.State state)
    {
        ...
        do
        {
            if (layoutstate.hasMore(state) && !mRemainingSpans.isEmpty())
            {
                ...
                int l2 = layoutparams.getViewLayoutPosition();
                int i = mLazySpanLookup.getSpan(l2);
                Span span;
                int k1;
                int l1;
                int i2;
                ...
                if (layoutparams.mFullSpan && layoutstate.mItemDirection == -1)
                {
                    ...
                    } else
                    {
                        ...
                        if (i != 0)
                        {
                            LazySpanLookup.FullSpanItem fullspanitem2 = mLazySpanLookup.getFullSpanItem(l2);
                ...
                i2 = i + mSecondaryOrientation.getDecoratedMeasurement(view);
                if (mOrientation == 1)
                {
                    layoutDecoratedWithMargins(view, i, k1, i2, l1);
                } else
                {
                    layoutDecoratedWithMargins(view, k1, i, l1, i2);
                }
                if (layoutparams.mFullSpan)
                {
...
_L5:
    ...
    View findFirstVisibleItemClosestToEnd(boolean flag, boolean flag1)
    {
        ...
        for (int i = getChildCount() - 1; i >= 0;)
        {
            ...
            View view1 = view;
            if (mPrimaryOrientation.getDecoratedStart(view2) >= j)
            {
                if (!flag || mPrimaryOrientation.getDecoratedEnd(view2) <= k)
                {
                    return view2;
                }
                view1 = view;
    ...
    View findFirstVisibleItemClosestToStart(boolean flag, boolean flag1)
    {
        ...
        for (int i = 0; i < l;)
        {
            View view2 = getChildAt(i);
            View view1 = view;
            if (mPrimaryOrientation.getDecoratedEnd(view2) <= k)
            {
                if (!flag || mPrimaryOrientation.getDecoratedStart(view2) >= j)
                {
                    return view2;
                }
                view1 = view;
