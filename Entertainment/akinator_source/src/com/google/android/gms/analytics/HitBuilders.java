...
public class HitBuilders
{
    ...
    protected static class HitBuilder
    {
        ...
        public Map build()
        {
            HashMap hashmap = new HashMap(zzLH);
            if (zzLI != null)
            {
                hashmap.putAll(zzLI.build());
            }
            Iterator iterator = zzLK.iterator();
            for (int i = 1; iterator.hasNext(); i++)
            {
                hashmap.putAll(((Promotion)iterator.next()).zzaX(zzc.zzV(i)));
            }

            iterator = zzLL.iterator();
            for (int j = 1; iterator.hasNext(); j++)
            {
                hashmap.putAll(((Product)iterator.next()).zzaX(zzc.zzT(j)));
            }

            iterator = zzLJ.entrySet().iterator();
            for (int k = 1; iterator.hasNext(); k++)
            {
                ...
                {
                    hashmap.putAll(((Product)((Iterator) (obj)).next()).zzaX((new StringBuilder()).append(s).append(zzc.zzX(l)).toString()));
                }
        ...
        {
            set(zzc.zzR(i), Float.toString(f));
            return this;
        }
