...
label1:
    ...
    SessionInsertRequest(int i, Session session, List list, List list1, IBinder ibinder)
    {
        mVersionCode = i;
        zzapP = session;
        zzapY = Collections.unmodifiableList(list);
        zzasQ = Collections.unmodifiableList(list1);
        zzasb = com.google.android.gms.internal.zzoj.zza.zzbJ(ibinder);
    }

    public SessionInsertRequest(Session session, List list, List list1, zzoj zzoj1)
    {
        mVersionCode = 3;
        zzapP = session;
        zzapY = Collections.unmodifiableList(list);
    ...
    {
        this(Builder.zza(builder), Builder.zzb(builder), Builder.zzc(builder), null);
    }


    public SessionInsertRequest(SessionInsertRequest sessioninsertrequest, zzoj zzoj1)
    {
        this(sessioninsertrequest.zzapP, sessioninsertrequest.zzapY, sessioninsertrequest.zzasQ, zzoj1);
    }

    private boolean zzb(SessionInsertRequest sessioninsertrequest)
    {
        return zzw.equal(zzapP, sessioninsertrequest.zzapP) && zzw.equal(zzapY, sessioninsertrequest.zzapY) && zzw.equal(zzasQ, sessioninsertrequest.zzasQ);
    }
    ...
    {
        return zzw.zzv(this).zzg("session", zzapP).zzg("dataSets", zzapY).zzg("aggregateDataPoints", zzasQ).toString();
    }
