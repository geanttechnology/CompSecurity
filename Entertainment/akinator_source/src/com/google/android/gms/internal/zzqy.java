...
public class zzqy
{
    ...
    public zzqy(Context context, int i, String s, String s1, String s2)
    {
        ...
        if (zznc.zzar(mContext))
        {
            ...
            if (zznb.zzcA(s2))
            {
                ...
                {
                    Log.e(TAG, (new StringBuilder()).append("callingPackage is not supposed to be empty for wakelock ").append(zzaia).append("!").toString(), new IllegalArgumentException());
    ...
    public void acquire(long l)
    {
        if (!zzmx.zzqx() && zzaVw)
        {
            Log.wtf(TAG, (new StringBuilder()).append("Do not acquire with timeout on reference counted WakeLocks before ICS. wakelock: ").append(zzaia).toString());
        }
        zzi(null, l);
        zzaVs.acquire(l);
    }
