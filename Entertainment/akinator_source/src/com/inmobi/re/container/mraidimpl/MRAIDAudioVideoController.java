...
public class MRAIDAudioVideoController
{
    ...
    private boolean a(String s, String s1, Activity activity, com.inmobi.re.controller.JSController.Dimensions dimensions)
    {
        ...
        com.inmobi.re.controller.util.AVPlayer.playerState playerstate = videoPlayer.getState();
        if (s.equalsIgnoreCase(videoPlayer.getPropertyID()))
        {
            s = videoPlayer.getMediaURL();
            if (s1.length() == 0 || s1.equalsIgnoreCase(s))
            {
    /* anonymous class not found */
    class _anm7 {}

                switch (_cls7.a[playerstate.ordinal()])
                {
                default:
                    return false;
...
_L3:
    ...
    {
        Log.debug("[InMobi]-[RE]-4.5.5", "MRAIDAudioVideoController: hiding all players");
    ...
    public void playVideoImpl(Bundle bundle, Activity activity)
    {
        ...
        Log.debug("[InMobi]-[RE]-4.5.5", (new StringBuilder()).append("Final dimensions: ").append(obj).toString());
...
_L3:
        ...
        exception1;
        Log.internal("[InMobi]-[RE]-4.5.5", "Unable to release player");
          goto _L3
        Exception exception;
        exception;
        Log.internal("[InMobi]-[RE]-4.5.5", "IMwebview release all players ", exception);
_L1:
