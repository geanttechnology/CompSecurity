...
//            AsyncHttpClient, LogInterface, JsonStreamerEntity, SimpleMultipartEntity, 
    ...
    implements Serializable
{
    ...
    public RequestParams(Map map)
    {
        ...
        fileArrayParams = new ConcurrentHashMap();
        urlParamsWithObjects = new ConcurrentHashMap();
        forceMultipartEntity = false;
        elapsedFieldInJsonStreamer = "_elapsed";
    ...
    private HttpEntity createFormEntity()
    {
        ...
        {
            AsyncHttpClient.log.e("RequestParams", "createFormEntity failed", unsupportedencodingexception);
            return null;
        }
        ...
        throws IOException
    {
        ...
        responsehandlerinterface = new JsonStreamerEntity(responsehandlerinterface, flag, elapsedFieldInJsonStreamer);
        java.util.Map.Entry entry;
        ...
        throws IOException
    {
        ...
        for (Iterator iterator4 = fileArrayParams.entrySet().iterator(); iterator4.hasNext();)
        {
            java.util.Map.Entry entry3 = (java.util.Map.Entry)iterator4.next();
            Iterator iterator5 = ((List)entry3.getValue()).iterator();
            while (iterator5.hasNext()) 
            {
                FileWrapper filewrapper1 = (FileWrapper)iterator5.next();
                responsehandlerinterface.addPart((String)entry3.getKey(), filewrapper1.file, filewrapper1.contentType, filewrapper1.customFileName);
            }
        }
    ...
    private List getParamsList(String s, Object obj)
    {
        ...
        if (obj instanceof List)
        {
            ...
            for (int i = 0; i < k; i++)
            {
                linkedlist.addAll(getParamsList(String.format(Locale.US, "%s[%d]", new Object[] {
                    s, Integer.valueOf(i)
                }), ((List) (obj)).get(i)));
            }
        ...
        throws IOException
    {
        ...
        if (!forceMultipartEntity && streamParams.isEmpty() && fileParams.isEmpty() && fileArrayParams.isEmpty())
        {
    ...
    {
        return urlParams.get(s) != null || streamParams.get(s) != null || fileParams.get(s) != null || urlParamsWithObjects.get(s) != null || fileArrayParams.get(s) != null;
    }
    ...
    public void put(String s, File afile[])
        throws FileNotFoundException
    {
        put(s, afile, null, null);
    }

    public void put(String s, File afile[], String s1, String s2)
        throws FileNotFoundException
    {
        if (s != null)
        {
            ...
            for (int i = 0; i < j; i++)
            {
                ...
                arraylist.add(new FileWrapper(file, s1, s2));
            }

            fileArrayParams.put(s, arraylist);
        }
    }
    ...
    public void setElapsedFieldInJsonStreamer(String s)
    {
        elapsedFieldInJsonStreamer = s;
    }

    public void setForceMultipartEntityContentType(boolean flag)
    {
        forceMultipartEntity = flag;
    }
    ...
    public String toString()
    {
        ...
        for (Iterator iterator3 = fileArrayParams.entrySet().iterator(); iterator3.hasNext(); stringbuilder.append("FILES(SIZE=").append(((List)entry3.getValue()).size()).append(")"))
        {
        ...
        BasicNameValuePair basicnamevaluepair;
        for (Iterator iterator4 = getParamsList(null, urlParamsWithObjects).iterator(); iterator4.hasNext(); stringbuilder.append(basicnamevaluepair.getValue()))
        {
            basicnamevaluepair = (BasicNameValuePair)iterator4.next();
            if (stringbuilder.length() > 0)
            {
                stringbuilder.append("&");
            }
