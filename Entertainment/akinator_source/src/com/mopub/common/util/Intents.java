...
public class Intents
{
    ...
    public static boolean canHandleApplicationUrl(Context context, Uri uri, boolean flag)
    {
        if (!deviceCanHandleIntent(context, new Intent("android.intent.action.VIEW", uri)))
        {
            if (flag)
            {
                MoPubLog.w((new StringBuilder()).append("Could not handle application specific action: ").append(uri).append(". ").append("You may be running in the emulator or another device which does not ").append("have the required application.").toString());
            }
            return false;
        } else
        {
        ...
        throws UrlParseException
    {
        ...
        {
            MoPubLog.w((new StringBuilder()).append("Could not handle url: ").append(uri).toString());
        ...
        throws IntentNotResolvableException
    {
        ...
        {
            launchActionViewIntent(context, uri, (new StringBuilder()).append("Unable to open intent for: ").append(uri).toString());
            return;
        } else
        {
            throw new IntentNotResolvableException((new StringBuilder()).append("Could not handle application specific action: ").append(uri).append("\n\tYou may be running in the emulator or another ").append("device which does not have the required application.").toString());
        }
    }
        ...
        throws IntentNotResolvableException
    {
        ...
        Preconditions.checkNotNull(uri);
        MoPubLog.d((new StringBuilder()).append("Final URI to show in browser: ").append(uri).toString());
        Bundle bundle = new Bundle();
        bundle.putString("URL", uri.toString());
        launchIntentForUserClick(context, getStartActivityIntent(context, com/mopub/common/MoPubBrowser, bundle), (new StringBuilder()).append("Could not show MoPubBrowser for url: ").append(uri).append("\n\tPerhaps you ").append("forgot to declare com.mopub.common.MoPubBrowser in your Android manifest file.").toString());
    }
