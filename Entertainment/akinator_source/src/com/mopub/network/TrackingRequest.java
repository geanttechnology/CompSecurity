...
public class TrackingRequest extends Request
{
    ...
    public static void makeTrackingHttpRequest(Iterable iterable, Context context, Listener listener, com.mopub.common.event.BaseEvent.Name name)
    {
        if (iterable != null && context != null)
        {
            ...
            while (iterable.hasNext()) 
            {
                ...
                {
                    context.add(new TrackingRequest(name, new Listener(listener, name) {
                        ...
                        {
                            MoPubLog.d((new StringBuilder()).append("Failed to hit tracking endpoint: ").append(url).toString());
                            if (listener != null)
                            {
    ...
    protected Response parseNetworkResponse(NetworkResponse networkresponse)
    {
        ...
        {
            return Response.error(new MoPubNetworkError((new StringBuilder()).append("Failed to log tracking request. Response code: ").append(networkresponse.statusCode).append(" for url: ").append(getUrl()).toString(), MoPubNetworkError.Reason.TRACKING_FAILURE));
        } else
        {
