    ...
    implements Parcelable
{
    ...
    public static void createFromNativeLinkingIntent(Intent intent, String s, AccessTokenCreationCallback accesstokencreationcallback)
    {
        ...
        intent = new Bundle(intent.getExtras());
        String s1 = intent.getString("access_token");
        if (s1 == null || s1.isEmpty())
        {
            accesstokencreationcallback.onError(new FacebookException("No access token found on intent"));
            return;
        }
        String s2 = intent.getString("user_id");
        if (s2 == null || s2.isEmpty())
        {
            Utility.getGraphMeRequestWithCacheAsync(s1, new com.facebook.internal.Utility.GraphMeRequestWithCacheCallback(intent, accesstokencreationcallback, s) {
        ...
        {
            accesstokencreationcallback.onSuccess(createFromBundle(null, intent, AccessTokenSource.FACEBOOK_APPLICATION_WEB, new Date(), s));
            return;
        }
    }
