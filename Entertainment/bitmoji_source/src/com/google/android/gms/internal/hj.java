    ...
    implements SafeParcelable, Geofence
{
    ...
    public int hashCode()
    {
        ...
        int i = (int)(l ^ l >>> 32);
        l = Double.doubleToLongBits(KY);
        return ((((i + 31) * 31 + (int)(l ^ l >>> 32)) * 31 + Float.floatToIntBits(KZ)) * 31 + KW) * 31 + KU;
    }

    public String toString()
    {
        return String.format(Locale.US, "Geofence[%s id:%s transitions:%d %.6f, %.6f %.0fm, resp=%ds, dwell=%dms, @%d]", new Object[] {
            bo(KW), Hh, Integer.valueOf(KU), Double.valueOf(KX), Double.valueOf(KY), Float.valueOf(KZ), Integer.valueOf(La / 1000), Integer.valueOf(Lb), Long.valueOf(Lz)
        });
    }
