    ...
    implements ConcurrentMap
{
        ...
        implements Serializable
    {
        ...
        CacheBuilder recreateCacheBuilder()
        {
            ...
            if (weigher == CacheBuilder.OneWeigher.INSTANCE) goto _L2; else goto _L1
_L1:
...
_L2:
            drainRecencyQueue();
            if ((long)referenceentry.getValueReference().getWeight() > maxSegmentWeight && !removeEntry(referenceentry, referenceentry.getHash(), RemovalCause.SIZE))
            {
...
_L3:
            ...
            throws ExecutionException
        {
            ...
            listenablefuture1 = listenablefuture;
            throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(String.valueOf(s).length() + 35)).append("CacheLoader returned null for key ").append(s).append(".").toString());
            listenablefuture;
...
_L1:
            count = i;
            evictEntries(((ReferenceEntry) (obj2)));
            unlock();
            postWriteCleanup();
            return null;
            setValue(((ReferenceEntry) (obj2)), obj, obj1, l);
...
_L2:
            ...
            throws ExecutionException
        {
            ...
            referenceentry = String.valueOf(obj);
            throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(String.valueOf(referenceentry).length() + 35)).append("CacheLoader returned null for key ").append(referenceentry).append(".").toString());
            referenceentry;
            statsCounter.recordMisses(1);
            throw referenceentry;
...
_L5:
        ...
        j = i;
        Object obj4 = iterable.get(obj3);
        if (obj4 != null)
        {
            break MISSING_BLOCK_LABEL_344;
        }
        j = i;
        iterable = String.valueOf(obj3);
        j = i;
        try
        {
            throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(String.valueOf(iterable).length() + 37)).append("loadAll failed to return a value for ").append(iterable).toString());
        }
        // Misplaced declaration of an exception variable
...
_L2:
        ...
        globalStatsCounter.recordHits(l);
        globalStatsCounter.recordMisses(k);
        return iterable;
    }
    ...
    ImmutableMap getAllPresent(Iterable iterable)
    {
        ...
        globalStatsCounter.recordHits(j);
    ...
    {
        for (iterable = iterable.iterator(); iterable.hasNext(); remove(iterable.next())) { }
    }
...
_L8:
        ...
        throws ExecutionException
    {
        ...
        {
            globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
            set = String.valueOf(cacheloader);
            throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(String.valueOf(set).length() + 31)).append(set).append(" returned null map from loadAll").toString());
        }
        ...
        stopwatch.stop();
        boolean flag1 = false;
        for (Iterator iterator = set.entrySet().iterator(); iterator.hasNext();)
        {
        ...
        if (flag1)
        {
            globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
            set = String.valueOf(cacheloader);
            throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(String.valueOf(set).length() + 42)).append(set).append(" returned null keys or values from loadAll").toString());
        } else
        {
