...
//            ImmutableSet, MultimapBuilder, Ordering, CollectPreconditions, 
//            Iterables, Serialization, ImmutableSetMultimap, Iterators, 
//            Maps, ImmutableList, Multisets
    ...
    implements Serializable
{
    public static class Builder
    {
        ...
        public ImmutableMultimap build()
        {
            ...
            if (keyComparator != null)
            {
                ...
                for (Iterator iterator1 = Ordering.from(keyComparator).onKeys().immutableSortedCopy(builderMultimap.asMap().entrySet()).iterator(); iterator1.hasNext(); listmultimap.putAll(entry.getKey(), (Iterable)entry.getValue()))
                {
                    entry = (java.util.Map.Entry)iterator1.next();
                }

                builderMultimap = listmultimap;
            }
        ...
        public Builder()
        {
            this(((Multimap) (MultimapBuilder.linkedHashKeys().arrayListValues().build())));
        }

        Builder(Multimap multimap)
        {
            builderMultimap = multimap;
        }
    }
    ...
    private static final class Values extends ImmutableCollection
    {
        ...
        {
            for (UnmodifiableIterator unmodifiableiterator = multimap.map.values().iterator(); unmodifiableiterator.hasNext();)
            {
                i = ((ImmutableCollection)unmodifiableiterator.next()).copyIntoArray(aobj, i);
            }
