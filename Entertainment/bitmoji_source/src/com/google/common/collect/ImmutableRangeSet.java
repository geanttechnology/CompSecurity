...
//            SortedLists, ImmutableSet, RegularImmutableSortedSet, Ordering, 
//            ImmutableSortedSet, DiscreteDomain, Cut, BoundType, 
    ...
    private final class AsSet extends ImmutableSortedSet
    {
        ...
        int indexOf(Object obj)
        {
            if (contains(obj))
            {
                obj = (Comparable)obj;
                long l = 0L;
                for (UnmodifiableIterator unmodifiableiterator = ranges.iterator(); unmodifiableiterator.hasNext();)
                {
    ...
    public static class Builder
    {
        ...
        public Builder add(Range range)
        {
            if (range.isEmpty())
            {
                ...
                throw new IllegalArgumentException((new StringBuilder(String.valueOf(range).length() + 33)).append("range must not be empty, but was ").append(range).toString());
            }
...
_L3:
    ...
    public ImmutableSet asDescendingSetOfRanges()
    {
        ...
        {
            return new RegularImmutableSortedSet(ranges.reverse(), Range.RANGE_LEX_ORDERING.reverse());
        }
    }

    public volatile Set asDescendingSetOfRanges()
    {
