...
public final class ByteStreams
{
    ...
    private static final int ZERO_COPY_CHUNK_SIZE = 0x80000;
        ...
        throws IOException
    {
        ...
        if (!(readablebytechannel instanceof FileChannel)) goto _L2; else goto _L1
_L1:
        long l2;
        readablebytechannel = (FileChannel)readablebytechannel;
        l2 = readablebytechannel.position();
        long l = l2;
        long l3;
        do
        {
            ...
            do
            {
                l4 = readablebytechannel.transferTo(l, 0x80000L, writablebytechannel);
                l3 = l + l4;
                readablebytechannel.position(l3);
                l = l3;
            } while (l4 > 0L);
            l = l3;
        } while (l3 < readablebytechannel.size());
        l2 = l3 - l2;
_L4:
        return l2;
_L2:
        ...
        do
        {
            ...
            {
                l1 += writablebytechannel.write(bytebuffer);
            }
            bytebuffer.clear();
        } while (true);
...
_L3:
    ...
    public static ByteArrayDataOutput newDataOutput(int i)
    {
        if (i < 0)
        {
            throw new IllegalArgumentException(String.format("Invalid size: %s", new Object[] {
                Integer.valueOf(i)
            }));
        } else
        {
...
label0:
    ...
    public static void skipFully(InputStream inputstream, long l)
        throws IOException
    {
        long l1 = skipUpTo(inputstream, l);
        if (l1 < l)
        {
            throw new EOFException((new StringBuilder(100)).append("reached end of stream after skipping ").append(l1).append(" bytes; ").append(l).append(" bytes expected").toString());
        } else
        {
            return;
        }
    ...
    private static long skipSafely(InputStream inputstream, long l)
        throws IOException
    {
        ...
        {
            return inputstream.skip(Math.min(i, l));
        }
    }

    static long skipUpTo(InputStream inputstream, long l)
        throws IOException
    {
        long l1 = 0L;
        do
...
label0:
            {
                ...
                {
                    long l4 = l - l1;
                    long l3 = skipSafely(inputstream, l4);
                    l2 = l3;
                    if (l3 != 0L)
                    {
                        break label0;
                    ...
                    l3 = inputstream.read(skipBuffer, 0, i);
                    l2 = l3;
                    if (l3 != -1L)
                    {
                        break label0;
