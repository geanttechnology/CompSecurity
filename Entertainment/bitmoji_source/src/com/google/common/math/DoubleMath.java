...
//            DoubleUtils, MathPreconditions, LongMath
...
_L8:
    ...
    public static double mean(Iterator iterator)
    {
        ...
        double d;
        double d1;
        for (d = checkFinite(((Number)iterator.next()).doubleValue()); iterator.hasNext(); d += (d1 - d) / (double)l)
        {
            d1 = checkFinite(((Number)iterator.next()).doubleValue());
            l++;
        }
    ...
    public static transient double mean(int ai[])
    {
        ...
        return (double)l / (double)ai.length;
    }
