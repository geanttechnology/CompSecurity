...
label0:
    ...
    private static Pattern fpPattern()
    {
        ...
        String s1 = (new StringBuilder(String.valueOf("(?:\\p{XDigit}++(?:\\.\\p{XDigit}*+)?|\\.\\p{XDigit}++)").length() + 25)).append("0[xX]").append("(?:\\p{XDigit}++(?:\\.\\p{XDigit}*+)?|\\.\\p{XDigit}++)").append("[pP][+-]?\\d++[fFdD]?").toString();
        return Pattern.compile((new StringBuilder(String.valueOf(s).length() + 23 + String.valueOf(s1).length())).append("[+-]?(?:NaN|Infinity|").append(s).append("|").append(s1).append(")").toString());
    }
