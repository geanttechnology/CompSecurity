...
label0:
    ...
    private static byte[] createAsciiDigits()
    {
        ...
        for (int j = 0; j <= 26; j++)
        {
            abyte0[j + 65] = (byte)(j + 10);
            abyte0[j + 97] = (byte)(j + 10);
        }
...
_L3:
    ...
    public static Long tryParse(String s, int i)
    {
        int j = 1;
        ...
        long l1 = -k;
        long l2 = 0x8000000000000000L / (long)i;
        for (j++; j < s.length(); j++)
        {
            int l = digit(s.charAt(j));
            if (l < 0 || l >= i || l1 < l2)
            {
                return null;
            }
            l1 *= i;
            if (l1 < (long)l - 0x8000000000000000L)
            {
                return null;
            }
            l1 -= l;
