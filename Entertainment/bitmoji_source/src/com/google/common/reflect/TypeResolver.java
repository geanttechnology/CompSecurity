...
_L3:
            ...
            if (!(type instanceof Class) && !(type instanceof TypeVariable))
            {
                ...
                if (type instanceof WildcardType)
                {
                    ...
                    if (wildcardtype.getLowerBounds().length == 0)
                    {
                        ...
                        type = Joiner.on('&').join(type);
                        return Types.newArtificialTypeVariable(com/google/common/reflect/TypeResolver$WildcardCapturer, (new StringBuilder(String.valueOf(type).length() + 33)).append("capture#").append(i).append("-of ? extends ").append(type).toString(), wildcardtype.getUpperBounds());
                    }
                } else
                {
    ...
    private static Object expectArgument(Class class1, Object obj)
    {
        ...
        catch (ClassCastException classcastexception)
        {
            ...
            throw new IllegalArgumentException((new StringBuilder(String.valueOf(obj).length() + 10 + String.valueOf(class1).length())).append(((String) (obj))).append(" is not a ").append(class1).toString());
        }
        return obj1;
    }
    ...
    {
        while (type.equals(type1) || (type1 instanceof WildcardType) != (type instanceof WildcardType)) 
        {
            return;
        }
        (new TypeVisitor(map, type1) {
            ...
            void visitClass(Class class1)
            {
                ...
                throw new IllegalArgumentException((new StringBuilder(String.valueOf(class1).length() + 21)).append("No type mapping from ").append(class1).toString());
            }
