...
//            Reflection, TypeCapture, TypeVisitor
...
_L1:
            if (obj == null || !Proxy.isProxyClass(obj.getClass()) || !(Proxy.getInvocationHandler(obj) instanceof TypeVariableInvocationHandler)) goto _L4; else goto _L3
_L3:
            obj = ((TypeVariableInvocationHandler)Proxy.getInvocationHandler(obj)).typeVariableImpl;
...
_L7:
        ...
        implements WildcardType, Serializable
    {
        ...
        public String toString()
        {
            StringBuilder stringbuilder = new StringBuilder("?");
            Type type;
            for (com.google.common.collect.UnmodifiableIterator unmodifiableiterator = lowerBounds.iterator(); unmodifiableiterator.hasNext(); stringbuilder.append(" super ").append(JavaVersion.CURRENT.typeName(type)))
            {
                type = (Type)unmodifiableiterator.next();
            }

            Type type1;
            for (Iterator iterator = Types.filterUpperBounds(upperBounds).iterator(); iterator.hasNext(); stringbuilder.append(" extends ").append(JavaVersion.CURRENT.typeName(type1)))
            {
    ...
    static transient TypeVariable newArtificialTypeVariable(GenericDeclaration genericdeclaration, String s, Type atype[])
    {
        ...
        return newTypeVariableImpl(genericdeclaration, s, atype1);
    }
    ...
    private static TypeVariable newTypeVariableImpl(GenericDeclaration genericdeclaration, String s, Type atype[])
    {
        return (TypeVariable)Reflection.newProxy(java/lang/reflect/TypeVariable, new TypeVariableInvocationHandler(new TypeVariableImpl(genericdeclaration, s, atype)));
    }
