...
public class CycleDetectingLockFactory
{
    ...
    private static class ExampleStackTrace extends IllegalStateException
    {
        ...
        static final Set EXCLUDED_CLASS_NAMES = ImmutableSet.of(com/google/common/util/concurrent/CycleDetectingLockFactory.getName(), com/google/common/util/concurrent/CycleDetectingLockFactory$ExampleStackTrace.getName(), com/google/common/util/concurrent/CycleDetectingLockFactory$LockGraphNode.getName());


        ExampleStackTrace(LockGraphNode lockgraphnode, LockGraphNode lockgraphnode1)
        {
            lockgraphnode = lockgraphnode.getLockName();
            lockgraphnode1 = lockgraphnode1.getLockName();
            super((new StringBuilder(String.valueOf(lockgraphnode).length() + 4 + String.valueOf(lockgraphnode1).length())).append(lockgraphnode).append(" -> ").append(lockgraphnode1).toString());
...
label0:
    ...
    private static class LockGraphNode
    {
        ...
        void checkAcquiredLock(Policy policy1, LockGraphNode lockgraphnode)
        {
            ...
            Preconditions.checkState(flag, "Attempted to acquire multiple locks with the same rank %s", new Object[] {
                lockgraphnode.getLockName()
            });
            if (allowedPriorLocks.containsKey(lockgraphnode))
            {
                return;
            }
            Object obj = (PotentialDeadlockException)disallowedPriorLocks.get(lockgraphnode);
            if (obj != null)
            {
    ...
    {
        String s = String.valueOf(enum.getDeclaringClass().getSimpleName());
        enum = String.valueOf(enum.name());
        return (new StringBuilder(String.valueOf(s).length() + 1 + String.valueOf(enum).length())).append(s).append(".").append(enum).toString();
    }
