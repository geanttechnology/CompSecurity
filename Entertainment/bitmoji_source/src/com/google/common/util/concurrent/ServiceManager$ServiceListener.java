    ...
    public void failed(state state1, Throwable throwable)
    {
        ...
        if (ate != null)
        {
            if (!(service instanceof service))
            {
                ...
                logger.log(level, (new StringBuilder(String.valueOf(s).length() + 34 + String.valueOf(s1).length())).append("Service ").append(s).append(" has failed in the ").append(s1).append(" state.").toString(), throwable);
            }
