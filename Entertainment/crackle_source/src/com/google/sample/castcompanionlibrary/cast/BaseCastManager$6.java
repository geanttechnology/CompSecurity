    ...
    implements ResultCallback
{
    ...
    public void onResult(Status status)
    {
        if (!status.isSuccess())
        {
            LogUtils.LOGD(BaseCastManager.access$000(), "stopApplication -> onResult: stopping application failed");
            onApplicationStopFailed(status.getStatusCode());
            return;
        } else
        {
            LogUtils.LOGD(BaseCastManager.access$000(), "stopApplication -> onResult Stopped application successfully");
            return;
        }
    }
