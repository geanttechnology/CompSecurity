    ...
    implements Runnable
{
    ...
    public void run()
    {
        if (val$request.aQ() == null)
        {
            if (getState() == e.DISMISSED || getState() == e.DISMISSING)
            {
                return;
            }
            if (val$request.aR() != null)
            {
                setActivityContent(val$request.aR().bA());
            }
        ...
        {
            Log.d("SessionM.Activity", (new StringBuilder()).append("Failed to load activity content, reason: ").append(val$request.aQ().getMessage()).toString());
        }
        ActivityController.access$400(_fld0, e.UNPRESENTABLE);
        loadContentListener.onFailure(val$request.aQ());
    }
