...
public class AdInstance extends EventCallbackHolder
    implements IAdInstance, IRendererContext, IRendererController
{
...
label0:
    ...
    private void dispatchEvent(final IEvent eventRef)
    {
        ...
        logger.debug((new StringBuilder()).append("process ad event:").append(eventRef.getType()).toString());
        s = eventRef.getType();
...
label0:
    ...
    public ArrayList getCompanionAdInstances()
    {
        ArrayList arraylist = new ArrayList();
        Iterator iterator = companionAds.iterator();
        do
        {
            if (!iterator.hasNext())
            {
            ...
            AdInstance adinstance = (AdInstance)iterator.next();
            if (!adinstance.ad.noLoad)
            {
                arraylist.add(adinstance);
            }
        } while (true);
        logger.debug((new StringBuilder()).append(this).append(" getCompanionAdInstances() ").append(arraylist).toString());
        return arraylist;
    }
...
_L5:
    ...
    public double getPlayheadTime()
    {
        ...
        } else
        {
            ...
            if (renderer != null)
            {
                try
                {
                    d = renderer.getPlayheadTime();
                }
                catch (NoSuchMethodError nosuchmethoderror)
                {
                    d = -1D;
                    logger.warn("The renderer has no getPlayheadTime implemented.");
                }
                catch (AbstractMethodError abstractmethoderror)
                {
                    d = -1D;
    ...
    public ArrayList getRenderableCreativeRenditions()
    {
        ...
        do
        {
            ...
            CreativeRendition creativerendition = (CreativeRendition)iterator.next();
            if (adRenderer == null || adRenderer.match(creativerendition.getBaseUnit(), ad.adUnit, creativerendition.getContentType(), Slot.slotTypeString(slot.timePositionClass), creativerendition.getCreativeAPI(), creativerendition.getWrapperType()))
            {
    ...
    {
        logger.debug((new StringBuilder()).append("processEvent(type=").append(s).append(")").toString());
        if (s.equals("defaultClick"))
        {
            dispatchEvent(s);
        }
