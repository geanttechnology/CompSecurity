    ...
    implements IRenderer, tv.freewheel.utils.renderer.RendererTimer.IRendererTimerService
{
    ...
    {
        logger.info((new StringBuilder()).append(MRAIDTAG()).append(" _expand()").toString());
        _expand(null);
    }

    private void _expand(String s)
    {
        logger.info((new StringBuilder()).append(MRAIDTAG()).append(" _expand(").append(s).append(")").toString());
        if (!isInterstitial)
        {
            ...
            {
                logger.error((new StringBuilder()).append(MRAIDTAG()).append(" Invalid state to expand").toString());
                return;
            }
        } else
        {
            logger.error((new StringBuilder()).append(MRAIDTAG()).append(" The expand operation of interstitial ad is not supported").toString());
            return;
        }
    }
    ...
    private void _open(String s)
    {
        logger.info((new StringBuilder()).append(MRAIDTAG()).append(" _open(").append(s).append(")").toString());
        if (s != null && s.length() != 0)
        {
            startWebBrowser(presentation.getAbsoluteURL(s));
            return;
        } else
        {
            logger.error((new StringBuilder()).append(MRAIDTAG()).append(" url is required").toString());
            return;
        }
    }
    ...
    {
        logger.info((new StringBuilder()).append(MRAIDTAG()).append(" _resize()").toString());
        ...
        if (isInterstitial)
        {
            logger.error((new StringBuilder()).append(MRAIDTAG()).append(" Cannot resize on interstitial ad").toString());
            return;
        } else
        {
    ...
    private void _setExpandProperties(String s)
    {
        int i;
        int j;
        logger.info((new StringBuilder()).append(MRAIDTAG()).append(" _setExpandProperties(").append(s).append(")").toString());
        if (s == null || s.isEmpty())
        {
            logger.error((new StringBuilder()).append(MRAIDTAG()).append(" Empty parameter, ignored").toString());
        ...
        try
        {
            ...
            i = (int)((float)s.optInt("width") * displaymetrics.density);
            j = (int)((float)s.optInt("height") * displaymetrics.density);
        }
...
_L1:
        if (i >= getScreenWidth()) goto _L4; else goto _L3
_L3:
        creativeRequiredExpandWidth = i;
_L7:
        if (j >= getScreenHeight()) goto _L6; else goto _L5
_L5:
        creativeRequiredExpandHeight = j;
_L8:
        isUseCustomClose = s.optBoolean("useCustomClose");
        return;
_L4:
        creativeRequiredExpandWidth = -1;
        logger.warn((new StringBuilder()).append(MRAIDTAG()).append(" The width parameter is equal or greater than screen width, using screen width instead").toString());
          goto _L7
_L6:
        creativeRequiredExpandHeight = -1;
        logger.warn((new StringBuilder()).append(MRAIDTAG()).append(" The height parameter is equal or greater than screen height, using screen height instead").toString());
          goto _L8
_L9:
        creativeRequiredExpandWidth = -1;
        creativeRequiredExpandHeight = -1;
        logger.warn((new StringBuilder()).append(MRAIDTAG()).append(" There are zero value in width or height, using screen width and height instead").toString());
          goto _L8
_L10:
        dispatchMraidError("Negative width or height", "setExpandProperties");
        return;
_L2:
        if (i != 0 && j != 0) goto _L10; else goto _L9
    }

    private void _setResizeProperties(String s)
    {
        logger.info((new StringBuilder()).append(MRAIDTAG()).append(" _setResizeProperties(").append(s).append(")").toString());
        if (s == null || s.isEmpty())
        {
            logger.error((new StringBuilder()).append(MRAIDTAG()).append(" Empty parameter, ignored").toString());
        ...
        s = activity.getResources().getDisplayMetrics();
        i = (int)((float)jsonobject.getInt("width") * ((DisplayMetrics) (s)).density);
        j = (int)((float)jsonobject.getInt("height") * ((DisplayMetrics) (s)).density);
        k = (int)((float)jsonobject.getInt("offsetX") * ((DisplayMetrics) (s)).density);
        l = (int)((float)jsonobject.getInt("offsetY") * ((DisplayMetrics) (s)).density);
        if (i < 50 || j < 50)
        {
            try
            {
        ...
        String s1 = jsonobject.optString("customClosePosition", "top-right");
        s = s1;
        if (closePositionOptions.contains(s1))
        {
            break MISSING_BLOCK_LABEL_273;
        }
        logger.warn((new StringBuilder()).append("Unknonw customClosePosition ").append(s1).append(", set to default top-right").toString());
        s = "top-right";
        creativeRequiredResizeWidth = i;
        creativeRequiredResizeHeight = j;
        creativeRequiredResizeOffsetX = k;
        creativeRequiredResizeOffsetY = l;
        customClosePosition = s;
        allowOffscreen = jsonobject.optBoolean("allowOffscreen", true);
        return;
    }
    ...
    private void _useCustomClose(boolean flag)
    {
        logger.info((new StringBuilder()).append(MRAIDTAG()).append("_useCustomClose(").append(flag).append(")").toString());
        isUseCustomClose = flag;
        IMRAIDPresentation imraidpresentation = presentation;
        if (!isUseCustomClose)
        {
            flag = true;
        } else
        ...
        imraidpresentation.setCloseButtonVisibility(flag);
    }
    ...
    private Long parseDateString(String s)
    {
        ...
        {
            s = (new StringBuilder()).append(s.substring(0, s.length() - 5)).append(s.substring(s.length() - 5).replace(":", "")).toString();
        }
        try
        {
            l = dateFormat.parse(s).getTime();
        }
        // Misplaced declaration of an exception variable
        catch (String s)
        {
    ...
    private void startWebBrowser(String s)
    {
        logger.debug((new StringBuilder()).append(CLASSTAG()).append(" startWebBrowser(").append(s).append(")").toString());
        if (s == null || s.length() == 0)
        {
            return;
        }
        if (isMRAIDAd)
        {
            ...
            {
                logger.debug((new StringBuilder()).append(CLASSTAG()).append(" It's already opened an external web browser.").toString());
                return;
            }
        ...
        {
            logger.debug("Sent EVENT_REQUEST_CONTENT_VIDEO_PAUSE");
        ...
        Bundle bundle = new Bundle();
        bundle.putString(constants.INFO_KEY_URL(), s);
        s = new HashMap();
        s.put(constants.INFO_KEY_EXTRA_INFO(), bundle);
...
_L5:
    ...
    {
        logger.info((new StringBuilder()).append(MRAIDTAG()).append(" createCalendarEvent(").append(s).append(")").toString());
        JSONObject jsonobject;
        jsonobject = new JSONObject(s);
        ...
        s = new Intent("android.intent.action.INSERT", Uri.parse("content://com.android.calendar/events"));
_L1:
        s.putExtra("title", jsonobject.optString("description")).putExtra("eventLocation", jsonobject.optString("location")).putExtra("description", jsonobject.optString("summary")).putExtra("beginTime", parseDateString(jsonobject.optString("start"))).putExtra("endTime", parseDateString(jsonobject.optString("end"))).putExtra("rrule", jsonobject.optString("recurrence"));
        activity.startActivity(s);
        return;
        try
        {
            s = (new Intent("android.intent.action.EDIT")).setType("vnd.android.cursor.item/event");
        }
        // Misplaced declaration of an exception variable
        catch (String s)
        {
    ...
    public void pause()
    {
        logger.info((new StringBuilder()).append(CLASSTAG()).append(" pause").toString());
        if (rendererTimer != null)
        {
            rendererTimer.pause();
        }
    ...
    public void playVideo(String s)
    {
        ...
        {
            dispatchMraidError((new StringBuilder()).append("No external player for playing video ").append(s).toString(), "playVideo");
        }
    }
    ...
    public void resume()
    {
        logger.info((new StringBuilder()).append(CLASSTAG()).append(" resume").toString());
        if (rendererTimer != null)
        {
            rendererTimer.resume();
        }
    ...
    public void setExpandProperties(final String propJsonString)
    {
        mainLoopHandler.post(new Runnable() {
            ...
            final HTMLRenderer this$0;
            final String val$propJsonString;

            public void run()
            {
                _setExpandProperties(propJsonString);
            }
    ...
    public void setResizeProperties(final String propJsonString)
    {
        mainLoopHandler.post(new Runnable() {
            ...
            public void run()
            {
                _setResizeProperties(propJsonString);
            }
...
/*
    static int access$1302(HTMLRenderer htmlrenderer, int i)
    {
        htmlrenderer.stateOfExternalWebBrowserActivity = i;
        return i;
    }
...
/*
    static boolean access$802(HTMLRenderer htmlrenderer, boolean flag)
    {
        htmlrenderer.pauseNotificationSentWhenExternalBrowserOpened = flag;
        return flag;
    }
