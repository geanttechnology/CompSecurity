...
_L5:
            MyBase64.encode3to4(abyte0, 0, j, buffer, 0, options);
            position = 0;
            numSigBytes = 4;
              goto _L2
_L6:
...
_L7:
            if (i == 4)
            {
                numSigBytes = MyBase64.decode4to3(abyte0, 0, buffer, 0, options);
                position = 0;
            } else
            if (i == 0)
            {
...
_L4:
        ...
        public InputStream(java.io.InputStream inputstream, int i)
        {
            ...
            position = -1;
            lineLength = 0;
            decodabet = MyBase64.getDecodabet(i);
        }
    }
...
label0:
            {
                if (position > 0)
                {
                    ...
                    out.write(MyBase64.encode3to4(b4, buffer, position, options));
                    position = 0;
                }
                return;
            }
...
_L2:
            ...
            if (position >= bufferLength)
            {
                out.write(MyBase64.encode3to4(b4, buffer, bufferLength, options));
...
_L5:
            i = MyBase64.decode4to3(buffer, 0, b4, 0, options);
            out.write(b4, 0, i);
            position = 0;
            return;
...
_L3:
    ...
    static 
    {
        byte abyte0[];
        boolean flag;
        if (!com/aio/downloader/utils/MyBase64.desiredAssertionStatus())
        {
            flag = true;
        } else
        {
