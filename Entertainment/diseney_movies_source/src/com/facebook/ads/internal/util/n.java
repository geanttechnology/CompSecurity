    ...
    private static void b(int ai[], int i, int k, int l, int i1, int j1, int k1)
    {
        ...
        if (k1 == 2)
        {
            ...
            while (i1 < k4) 
            {
                ...
                {
                    ai1[j1] = ai[i1];
                    l8 += (ai[i1] >>> 16 & 0xff) * (j1 + 1);
                    l7 += (ai[i1] >>> 8 & 0xff) * (j1 + 1);
                    l9 += (ai[i1] & 0xff) * (j1 + 1);
                    l6 += ai[i1] >>> 16 & 0xff;
                    l5 += ai[i1] >>> 8 & 0xff;
                    l4 += ai[i1] & 0xff;
                ...
                for (k1 = i1; j1 <= l; k1 = l1)
                {
                    ...
                    ai1[j1 + l] = ai[l1];
                    l8 += (ai[l1] >>> 16 & 0xff) * ((l + 1) - j1);
                    l7 += (ai[l1] >>> 8 & 0xff) * ((l + 1) - j1);
                    l9 += (ai[l1] & 0xff) * ((l + 1) - j1);
                    l10 += ai[l1] >>> 16 & 0xff;
                    l11 += ai[l1] >>> 8 & 0xff;
                    l12 += ai[l1] & 0xff;
                    j1++;
                ...
                {
                    ai[l1] = (int)((long)(ai[l1] & 0xff000000) | ((long)j4 * l9 >>> byte0 & 255L) << 16 | ((long)j4 * l8 >>> byte0 & 255L) << 8 | (long)j4 * l7 >>> byte0 & 255L);
                    k2 = (j1 + i4) - l;
                    k3 = k2;
                    if (k2 >= i4)
                    {
