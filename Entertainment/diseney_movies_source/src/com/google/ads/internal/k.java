...
public class k
{
        ...
        implements android.gesture.GestureOverlayView.OnGesturePerformedListener
    {
        ...
        {
            gestureoverlayview = a.recognize(gesture);
            Prediction prediction;
            for (gesture = gestureoverlayview.iterator(); gesture.hasNext(); com.google.ads.util.b.a((new StringBuilder()).append("Gesture: '").append(prediction.name).append("' = ").append(prediction.score).toString()))
            {
                prediction = (Prediction)gesture.next();
            }

            if (gestureoverlayview.size() == 0)
            {
                com.google.ads.util.b.a("Gesture: No remotely reasonable predictions");
            } else
            if (((Prediction)gestureoverlayview.get(0)).score > 2D && "debug".equals(((Prediction)gestureoverlayview.get(0)).name) && c != null)
            {
                if (c.c() == null)
                {
                    gestureoverlayview = "[No diagnostics available]";
                } else
                {
                    gestureoverlayview = c.c();
                }
                (new android.app.AlertDialog.Builder(b)).setMessage(gestureoverlayview).setTitle("Ad Information").setPositiveButton("Share", new android.content.DialogInterface.OnClickListener(this, gestureoverlayview) {
                    ...
                    {
                        com.google.ads.internal.a.b(b).startActivity(Intent.createChooser((new Intent("android.intent.action.SEND")).setType("text/plain").putExtra("android.intent.extra.TEXT", a), "Share via"));
                    }
            ...
            
            {
                ...
                }).setNeutralButton("Report", new android.content.DialogInterface.OnClickListener(this) {

                    final a a;

                    public void onClick(DialogInterface dialoginterface, int i)
                    {
                        (new Thread(new j(a.a(a).d(), com.google.ads.internal.a.b(a).getApplicationContext()))).start();
                    }
            ...
            
            {
                ...
                }).setNegativeButton("Close", new android.content.DialogInterface.OnClickListener(this) {
        ...
        public a(Activity activity, d d1, GestureStore gesturestore)
        {
            b = activity;
            c = d1;
            a = gesturestore;
    ...
    public static ViewGroup a(Activity activity, d d)
    {
        GestureOverlayView gestureoverlayview;
        try
        {
            GestureStore gesturestore = new GestureStore();
            gesturestore.load(new ByteArrayInputStream(a));
            ...
            gestureoverlayview.addOnGesturePerformedListener(new a(activity, d, gesturestore));
        }
        // Misplaced declaration of an exception variable
        catch (Activity activity)
        {
            b.a("Gestures disabled: Error loading gesture data.", activity);
            return null;
        }
