    ...
    implements HeaderExtractor
{
    ...
    public String extract(OAuthRequest oauthrequest)
    {
        ...
        oauthrequest = new StringBuilder(((Map) (obj)).size() * 20);
        oauthrequest.append("OAuth ");
        java.util.Map.Entry entry;
        for (obj = ((Map) (obj)).entrySet().iterator(); ((Iterator) (obj)).hasNext(); oauthrequest.append(String.format("%s=\"%s\"", new Object[] {
    entry.getKey(), OAuthEncoder.encode((String)entry.getValue())
})))
        {
            entry = (java.util.Map.Entry)((Iterator) (obj)).next();
            if (oauthrequest.length() > "OAuth ".length())
            {
                oauthrequest.append(", ");
            }
        }

        return oauthrequest.toString();
    }
}
