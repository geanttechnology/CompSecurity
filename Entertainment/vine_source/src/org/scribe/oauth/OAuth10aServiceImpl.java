    ...
    implements OAuthService
{
    ...
    private void addOAuthParams(OAuthRequest oauthrequest, Token token)
    {
        ...
        config.log((new StringBuilder()).append("appended additional OAuth parameters: ").append(MapUtils.toString(oauthrequest.getOauthParameters())).toString());
    }
...
_L4:
    ...
    private String getSignature(OAuthRequest oauthrequest, Token token)
    {
        ...
        config.log((new StringBuilder()).append("using base64 encoder: ").append(Base64Encoder.type()).toString());
        ...
        config.log((new StringBuilder()).append("base string is: ").append(oauthrequest).toString());
        config.log((new StringBuilder()).append("signature is: ").append(token).toString());
        return token;
    }
    ...
    {
        config.log((new StringBuilder()).append("obtaining access token from ").append(api.getAccessTokenEndpoint()).toString());
        ...
        config.log((new StringBuilder()).append("setting token to: ").append(token).append(" and verifier to: ").append(verifier).toString());
    ...
    {
        config.log((new StringBuilder()).append("obtaining request token from ").append(api.getRequestTokenEndpoint()).toString());
        Object obj = new OAuthRequest(api.getRequestTokenVerb(), api.getRequestTokenEndpoint());
        config.log((new StringBuilder()).append("setting oauth_callback to ").append(config.getCallback()).toString());
        ...
        config.log((new StringBuilder()).append("response status code: ").append(requesttuner.getCode()).toString());
        config.log((new StringBuilder()).append("response body: ").append(((String) (obj))).toString());
    ...
    {
        config.log((new StringBuilder()).append("signing request: ").append(oauthrequest.getCompleteUrl()).toString());
        if (!token.isEmpty())
        {
            oauthrequest.addOAuthParameter("oauth_token", token.getToken());
        }
        config.log((new StringBuilder()).append("setting token to: ").append(token).toString());
        addOAuthParams(oauthrequest, token);
        appendSignature(oauthrequest);
    }
}
