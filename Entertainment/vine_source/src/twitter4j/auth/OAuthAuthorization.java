...
_L7:
    ...
    String generateAuthorizationHeader(String s, String s1, HttpParameter ahttpparameter[], String s2, String s3, OAuthToken oauthtoken)
    {
        ...
        if (LOGGABLE)
        {
            Log.d("t4jOAuthAuth", (new StringBuilder()).append("OAuth base string: ").append(s).toString());
        }
        s = generateSignature(s, oauthtoken);
        if (LOGGABLE)
        {
            Log.d("t4jOAuthAuth", (new StringBuilder()).append("OAuth signature: ").append(s).toString());
        }
    ...
    String generateSignature(String s, OAuthToken oauthtoken)
    {
        ...
        catch (String s)
        {
            if (LOGGABLE)
            {
                Log.e("t4jOAuthAuth", "Failed initialize \"Message Authentication Code\" (MAC)", s);
            }
        ...
        catch (String s)
        {
            if (LOGGABLE)
            {
                Log.e("t4jOAuthAuth", "Failed to get HmacSHA1 \"Message Authentication Code\" (MAC)", s);
            }
            throw new AssertionError(s);
        }
