// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.comcast.cim.android.authentication;

import android.widget.ViewFlipper;

// Referenced classes of package com.comcast.cim.android.authentication:
//            AuthenticationActivity

class this._cls0
    implements Runnable
{

    final AuthenticationActivity this$0;

    public void run()
    {
        if (AuthenticationActivity.access$200(AuthenticationActivity.this) != null)
        {
            AuthenticationActivity.access$200(AuthenticationActivity.this).setDisplayedChild(1);
        }
    }

    ()
    {
        this$0 = AuthenticationActivity.this;
        super();
    }
}
