    ...
    implements Runnable
{
    ...
    final leDownloadFinished this$1;
    final IVirtuosoAsset val$file;

    public void run()
    {
        DownloadServiceManager.access$000().debug((new StringBuilder()).append("File Progress is : ").append(val$file.currentSize()).append(":").append(val$file.expectedSize()).append(", percent complete: ").append(val$file.fractionComplete()).toString());
        ...
        implements IQueueObserver
    {
        ...
        public void downloadEngineDidFinishDownloadingFile(IVirtuosoIdentifier ivirtuosoidentifier)
        {
            ...
            if (ivirtuosoidentifier.type() == 4)
            {
                ...
                backgroundExecutor.execute(ivirtuosoidentifier. new DownloadServiceManager._cls4._cls3());
            }
        }
