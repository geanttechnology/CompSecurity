...
public class ArrayNode extends ContainerNode
{
    ...
    protected ArrayNode(JsonNodeFactory jsonnodefactory, List list)
    {
        super(jsonnodefactory);
        _children = list;
    }
    ...
    protected List _createList(int i)
    {
    ...
    protected ArrayNode _defaultDeepCopy()
    {
        ...
        List list = _createList(Math.max(4, j));
        for (int i = 0; i < j; i++)
        {
            list.add(((JsonNode)_children.get(i)).deepCopy());
        }

        return new ArrayNode(_nodeFactory, list);
    }
    ...
    public ArrayNode addAll(Collection collection)
    {
        if (collection.size() > 0)
        {
            if (_children == null)
            {
                _children = _createList(collection.size());
            }
    ...
    {
        if (getClass() != com/fasterxml/jackson/databind/node/ArrayNode)
        {
            throw new IllegalStateException((new StringBuilder()).append("ArrayNode subtype (").append(getClass().getName()).append(" does not override deepCopy(), needs to").toString());
        } else
        {
