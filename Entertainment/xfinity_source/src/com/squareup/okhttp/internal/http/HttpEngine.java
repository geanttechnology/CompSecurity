...
//            CacheRequest, RealResponseBody, OkHeaders, RequestException, 
//            RouteException, RouteSelector, CacheStrategy, HttpMethod, 
//            Transport, RetryableSink
...
public final class HttpEngine
{
        ...
        implements com.squareup.okhttp.Interceptor.Chain
    {
            ...
            throws IOException
        {
            ...
            } else
            {
                ...
                int i = request1.code();
                if ((i == 204 || i == 205) && request1.body().contentLength() > 0L)
                {
                    throw new ProtocolException((new StringBuilder()).append("HTTP ").append(i).append(" had non-zero Content-Length: ").append(request1.body().contentLength()).toString());
                }
            }
            return request1;
        }
    ...
    private boolean isRecoverable(RouteException routeexception)
    {
        ...
        {
            if (!((routeexception = routeexception.getLastConnectException()) instanceof ProtocolException) && !(routeexception instanceof InterruptedIOException) && (!(routeexception instanceof SSLHandshakeException) || !(routeexception.getCause() instanceof CertificateException)) && !(routeexception instanceof SSLPeerUnverifiedException))
            {
                return true;
            }
        }
    ...
    {
        while (!client.getRetryOnConnectionFailure() || (ioexception instanceof ProtocolException) || (ioexception instanceof InterruptedIOException)) 
        {
            return false;
        }
        return true;
...
_L6:
    ...
    public HttpEngine recover(RouteException routeexception)
    {
        ...
        if (routeSelector == null && connection == null || routeSelector != null && !routeSelector.hasNext() || !isRecoverable(routeexception))
        {
            return null;
        } else
        {
            routeexception = close();
            return new HttpEngine(client, userRequest, bufferRequestBody, callerWritesRequestBody, forWebSocket, routeexception, routeSelector, (RetryableSink)requestBodyOut, priorResponse);
        }
    }
...
/*
    static Request access$202(HttpEngine httpengine, Request request)
    {
