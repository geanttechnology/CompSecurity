...
public class QueueFile
{
    ...
    private static final Logger LOGGER = Logger.getLogger(com/squareup/tape/QueueFile.getName());
    ...
    private void ringErase(int i, int j)
        throws IOException
    {
        while (j > 0) 
        {
            int k = Math.min(j, ZEROES.length);
            ringWrite(i, ZEROES, 0, k);
            j -= k;
            i += k;
        }
    }
