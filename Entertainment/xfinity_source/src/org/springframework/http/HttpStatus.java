...
{
    public static final class Series extends Enum
    {

        private static final Series $VALUES[];
        ...
        public static Series valueOf(String s)
        {
            return (Series)Enum.valueOf(org/springframework/http/HttpStatus$Series, s);
        }

        private static Series valueOf(HttpStatus httpstatus)
        {
            int j = httpstatus.value() / 100;
            Series aseries[] = values();
            int k = aseries.length;
            for (int i = 0; i < k; i++)
            {
                Series series1 = aseries[i];
                if (series1.value == j)
                {
                    return series1;
                }
        ...
        static 
        {
            ...
            SERVER_ERROR = new Series("SERVER_ERROR", 4, 5);
            $VALUES = (new Series[] {
        ...
        private Series(String s, int i, int j)
        {
            super(s, i);
            value = j;
        }
    ...
    private HttpStatus(String s, int i, int j, String s1)
    {
        super(s, i);
        value = j;
        reasonPhrase = s1;
    ...
    {
        HttpStatus ahttpstatus[] = values();
        int k = ahttpstatus.length;
        for (int j = 0; j < k; j++)
        {
            HttpStatus httpstatus = ahttpstatus[j];
        ...
        throw new IllegalArgumentException((new StringBuilder()).append("No matching constant for [").append(i).append("]").toString());
    }

    public static HttpStatus valueOf(String s)
    {
        return (HttpStatus)Enum.valueOf(org/springframework/http/HttpStatus, s);
    }
    ...
    static 
    {
        ...
        IM_USED = new HttpStatus("IM_USED", 13, 226, "IM Used");
        ...
        EXPECTATION_FAILED = new HttpStatus("EXPECTATION_FAILED", 40, 417, "Expectation Failed");
        I_AM_A_TEAPOT = new HttpStatus("I_AM_A_TEAPOT", 41, 418, "I'm a teapot");
        INSUFFICIENT_SPACE_ON_RESOURCE = new HttpStatus("INSUFFICIENT_SPACE_ON_RESOURCE", 42, 419, "Insufficient Space On Resource");
        METHOD_FAILURE = new HttpStatus("METHOD_FAILURE", 43, 420, "Method Failure");
        DESTINATION_LOCKED = new HttpStatus("DESTINATION_LOCKED", 44, 421, "Destination Locked");
        UNPROCESSABLE_ENTITY = new HttpStatus("UNPROCESSABLE_ENTITY", 45, 422, "Unprocessable Entity");
        LOCKED = new HttpStatus("LOCKED", 46, 423, "Locked");
        FAILED_DEPENDENCY = new HttpStatus("FAILED_DEPENDENCY", 47, 424, "Failed Dependency");
        ...
        NETWORK_AUTHENTICATION_REQUIRED = new HttpStatus("NETWORK_AUTHENTICATION_REQUIRED", 63, 511, "Network Authentication Required");
