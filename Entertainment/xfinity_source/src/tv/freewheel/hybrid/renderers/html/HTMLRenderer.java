...
public class HTMLRenderer
    implements IRenderer, tv.freewheel.hybrid.utils.renderer.RendererTimer.IRendererTimerService
{
    private static final class MRAIDState extends Enum
    {
        ...
        {
            return (MRAIDState)Enum.valueOf(tv/freewheel/hybrid/renderers/html/HTMLRenderer$MRAIDState, s);
        }
    ...
    private void _load(IRendererContext irenderercontext)
    {
        ...
        logger.debug((new StringBuilder()).append("Display size: ").append(displaymetrics.widthPixels).append("x").append(displaymetrics.heightPixels).append(" px, app size: ").append(getAppWidth()).append("x").append(getAppHeight()).toString());
        Object obj = irenderercontext.getAdInstance().getActiveCreativeRendition().getCreativeAPI();
        ...
        s1 = ((ICreativeRenditionAsset) (obj)).getURL();
        if (s1 != null && s1.length() != 0)
        {
            presentation.loadURL(s1);
        } else
        {
            String s2 = ((ICreativeRenditionAsset) (obj)).getContent();
            if (s2 != null && s2.length() != 0)
            {
                String s = s2;
                if (i == constants.TIME_POSITION_CLASS_DISPLAY())
                {
                    s = s2;
                    if (parameters.shouldUseDip.booleanValue())
                    {
                        float f = displaymetrics.density;
                        int l = (int)((float)slot.getWidth() * f);
                        int i1 = (int)((float)slot.getHeight() * f);
                        int k = i1;
                        int j = l;
                        if (l > getAppWidth())
                        {
                            k = (int)((float)(getAppWidth() * i1) / (float)l);
                            j = getAppWidth();
                        }
                        i1 = k;
                        l = j;
                        if (k > getAppHeight())
                        {
                            l = (int)((float)(getAppHeight() * j) / (float)k);
                            i1 = getAppHeight();
                        }
                        logger.info((new StringBuilder()).append("Convert display size ").append(slot.getWidth()).append("x").append(slot.getHeight()).append("dp to ").append(l).append("x").append(i1).append("px").toString());
                        s = s2.replaceAll("\\swidth=\"[1-9]{1}[0-9]*\"", (new StringBuilder()).append(" width=\"").append(String.valueOf(l)).append("\"").toString()).replaceAll("\\sheight=\"[1-9]{1}[0-9]*\"", (new StringBuilder()).append(" height=\"").append(String.valueOf(i1)).append("\"").toString());
                    }
                }
                presentation.loadContent(s);
            } else
            {
    ...
    private String getPrintableState(MRAIDState mraidstate)
    {
        static class _cls14
        {
            ...
            static final int $SwitchMap$tv$freewheel$hybrid$renderers$html$HTMLRenderer$MRAIDState[];

            static 
            {
                $SwitchMap$tv$freewheel$hybrid$renderers$html$HTMLRenderer$MRAIDState = new int[MRAIDState.values().length];
                try
                {
                    $SwitchMap$tv$freewheel$hybrid$renderers$html$HTMLRenderer$MRAIDState[MRAIDState.LOADING.ordinal()] = 1;
                }
        ...
        switch (_cls14..SwitchMap.tv.freewheel.hybrid.renderers.html.HTMLRenderer.MRAIDState[mraidstate.ordinal()])
        {
        default:
            return null;
