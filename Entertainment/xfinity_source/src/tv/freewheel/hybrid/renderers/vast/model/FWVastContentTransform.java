...
public class FWVastContentTransform
{
    ...
    public static String image_TO_text_html_doc_lit_mobile(String s, int i, int j, String s1)
    {
        ...
        if (j > 0)
        {
            s3 = (new StringBuilder()).append("").append(j).toString();
        }
        instance.macros.put("#{ad.creative.width}", s2);
        instance.macros.put("#{ad.creative.height}", s3);
        s2 = s1;
        if (s1 == null)
        {
            s2 = "";
        }
        instance.macros.put("#{altText_VAST}", s2);
        return instance.injectContent(s).ImgRef_to_HTMLLit().HTMLLit_to_HTMLLit_deco().HTMLLit_to_HTMLDocLit_mobile().retrieveContent();
    }
