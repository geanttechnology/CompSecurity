    ...
    implements IRenderer
{
    ...
    private double timeoutMillisecondsForAdFreeze;
    private Timer timer;
    ...
    public VideoRenderer()
    {
        ...
        freezeCount = 0;
        timeoutMillisecondsForAdFreeze = 10000D;
        timeoutCountForAdFreeze = 0.0D;
    ...
    {
        if ((double)freezeCount >= timeoutCountForAdFreeze)
        {
            logger.warn((new StringBuilder()).append("ad content is unexpected paused for ").append(timeoutMillisecondsForAdFreeze).append("ms, just fail!").toString());
            freezeCount = 0;
            Bundle bundle = new Bundle();
            bundle.putString(constants.INFO_KEY_ERROR_CODE(), constants.ERROR_TIMEOUT());
            bundle.putString(constants.INFO_KEY_ERROR_INFO(), (new StringBuilder()).append("ad content is unexpected paused for ").append(timeoutMillisecondsForAdFreeze / 1000D).append("s").toString());
...
_L2:
    ...
    private void onImprTimer()
    {
        ...
        } else
        {
            ...
            if (!isPaused)
            {
                if (d2 - lastPlayheadTime < 0.10000000000000001D)
                {
    ...
    {
        logger.debug("startQuartileImpressionAndTimeoutTimer");
        if (timer != null)
        {
            return;
        } else
    ...
    public void load(IRendererContext irenderercontext)
    {
        ...
        obj1 = rendererContext.getParameter("renderer.video.playbackUnexpectedPauseTimeout");
        if (obj != null)
        {
        ...
        if (obj1 != null)
        {
            ...
            if (d1 > 0.0D)
            {
                timeoutMillisecondsForAdFreeze = d1;
            }
        }
        timeoutCountBeforeStart = (int)(timeoutMillisecondsBeforeStart / 500D);
        timeoutCountForAdFreeze = (int)(timeoutMillisecondsForAdFreeze / 500D);
        ...
        if (obj == null)
        {
            ...
            obj1 = new HashMap();
            ((HashMap) (obj1)).put(constants.INFO_KEY_EXTRA_INFO(), obj);
            irenderercontext.dispatchEvent(constants.EVENT_ERROR(), ((HashMap) (obj1)));
            return;
        }
        ...
        if (!((URI) (obj1)).isAbsolute())
        {
            ...
            hashmap = new HashMap();
            hashmap.put(constants.INFO_KEY_EXTRA_INFO(), obj1);
