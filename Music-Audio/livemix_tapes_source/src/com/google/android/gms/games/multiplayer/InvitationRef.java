    ...
    implements Invitation
{
    ...
    InvitationRef(DataHolder dataholder, int i, int j)
    {
        ...
        for (; i < j; i++)
        {
            ParticipantRef participantref = new ParticipantRef(zzYX, zzabg + i);
            if (participantref.getParticipantId().equals(s))
            {
                dataholder = participantref;
            }
            zzayc.add(participantref);
        }

        zzayf = (ParticipantRef)zzx.zzb(dataholder, "Must have a valid inviter!");
    }
