...
public class zzkk
{
    ...
    private static final String zzXV = (new StringBuilder()).append("yyyyMMdd'T'HHmmss").append(zzXU[0]).toString();
    ...
    public static String zza(Calendar calendar)
    {
        ...
        } else
        {
            ...
            {
                return ((String) (obj)).replace("+0000", zzXU[0]);
            }
        }
        return calendar;
    }
...
_L1:
    ...
    public static Calendar zzbS(String s)
    {
        ...
        {
            zzTy.zzb("Error parsing string: %s", new Object[] {
                s.getMessage()
            });
            return null;
        }
    ...
    private static String zzbT(String s)
    {
        ...
        {
            zzTy.zze("Error extracting the date: %s", new Object[] {
                s.getMessage()
            });
            return null;
        }
...
_L2:
        ...
        {
            zzTy.zzb("Error extracting the time substring: %s", new Object[] {
                s.getMessage()
            });
            return null;
        }
...
_L3:
        return null;
_L4:
        if (!zzbV(s)) goto _L7; else goto _L6
_L6:
        return s.replaceAll("([\\+\\-]\\d\\d):(\\d\\d)", "$1$2");
        if (s.length() != "HHmmss".length() + zzXU[0].length()) goto _L7; else goto _L8
_L8:
    ...
    private static boolean zzbV(String s)
    {
        int i = s.length();
        int j = "HHmmss".length();
        return i == zzXU[1].length() + j || i == zzXU[2].length() + j || i == j + zzXU[3].length();
    }
