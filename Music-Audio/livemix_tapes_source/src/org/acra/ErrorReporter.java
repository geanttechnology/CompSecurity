    ...
    implements Thread.UncaughtExceptionHandler
{
    ...
    ErrorReporter(Application application, SharedPreferences sharedpreferences, boolean flag)
    {
        enabled = false;
    ...
    private void deletePendingReports(boolean flag, boolean flag1, int i)
    {
        ...
        if (as != null)
        {
            for (int j = 0; j < as.length - i; j++)
            {
                ...
                ACRA.log.d(ACRA.LOG_TAG, (new StringBuilder()).append("Deleting file ").append(s).toString());
                if (!file.delete())
                {
    ...
    private void endApplication()
    {
        ...
        if (lastActivityCreated != null)
        {
            ...
            Log.i(ACRA.LOG_TAG, (new StringBuilder()).append("Finished ").append(lastActivityCreated.getClass()).toString());
    ...
    private void notifySendReport(String s)
    {
        ...
        s = PendingIntent.getActivity(s, i, intent, 0x8000000);
        notification.setLatestEventInfo(mContext, charsequence, ((CharSequence) (obj)), s);
        s = new Intent(mContext, org/acra/CrashReportDialog);
        s.putExtra("FORCE_CANCEL", true);
        notification.deleteIntent = PendingIntent.getActivity(mContext, -1, s, 0);
...
/*
    static Activity access$002(ErrorReporter errorreporter, Activity activity)
    {
