...
_L7:
    ...
    private void sendImageRequest(boolean flag)
    {
        try
        {
            ImageRequest imagerequest = (new ImageRequest.Builder(getContext(), ImageRequest.getProfilePictureUrl(profileId, queryWidth, queryHeight))).setAllowCachedRedirects(flag).setCallerTag(this).setCallback(new _cls1()).build();
            if (lastRequest != null)
            {
