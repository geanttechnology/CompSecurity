...
_L2:
        ...
        {
            Iterator iterator = annotatedclass.iterator();
            while (iterator.hasNext()) 
            {
                namedtype = (NamedType)iterator.next();
                AnnotatedClass annotatedclass1 = AnnotatedClass.constructWithoutSuperTypes(namedtype.getType(), annotationintrospector, mapperconfig);
                annotatedclass = namedtype;
                if (!namedtype.hasName())
                {
                    annotatedclass = new NamedType(namedtype.getType(), annotationintrospector.findTypeName(annotatedclass1));
                }
