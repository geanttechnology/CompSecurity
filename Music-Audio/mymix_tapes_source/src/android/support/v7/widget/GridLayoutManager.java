...
_L5:
        ...
        while (k < k2) 
        {
            ...
            i1 = getSpanSize(recycler, state, getPosition(view1));
            i1 = android.view.View.MeasureSpec.makeMeasureSpec(mSizePerSpan * i1, 0x40000000);
            ...
            l1 = mOrientationHelper.getDecoratedMeasurement(view1);
            i1 = j;
            if (l1 > j)
            {
                i1 = l1;
        ...
        while (k < k2) 
        {
            ...
            {
                int i2 = getSpanSize(recycler, state, getPosition(view2));
                i2 = android.view.View.MeasureSpec.makeMeasureSpec(mSizePerSpan * i2, 0x40000000);
                if (mOrientation == 1)
                {
                    measureChildWithDecorationsAndMargin(view2, i2, j1);
                } else
                {
                    measureChildWithDecorationsAndMargin(view2, j1, i2);
                }
            }
            k++;
        }
        ...
        while (k < k2) 
        {
            ...
            if (mOrientation == 1)
            {
                j2 = getPaddingLeft() + mSizePerSpan * ((LayoutParams) (state)).mSpanIndex;
                j1 = j2 + mOrientationHelper.getDecoratedMeasurementInOther(recycler);
            } else
            {
                l2 = getPaddingTop() + mSizePerSpan * ((LayoutParams) (state)).mSpanIndex;
