...
//            VideoViewabilityTracker, VastErrorCode
    ...
    implements Serializable
{
    ...
    private void handleClick(final Context context, int i, final Integer requestCode)
    {
        Preconditions.checkNotNull(context, "context cannot be null");
        TrackingRequest.makeVastTrackingHttpRequest(mClickTrackers, null, Integer.valueOf(i), mNetworkMediaFileUrl, context);
        ...
        } else
        {
            (new com.mopub.common.UrlHandler.Builder()).withSupportedUrlActions(UrlAction.IGNORE_ABOUT_SCHEME, new UrlAction[] {
                ...
                public void urlHandlingSucceeded(String s, UrlAction urlaction)
                {
                    ...
                    s = Intents.getStartActivityIntent(context, com/mopub/common/MoPubBrowser, urlaction);
                    if (context instanceof Activity)
                    {
                        Preconditions.checkNotNull(requestCode);
                        ((Activity)context).startActivityForResult(s, requestCode.intValue());
                        return;
                    }
                    try
                    {
                        Intents.startActivity(context, s);
                        return;
                    }
            ...
            }).withoutMoPubBrowser().build().handleUrl(context, mClickThroughUrl);
            return;
        }
    }
...
_L4:
    ...
    public void handleClickForResult(Activity activity, int i, int j)
    {
        handleClick(activity, i, Integer.valueOf(j));
    }
