...
//            FindCallback, ParseQueryAdapter, OfflineStore, ParseQuery, 
//            ParseException

class val.page extends FindCallback
{

    final ParseQueryAdapter this$0;
    final Capture val$firstCallBack;
    final int val$page;
    ...
    {
        if (!OfflineStore.isEnabled() && val$query.getCachePolicy() == cy.CACHE_ONLY && parseexception != null && parseexception.getCode() == 120)
        {
            return;
        }
...
_L2:
        if (list != null)
        {
            if (val$shouldClear && ((Boolean)val$firstCallBack.get()).booleanValue())
            {
                ...
                val$firstCallBack.set(Boolean.valueOf(false));
            }
