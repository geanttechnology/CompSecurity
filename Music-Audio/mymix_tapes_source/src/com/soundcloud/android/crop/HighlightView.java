...
class HighlightView
{
    ...
    private void initStyles(Context context)
    {
        ...
        showThirds = context.getBoolean(1, false);
        highlightColor = context.getColor(0, 0xff33b5e5);
        handleMode = HandleMode.values()[context.getInt(2, 0)];
        context.recycle();
        return;
        Exception exception;
        exception;
    ...
    void growBy(float f, float f1)
    {
        ...
        if (f3 > 0.0F)
        {
            f = f3;
            f1 = f2;
            if (rectf.width() + 2.0F * f3 > imageRect.width())
            {
                f3 = (imageRect.width() - rectf.width()) / 2.0F;
                f = f3;
                f1 = f2;
                if (maintainAspectRatio)
                {
                    f1 = f3 / initialAspectRatio;
                    f = f3;
                }
            }
        }
