// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.amazon.device.ads;

import java.io.File;
import java.io.IOException;
import java.io.OutputStream;

// Referenced classes of package com.amazon.device.ads:
//            cm, be, dn, ck, 
//            co, dq, bm, bo, 
//            cn, cl

class aw
{
    final class a
    {

        boolean a;
        final aw b;

        a()
        {
            b = aw.this;
            super();
            a = false;
        }
    }


    private static final String a = com/amazon/device/ads/aw.getSimpleName();
    private static aw b = new aw(cm.a(), new be(), dn.a(), ck.a());
    private String c;
    private final cm d;
    private final bm e;
    private final dn f;
    private final ck g;
    private final cn h;

    private aw(cm cm1, bm bm1, dn dn1, ck ck1)
    {
        new co();
        h = co.a(a);
        d = cm1;
        e = bm1;
        f = dn1;
        g = ck1;
        c = c();
    }

    public static final aw a()
    {
        return b;
    }

    private void a(a a1, String s, String s1)
    {
        if (dq.b(s))
        {
            throw new IllegalArgumentException("Encoded String must not be null or white space");
        }
        int i = s.indexOf("=");
        byte abyte0[];
        IOException ioexception;
        int j;
        int k;
        int l;
        int i1;
        byte byte0;
        if (i >= 0)
        {
            i = s.length() - i;
        } else
        {
            i = 0;
        }
        l = ((s.length() + 3) / 4) * 3 - i;
        if (l <= 0)
        {
            throw new IllegalArgumentException("Encoded String decodes to zero bytes");
        }
        abyte0 = new byte[l];
        j = 0;
        i = 0;
        if (j >= s.length() || i >= l || j % 4 == 0 && s.length() < j + 4)
        {
            break MISSING_BLOCK_LABEL_333;
        }
        i1 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".indexOf(s.charAt(j));
        if (i1 == -1)
        {
            break MISSING_BLOCK_LABEL_333;
        }
        switch (j % 4)
        {
        default:
            break;

        case 0: // '\0'
            break; /* Loop/switch isn't completed */

        case 1: // '\001'
            k = i + 1;
            abyte0[i] = (byte)(abyte0[i] | (byte)(i1 >> 4 & 3));
            i = k;
            if (k < l)
            {
                abyte0[k] = (byte)(i1 << 4);
                i = k;
            }
            continue; /* Loop/switch isn't completed */

        case 2: // '\002'
            k = i + 1;
            abyte0[i] = (byte)(abyte0[i] | (byte)(i1 >> 2 & 0xf));
            i = k;
            if (k < l)
            {
                abyte0[k] = (byte)(i1 << 6);
                i = k;
            }
            continue; /* Loop/switch isn't completed */

        case 3: // '\003'
            break;
        }
        break MISSING_BLOCK_LABEL_298;
_L4:
        j++;
        if (true) goto _L2; else goto _L1
_L2:
        break MISSING_BLOCK_LABEL_81;
_L1:
        abyte0[i] = (byte)(i1 << 2);
        continue; /* Loop/switch isn't completed */
        k = i + 1;
        byte0 = abyte0[i];
        abyte0[i] = (byte)((byte)(i1 & 0x3f) | byte0);
        i = k;
        if (true) goto _L4; else goto _L3
_L3:
        if (a1.a) goto _L6; else goto _L5
_L5:
        s = e.b(s1);
        if (!s.a(bo.a.b)) goto _L8; else goto _L7
_L7:
        s.f();
        ((bo) (s)).b.write(abyte0);
_L10:
        s.close();
_L6:
        return;
        ioexception;
        h.c("Caught IOException while writing asset to disk: %s", new Object[] {
            s1
        });
_L8:
        a1.a = true;
        if (true) goto _L10; else goto _L9
_L9:
    }

    private String c()
    {
        if (d != null && d.i != null)
        {
            return (new StringBuilder()).append(d.i.getAbsolutePath()).append(File.separator).toString();
        } else
        {
            return null;
        }
    }

    public final String a(String s)
    {
        return (new StringBuilder()).append(c).append(s).toString();
    }

    public final boolean b()
    {
        boolean flag = false;
        boolean flag1 = false;
        this;
        JVM INSTR monitorenter ;
        if (c != null) goto _L2; else goto _L1
_L1:
        Object obj;
        c = c();
        obj = c;
        if (obj != null) goto _L2; else goto _L3
_L3:
        this;
        JVM INSTR monitorexit ;
        return flag1;
_L2:
        long l = System.nanoTime();
        if (f.a("mraid-assets-version") >= 5) goto _L5; else goto _L4
_L4:
        h.b("Writing assets to disk: version %d", new Object[] {
            Integer.valueOf(5)
        });
        obj = new a();
        a(((a) (obj)), "", a("amazon_ads_close_normal.png"));
        a(((a) (obj)), "", a("amazon_ads_close_pressed.png"));
        a(((a) (obj)), "", a("amazon_ads_close.png"));
        a(((a) (obj)), "", a("amazon_ads_leftarrow.png"));
        a(((a) (obj)), "", a("amazon_ads_refresh.png"));
        a(((a) (obj)), "", a("amazon_ads_rightarrow.png"));
        a(((a) (obj)), "", a("amazon_ads_close_normal.png"));
        a(((a) (obj)), "", a("amazon_ads_close_pressed.png"));
        a(((a) (obj)), "", a("amazon_ads_open_external_browser.png"));
        cl cl1;
        if (!((a) (obj)).a)
        {
            flag = true;
        } else
        {
            flag = false;
        }
        if (!flag) goto _L7; else goto _L6
_L6:
        obj = f;
        ((dn) (obj)).a("mraid-assets-version", new dn.c(((dn) (obj)), java/lang/Integer, Integer.valueOf(5)));
        flag = true;
_L5:
        l = (System.nanoTime() - l) / 0xf4240L;
        cl1 = g.b;
        if (!flag)
        {
            break MISSING_BLOCK_LABEL_302;
        }
        obj = ck.a.l;
_L8:
        cl1.a(((ck.a) (obj)), l);
        flag1 = true;
          goto _L3
_L7:
        g.b.a(ck.a.n);
          goto _L3
        obj;
        throw obj;
        obj = ck.a.m;
          goto _L8
    }

}
