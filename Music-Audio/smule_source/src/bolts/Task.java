...
public class Task
{
    ...
    private static void completeAfterTask(final TaskCompletionSource tcs, final Continuation continuation, final Task task, Executor executor, final CancellationToken ct)
    {
        executor.execute(new Runnable() {
    ...
    private static void completeImmediately(final TaskCompletionSource tcs, final Continuation continuation, final Task task, Executor executor, final CancellationToken ct)
    {
        executor.execute(new Runnable() {
...
_L1:
    ...
    public static Task whenAll(Collection collection)
    {
        ...
        final AtomicInteger count = new AtomicInteger(collection.size());
        final AtomicBoolean isCancelled = new AtomicBoolean(false);
        for (collection = collection.iterator(); collection.hasNext(); ((Task)collection.next()).continueWith(new Continuation() {
    ...
    public static Task whenAllResult(final Collection tasks)
    {
        return whenAll(tasks).onSuccess(new Continuation() {
            ...
            public List then(Task task)
            {
                ...
                task = new ArrayList();
                for (Iterator iterator = tasks.iterator(); iterator.hasNext(); task.add(((Task)iterator.next()).getResult())) { }
                return task;
            }
