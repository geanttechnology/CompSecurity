...
public class zzaj
{
    ...
    static String zza(String as[], int i, int j)
    {
        if (as.length < i + j)
        {
            com.google.android.gms.ads.internal.util.client.zzb.zzak("Unable to construct shingle");
            return "";
        }
    ...
    public static void zza(String as[], int i, int j, PriorityQueue priorityqueue)
    {
        ...
        {
            l = zza(zzah.zzq(as[k - 1]), zzah.zzq(as[(k + j) - 1]), l, l1, 0x1001fffL);
    ...
    {
        long l = ((long)zzah.zzq(as[i]) + 0x7fffffffL) % 0x4000ffffL;
        for (int k = i + 1; k < i + j; k++)
        {
            l = ((l * 0x1001fffL) % 0x4000ffffL + ((long)zzah.zzq(as[k]) + 0x7fffffffL) % 0x4000ffffL) % 0x4000ffffL;
        }
