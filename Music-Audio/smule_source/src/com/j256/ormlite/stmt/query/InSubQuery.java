...
public class InSubQuery extends BaseComparison
{
    ...
    public InSubQuery(String s, FieldType fieldtype, com.j256.ormlite.stmt.QueryBuilder.InternalQueryBuilderWrapper internalquerybuilderwrapper)
    {
    ...
    public void appendValue(DatabaseType databasetype, StringBuilder stringbuilder, List list)
    {
        ...
        databasetype = subQueryBuilder.getResultFieldTypes();
        if (databasetype.length != 1)
        {
            throw new SQLException((new StringBuilder()).append("There must be only 1 result column in sub-query but we found ").append(databasetype.length).toString());
        }
        if (fieldType.getSqlType() != databasetype[0].getSqlType())
        {
            throw new SQLException((new StringBuilder()).append("Outer column ").append(fieldType).append(" is not the same type as inner column ").append(databasetype[0]).toString());
        } else
        {
            stringbuilder.append(") ");
            return;
