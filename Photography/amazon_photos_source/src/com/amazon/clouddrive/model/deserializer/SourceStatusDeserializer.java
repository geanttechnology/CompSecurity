...
//            JsonDeserializer, MediaTypeUploadStatusMapDeserializer, SimpleDeserializers

public class SourceStatusDeserializer
    implements JsonDeserializer
{

    public static final JsonDeserializer INSTANCE = new SourceStatusDeserializer();

    private SourceStatusDeserializer()
    {
    }

    public SourceStatus deserialize(JsonParser jsonparser)
        throws IOException
    {
...
_L2:
        ...
        do
        {
            ...
            {
                sourcestatus.setUploadStatus((Map)MediaTypeUploadStatusMapDeserializer.INSTANCE.deserialize(jsonparser));
            } else
            if ("isUploading".equals(obj))
            {
                sourcestatus.setIsUploading(SimpleDeserializers.deserializePrimitiveBoolean(jsonparser));
            } else
            if ("cacheSize".equals(obj))
            {
                sourcestatus.setCacheSize(SimpleDeserializers.deserializePrimitiveLong(jsonparser));
            } else
            if ("isDownloading".equals(obj))
            {
