// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.amazon.gallery.thor.whisperplay;

import com.amazon.gallery.thor.view.NoOpSidePanelListener;
import com.amazon.gallery.whisperplay.WhisperPlayConnectionManager;

public class ThorWhisperPlayPanelListener extends NoOpSidePanelListener
{

    public ThorWhisperPlayPanelListener(WhisperPlayConnectionManager whisperplayconnectionmanager)
    {
    }
}
