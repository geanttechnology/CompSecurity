    ...
    implements CustomerAttributeStoreDefinition
{
    ...
    public CustomerAttributeStoreLogic(ServiceWrappingContext servicewrappingcontext, DataStorage datastorage)
    {
        ...
        mAccountManagerDefinition = AccountManagerImplementationFactory.getAccountManagerImplementation(mContext);
    ...
    private boolean updatePackageCredentials(String s, KeyInfo keyinfo, Tracer tracer)
    {
        ...
        s = (Bundle)mAccountManagerDefinition.updateAccountWithKey$5e2b1462(s, keyinfo, tracer).get();
        return s != null;
        s;
...
_L1:
    ...
    public MAPFuture getAttribute(final String directedId, final String key, final Callback callbackFuture, Bundle bundle, final EnumSet options, final Tracer tracer)
    {
        ...
        if (options.contains(com.amazon.identity.auth.device.api.CustomerAttributeStore.GetAttributeOptions.ForceRefresh))
        {
            ...
            MAPLog.formattedWarning(TAG, "Packages %s is force refreshing key %s", new Object[] {
        ...
        ThreadUtils.submitToBackgroundThread(new Runnable() {
            ...
            public void run()
            {
                ...
                {
                    CustomerAttributeStoreLogic.access$000(CustomerAttributeStoreLogic.this, callbackFuture, directedId, keyinfo.getKey(), options, tracer);
                    return;
                }
                ...
                if (CustomerAttributeStoreKeyUtils.isXMainAndXACBCookies(keyinfo))
                {
                    ...
                    String s1 = ((isXFSNCookie) (this)).isXFSNCookie.getUserData(<returnValue>, 
        ...
        } else
        {
            ThreadUtils.submitToBackgroundThread(new Runnable() {
                ...
                public void run()
                {
                    ...
                    callbackWithValueDefaultBundleResult(callbackFuture, value);
                }
...
/*
    static void access$000(CustomerAttributeStoreLogic customerattributestorelogic, Callback callback, String s, String s1, EnumSet enumset, Tracer tracer)
    {
        if (enumset.contains(com.amazon.identity.auth.device.api.CustomerAttributeStore.GetAttributeOptions.ForceRefresh))
        {
            ...
            s = customerattributestorelogic.mCorPfmLogic.fetchCorPfm(s, tracer);
        } else
        {
            s = customerattributestorelogic.mCorPfmLogic.fetchCorPfmIfNotSet(s, tracer);
        }
        if (s == null)
        {
