...
public abstract class WebResponseParser
{
    ...
    private void checkAndChangeState(WebResponseParserState webresponseparserstate)
    {
        ...
        {
            MAPLog.formattedError(TAG, "%s: beginParse has been called more than once.", new Object[] {
                getParserName()
            });
            return;
        }
        if (mCurrentState == WebResponseParserState.Before_Parse)
        {
            ...
            {
                MAPLog.formattedError(TAG, "%s: parseBodyChunk called before beginParse", new Object[] {
                    getParserName()
                });
                return;
            }
            ...
            {
                MAPLog.formattedError(TAG, "%s: endParse called before beginParse", new Object[] {
                    getParserName()
                });
                return;
            }
        ...
        if (mCurrentState == WebResponseParserState.Begin_Parse)
        {
            ...
            {
                MAPLog.formattedError(TAG, "%s: shouldParseBody is false. parseBodyChunk should not be called", new Object[] {
                    getParserName()
                });
                return;
            }
        ...
        {
            MAPLog.formattedError(TAG, "%s: parseBodyChunk called after endParse", new Object[] {
                getParserName()
            });
            return;
        }
    ...
    public void beginParse(WebResponseHeaders webresponseheaders)
    {
        ...
        if (l < 200L || l >= 300L)
        {
            MAPLog.formattedInfo(TAG, "%s: HTTP Error: %d", new Object[] {
    ...
    public ParseError endParse()
    {
        ...
        {
            MAPLog.formattedError(TAG, "%s: endParse: called after another method returned a parse error.", new Object[] {
                getParserName()
            });
        ...
        if (getParseError() == ParseError.ParseErrorMalformedBody)
        {
            ...
            {
                MAPLog.formattedWarning(TAG, "%s: endParse called before parseBodyChunk. Confirm that this is by design.", new Object[] {
                    getParserName()
                });
            }
            MAPLog.formattedError(TAG, "%s: endParse: Malformed response. Confirm all received data is being properly passed to the parser, device capabilities are set properly, and no server-side behavior changes have occurred.", new Object[] {
                getParserName()
            });
        }
        return getParseError();
    ...
    public ParseError parseBodyChunk(byte abyte0[], long l)
    {
        ...
        if (getParseError() != ParseError.ParseErrorNoError)
        {
            MAPLog.formattedError(TAG, "%s: parseBodyChunk: called after another method returned a parse error.", new Object[] {
                getParserName()
            });
            return getParseError();
        }
        ...
        if (getParseError() == ParseError.ParseErrorMalformedBody)
        {
            MAPLog.formattedInfo(TAG, "%s: parseBodyChunk: Malformed response. confirm all received data is being properly passed to the parser, device capabilities are set properly, and no server-side behavior changes have occurred.", new Object[] {
                getParserName()
            });
        }
        return getParseError();
    ...
    protected boolean setParseError(ParseError parseerror)
    {
        ...
        {
            MAPLog.formattedInfo(TAG, "%s: setParseError has been called more than once.  Was %s, Now %s.", new Object[] {
