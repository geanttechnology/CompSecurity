...
*/
    ...
    private currentEditor(String s)
    {
        ...
        cleanFiles = new File[DiskLruCache.access$1800(DiskLruCache.this)];
        dirtyFiles = new File[DiskLruCache.access$1800(DiskLruCache.this)];
        s = (new StringBuilder(s)).append('.');
        int j = s.length();
        for (int i = 0; i < DiskLruCache.access$1800(DiskLruCache.this); i++)
        {
            s.append(i);
            cleanFiles[i] = new File(DiskLruCache.access$1900(DiskLruCache.this), s.toString());
            s.append(".tmp");
            dirtyFiles[i] = new File(DiskLruCache.access$1900(DiskLruCache.this), s.toString());
            s.setLength(j);
        }
