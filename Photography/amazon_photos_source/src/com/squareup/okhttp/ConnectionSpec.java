...
public final class ConnectionSpec
{
    ...
    private static boolean contains(Object aobj[], Object obj)
    {
        boolean flag1 = false;
        int j = aobj.length;
        int i = 0;
...
label0:
    ...
    {
        if (as != null && as1 != null && as.length != 0 && as1.length != 0)
        {
            int j = as.length;
            int i = 0;
            while (i < j) 
            {
                if (contains(as1, as[i]))
                {
                    return true;
                }
                i++;
    ...
    private ConnectionSpec supportedSpec(SSLSocket sslsocket, boolean flag)
    {
        ...
        return (new Builder(this)).cipherSuites(as1).tlsVersions(sslsocket).build();
    }
    ...
    public boolean isCompatible(SSLSocket sslsocket)
    {
        ...
        if (tls)
        {
            if (nonEmptyIntersection(tlsVersions, as = sslsocket.getEnabledProtocols()))
            {
                if (cipherSuites == null)
                {
                ...
                } else
                {
                    ...
                    return nonEmptyIntersection(cipherSuites, sslsocket);
                }
            }
        }
        return false;
