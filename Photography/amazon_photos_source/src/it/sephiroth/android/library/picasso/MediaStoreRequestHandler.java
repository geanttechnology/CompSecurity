...
class MediaStoreRequestHandler extends ContentStreamRequestHandler
{
    static final class PicassoKind extends Enum
    {
        ...
        {
            return (PicassoKind)Enum.valueOf(it/sephiroth/android/library/picasso/MediaStoreRequestHandler$PicassoKind, s);
        }
...
_L4:
        ...
        throws IOException
    {
        ...
        int k = getExifOrientation(((ContentResolver) (obj)), request.uri);
        ...
        if (request.hasSize())
        {
            int l = request.targetWidth;
            PicassoKind picassokind;
            int j;
            if (request.resizeByMaxSide)
            {
                j = request.targetWidth;
            } else
            {
                j = request.targetHeight;
            }
            picassokind = getPicassoKind(l, j);
            if (!flag && picassokind == PicassoKind.FULL)
            {
                return new RequestHandler.Result(decodeContentStream(request), Picasso.LoadedFrom.DISK, k);
            }
            long l1 = ContentUris.parseId(request.uri);
            ...
            if (flag)
            {
                ...
                obj = android.provider.MediaStore.Video.Thumbnails.getThumbnail(((ContentResolver) (obj)), l1, i, options);
            } else
            {
                obj = android.provider.MediaStore.Images.Thumbnails.getThumbnail(((ContentResolver) (obj)), l1, picassokind.androidKind, options);
            }
            if (obj != null)
            {
                return new RequestHandler.Result(((android.graphics.Bitmap) (obj)), Picasso.LoadedFrom.DISK, k);
            }
        }
        return new RequestHandler.Result(decodeContentStream(request), Picasso.LoadedFrom.DISK, k);
    }
