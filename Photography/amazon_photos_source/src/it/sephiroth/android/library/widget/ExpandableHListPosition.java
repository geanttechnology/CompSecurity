...
label0:
    ...
    static ExpandableHListPosition obtainChildPosition(int i, int j)
    {
    ...
    static ExpandableHListPosition obtainPosition(long l)
    {
        if (l == 0xffffffffL)
        {
            return null;
        ...
        ExpandableHListPosition expandablehlistposition = getRecycledOrCreate();
        expandablehlistposition.groupPos = ExpandableListView.getPackedPositionGroup(l);
        if (ExpandableListView.getPackedPositionType(l) == 1)
        {
            expandablehlistposition.type = 1;
            expandablehlistposition.childPos = ExpandableListView.getPackedPositionChild(l);
            return expandablehlistposition;
        } else
        {
            expandablehlistposition.type = 2;
            return expandablehlistposition;
        }
    }
