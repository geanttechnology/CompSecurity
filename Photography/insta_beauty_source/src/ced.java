    ...
    implements android.opengl.GLSurfaceView.Renderer
{
    ...
    public ced(bzk bzk1)
    {
        ...
        p.put(x).position(0);
        q = ByteBuffer.allocateDirect(byq.d.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
        q.put(byq.d).position(0);
        Log.e("ClassNotFound Test", "BaseSurfaceRenderer finish");
    }

    private void g()
    {
        c = cdo.a("attribute highp vec4 position;\nattribute highp vec4 inputTextureCoordinate;\n \nvarying highp vec2 textureCoordinate;\n \nvoid main()\n{\n    gl_Position = position;\n    textureCoordinate = inputTextureCoordinate.xy;\n}", "varying highp vec2 textureCoordinate;\n \nuniform sampler2D inputImageTexture;\n \nvoid main()\n{\n     gl_FragColor = texture2D(inputImageTexture, textureCoordinate);\n}");
        if (c == 0)
        {
        ...
        } else
        {
            ...
            cdo.b(e, "inputTextureCoordinate");
            f = GLES20.glGetUniformLocation(c, "inputImageTexture");
            cdo.b(f, "inputImageTexture");
            return;
        }
    }
    ...
    public void a(bzl bzl1)
    {
        if (bzl1 != null)
        {
            i = new WeakReference(bzl1);
        }
    }

    public void a(bzq bzq1)
    {
        h = bzq1;
        if (bzq1 != null && g != null)
        {
