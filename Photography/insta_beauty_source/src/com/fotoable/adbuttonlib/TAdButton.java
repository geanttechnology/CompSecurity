...
_L6:
    ...
    {
        View view = LayoutInflater.from(getContext()).inflate(aas.adbutton_view, this);
        m_Switcher = (ImageSwitcher)view.findViewById(aar.imageswitcher);
        m_Switcher.setFactory(new ViewFactoryImpl(null));
        m_Switcher.setOnClickListener(new _cls1());
        m_Switcher.setPersistentDrawingCache(1);
        fBNativeClickView = (FrameLayout)view.findViewById(aar.fbnative_clickedview);
        fBNativeClickView.setBackgroundColor(0);
        fBNativeClickView.setVisibility(4);
        fBNativeClickView.setOnClickListener(new _cls2());
    }
...
_L8:
        if (!yp.a(mActivity, taditem.mdefaultSchemeURL) && (!isHaveClicked(taditem.madId) || !taditem.clearWhenClicked))
        {
            adItemlist.add(taditem);
            taditem.loadImage();
        }
