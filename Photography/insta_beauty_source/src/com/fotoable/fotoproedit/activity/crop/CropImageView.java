...
public class CropImageView extends View
{
    ...
    private void init(Context context)
    {
        ...
        mFloatDrawable = new ajf(context);
        mFloatDrawable.a(true);
        mPaint = new Paint();
        mPaint.setColor(Color.parseColor("#a03e3e3e"));
...
label0:
    ...
    {
        int k3 = mFloatDrawable.c();
        int j3 = mFloatDrawable.d();
        int k = mFloatDrawable.getBounds().left - k3;
        int i1 = mFloatDrawable.getBounds().top - j3;
        int k1 = mFloatDrawable.getBounds().right + k3;
        int i2 = mFloatDrawable.getBounds().bottom + j3;
        int k2 = k3 * 2 + k;
        int i3 = j3 * 2 + i1;
        k3 = k1 - k3 * 2;
        j3 = i2 - j3 * 2;
        if (k <= i && i < k2 && i1 <= j && j < i3)
        {
            return 1;
        }
        if (k3 <= i && i < k1 && i1 <= j && j < i3)
        {
            return 2;
        }
        if (k <= i && i < k2 && j3 <= j && j < i2)
        {
            return 3;
        }
        if (k3 <= i && i < k1 && j3 <= j && j < i2)
        {
            return 4;
        }
        if (mFloatDrawable.e())
        {
            int l = (mFloatDrawable.getBounds().left + mFloatDrawable.getBounds().right) / 2;
            int j1 = (mFloatDrawable.getBounds().top + mFloatDrawable.getBounds().bottom) / 2;
            int l1 = mFloatDrawable.a() / 2;
            int j2 = mFloatDrawable.b() / 2;
            int l2 = yp.a(mContext, 25F);
            if ((l - l1) + l2 <= i && (l + l1) - l2 > i && mFloatDrawable.getBounds().top <= j && mFloatDrawable.getBounds().top + l2 > j)
            {
                return 8;
            }
            if ((l - l1) + l2 <= i && (l1 + l) - l2 > i && mFloatDrawable.getBounds().bottom - l2 <= j && j1 * 2 > j)
            {
                return 9;
            }
            if (mFloatDrawable.getBounds().left <= i && mFloatDrawable.getBounds().left + l2 > i && (j1 - j2) + l2 <= j && (j1 + j2) - l2 > j)
            {
                return 10;
            }
            if (mFloatDrawable.getBounds().right - l2 <= i && l * 2 > i && (j1 - j2) + l2 <= j && (j1 + j2) - l2 > j)
            {
                return 11;
            }
        }
