    ...
    implements IMMPluginAPI
{
    ...
    private String aE;
    private com.tencent.mm.sdk.channel.MMessage.Receiver bz;
    ...
    {
        Object obj = null;
        Cursor cursor = R.getContentResolver().query(MMPluginProviderConstants.PluginDB.CONTENT_URI, columns, null, new String[] {
            s
        }, null);
        if (cursor == null)
        {
    ...
    {
        return Util.nullAsFalse((Boolean)c((new StringBuilder("plugindb://")).append(aE).append("/action.appnetstat?recv=").append(i).append("&send=").append(j).append("&type=").append(k).toString()));
    }
    ...
    {
        Object obj = null;
        Cursor cursor = R.getContentResolver().query(Profile.CONTENT_URI, Profile.columns, null, new String[] {
            s
        }, null);
        if (cursor == null)
        {
    ...
    {
        return (String)c((new StringBuilder("plugindb://")).append(s).append("/comm.pluginkey").toString());
    }

    public boolean installPlugin(String s)
    {
        return Util.nullAsFalse((Boolean)c((new StringBuilder("plugindb://")).append(s).append("/action.install").toString()));
    }

    public boolean isPluginInstalled(String s)
    {
        return Util.nullAsFalse((Boolean)c((new StringBuilder("plugindb://")).append(s).append("/comm.installed").toString()));
    }
    ...
    {
        return Util.nullAsFalse((Boolean)c((new StringBuilder("plugindb://")).append(aE).append("/action.automsg?pluginMsgUsername=").append(s).append("&pluginUsername=").append(s1).toString()));
    }
    ...
    public boolean registerQRCodePattern(String s, com.tencent.mm.sdk.channel.MMessage.CallBack callback, String s1)
    {
        if (bz != null)
        {
            R.unregisterReceiver(bz);
        }
        bz = new com.tencent.mm.sdk.channel.MMessage.Receiver(callback);
        R.registerReceiver(bz, new IntentFilter("com.tencent.mm.sdk.plugin.Intent.ACTION_QRCODE_SCANNED"));
        return Util.nullAsFalse((Boolean)c((new StringBuilder("plugindb://")).append(s).append("/action.register_qrcode_result?pattern=").append(s1).toString()));
    }
    ...
    public boolean sendMsgNotify(String s, String s1, int i, String s2, Class class1)
    {
        ...
        {
            return Util.nullAsFalse((Boolean)c((new StringBuilder("plugindb://")).append(aE).append("/action.msgnotify?username=").append(s).append("&msgContent=").append(s1).append("&msgType=").append(i).append("&customNotify=").append(s2).append("&intentUri=").append(Base64.encodeToString((new Intent(R, class1)).toUri(0).getBytes(), false)).toString()));
        }
    }

    public boolean unregisterAutoMsg(String s, String s1)
    {
        return Util.nullAsFalse((Boolean)c((new StringBuilder("plugindb://")).append(aE).append("/action.unautomsg?pluginMsgUsername=").append(s).append("&pluginUsername=").append(s1).toString()));
    }
