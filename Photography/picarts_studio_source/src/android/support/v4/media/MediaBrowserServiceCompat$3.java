...
class sult extends sult
{
    ...
    void onResultSent(List list)
    {
        if (list == null)
        {
            throw new IllegalStateException((new StringBuilder("onLoadChildren sent null list for id ")).append(val$parentId).toString());
        }
        ...
        {
            Log.w("MediaBrowserServiceCompat", (new StringBuilder("Calling onLoadChildren() failed for id=")).append(val$parentId).append(" package=").append(val$connection.pkg).toString());
        }
    }
