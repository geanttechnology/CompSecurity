...
//            IMediaBrowserServiceCompatCallbacks, MediaBrowserServiceCompat
    ...
    implements Runnable
{
    ...
    public void run()
    {
        ...
        Log.i("MediaBrowserServiceCompat", (new StringBuilder("No root for client ")).append(val$pkg).append(" from service ").append(getClass().getName()).toString());
        val$callbacks.onConnectFailed();
_L1:
        return;
        obj;
        Log.w("MediaBrowserServiceCompat", (new StringBuilder("Calling onConnectFailed() failed. Ignoring. pkg=")).append(val$pkg).toString());
        return;
        try
        {
        ...
        {
            Log.w("MediaBrowserServiceCompat", (new StringBuilder("Calling onConnect() failed. Dropping client. pkg=")).append(val$pkg).toString());
            MediaBrowserServiceCompat.access$100(_fld0).remove(obj);
            return;
        }
          goto _L1
