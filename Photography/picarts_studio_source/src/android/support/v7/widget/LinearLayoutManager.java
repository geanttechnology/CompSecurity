...
label1:
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler, RecyclerView.State state, int i, int j)
    {
        ...
        while (i1 < j1) 
        {
            ...
            if (byte0 == -1)
            {
                l = mOrientationHelper.getDecoratedMeasurement(viewholder.itemView) + l;
            } else
            {
                k = mOrientationHelper.getDecoratedMeasurement(viewholder.itemView) + k;
            }
            i1++;
        }
        ...
        if (l > 0)
        {
            ...
            layoutstate.mCurrentPosition = i + l;
    ...
    private void logChildren()
    {
        for (int i = 0; i < getChildCount(); i++)
        {
            ...
            (new StringBuilder("item ")).append(getPosition(view)).append(", coord:").append(mOrientationHelper.getDecoratedStart(view));
        }
...
label1:
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state)
    {
        ...
        int l;
        boolean flag;
        if (state.hasTargetScrollPosition() && state.getTargetScrollPosition() < mAnchorInfo.mPosition)
        {
            flag = true;
        } else
        {
...
_L4:
    ...
    private class LayoutState
    {
        ...
        private View nextFromLimitedList()
        {
            ...
            for (; j < l; j++)
            {
                RecyclerView.ViewHolder viewholder1 = (RecyclerView.ViewHolder)mScrapList.get(j);
                if (!mIsPreLayout && viewholder1.isRemoved())
                {
                    continue;
                }
                int k = (viewholder1.getLayoutPosition() - mCurrentPosition) * mItemDirection;
                if (k < 0 || k >= i)
                {
                    continue;
                }
            ...
            if (viewholder != null)
            {
                mCurrentPosition = viewholder.getLayoutPosition() + mItemDirection;
