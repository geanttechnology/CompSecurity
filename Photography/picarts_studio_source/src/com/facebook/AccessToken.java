    ...
    implements Parcelable
{
    ...
    public static void createFromNativeLinkingIntent(final Intent extras, final String applicationId, final AccessTokenCreationCallback accessTokenCallback)
    {
        ...
        String s = extras.getString("access_token");
        if (s == null || s.isEmpty())
        {
            accessTokenCallback.onError(new FacebookException("No access token found on intent"));
            return;
        }
        String s1 = extras.getString("user_id");
        if (s1 == null || s1.isEmpty())
        {
            Utility.getGraphMeRequestWithCacheAsync(s, new com.facebook.internal.Utility.GraphMeRequestWithCacheCallback() {
