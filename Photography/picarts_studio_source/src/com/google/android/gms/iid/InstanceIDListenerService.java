...
public class InstanceIDListenerService extends Service
{
    ...
    {
        zzaDD = new MessengerCompat(new Handler(Looper.getMainLooper()) {
    ...
    void stop()
    {
        ...
        if (Log.isLoggable("InstanceID", 3))
        {
            (new StringBuilder("Stop ")).append(zzaDH).append(" ").append(zzaDI);
        }
        this;
...
_L2:
        if (Log.isLoggable("InstanceID", 3))
        {
            (new StringBuilder("Service command ")).append(s1).append(" ").append(s).append(" ").append(intent.getExtras());
        }
