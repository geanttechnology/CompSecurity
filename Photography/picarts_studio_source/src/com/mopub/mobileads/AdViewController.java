...
public class AdViewController
{
    ...
    public AdViewController(Context context, MoPubView mopubview)
    {
        ...
        mUrlGenerator = new WebViewAdUrlGenerator(mContext.getApplicationContext(), true);
    ...
    private void setAutorefreshEnabled(boolean flag)
    {
        ...
        if (flag1)
        {
            ...
            MoPubLog.d((new StringBuilder("Refresh ")).append(s).append(" for ad unit (").append(mAdUnitId).append(").").toString());
        }
    ...
    void loadNonJavascript(String s)
    {
        if (s != null)
        {
            ...
            if (mIsLoading)
            {
                if (!TextUtils.isEmpty(mAdUnitId))
                {
                    MoPubLog.i((new StringBuilder("Already loading an ad for ")).append(mAdUnitId).append(", wait to finish.").toString());
                    return;
                }
            } else
            {
    ...
    public void reload()
    {
        MoPubLog.d((new StringBuilder("Reload ad: ")).append(mUrl).toString());
