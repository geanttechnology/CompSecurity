...
public class MoPubStreamAdPlacer
{
    ...
    private boolean tryPlaceAd(int i)
    {
        ...
        if (isNative)
        {
            obj = mAdSource.dequeueAd();
            if (obj == null)
            {
                return false;
            }
            obj = createAdData(i, ((NativeResponse) (obj)));
        }
        if (!isNative && isCleared)
        {
            return false;
        } else
        {
    ...
    {
        int k;
        for (j--; i <= j && i != -1 && i < mItemCount; j = k)
        {
            k = j;
    ...
    public void bindAdView(NativeAdData nativeaddata, View view)
    {
        ...
        Object obj = (WeakReference)mViewMap.get(nativeresponse);
        if (obj != null)
        {
    ...
    public void destroy()
    {
        ...
        mViewMap.clear();
        mNativeResponseMap.clear();
        if (mPendingPlacementData != null)
        {
            mPendingPlacementData.clearAds();
        }
    }
    ...
    {
        mopubclientpositioning = PlacementData.fromAdPositioning(mopubclientpositioning, adsOffset);
        if (mHasReceivedAds)
        {
    ...
    public void loadPositions()
    {
        isNative = false;
        isCleared = false;
        mPositioningSource.loadPositions(null, new PositioningSource.PositioningListener() {
    ...
    public void trackImpression(NativeResponse nativeresponse, View view)
    {
        Object obj = (WeakReference)mViewMap.get(nativeresponse);
        if (obj != null)
        {
...
/*
    static boolean access$202(MoPubStreamAdPlacer mopubstreamadplacer, boolean flag)
    {
        mopubstreamadplacer.mHasReceivedAds = flag;
        return flag;
    }
