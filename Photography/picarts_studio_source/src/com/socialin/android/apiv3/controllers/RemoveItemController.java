...
public class RemoveItemController extends BaseSocialinApiRequestController
{
    ...
    public void doRequest(String s, ParamWithItemId paramwithitemid)
    {
        params = paramwithitemid;
        if (status == 0)
        {
            return;
        } else
        {
            status = 0;
            requestId = SocialinApiV3.getInstance().removePhoto(paramwithitemid.itemId, paramwithitemid.isDesignItem, s, this);
            return;
        }
    }
    ...
    {
        doRequest(s, (ParamWithItemId)requestparams);
    }
