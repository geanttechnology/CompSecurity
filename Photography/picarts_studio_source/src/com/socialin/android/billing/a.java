...
_L5:
        ...
        if (j == 0 || j == 1)
        {
            throw new Base64DecoderException((new StringBuilder("invalid padding byte '=' at byte offset ")).append(k).toString());
        }
        if (j == 3 && l > 2 || j == 4 && l > 1)
        {
            throw new Base64DecoderException((new StringBuilder("padding byte '=' falsely signals end of encoded value at offset ")).append(k).toString());
        }
...
_L8:
        ...
        k = i1;
          goto _L7
_L4:
        throw new Base64DecoderException((new StringBuilder("Bad Base64 input character at ")).append(k).append(": ").append(abyte0[k + 0]).append("(decimal)").toString());
_L2:
        k = i;
        if (j != 0)
        {
            if (j == 1)
            {
                throw new Base64DecoderException((new StringBuilder("single trailing character at offset ")).append(k1 - 1).toString());
            }
            abyte2[j] = 61;
    ...
    static 
    {
        com/socialin/android/billing/a.desiredAssertionStatus();
    }
}
