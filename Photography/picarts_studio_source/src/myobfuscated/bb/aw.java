...
public final class aw extends ScriptC
{
    ...
    {
        this(renderscript, renderscript.getApplicationContext().getResources(), renderscript.getApplicationContext().getResources().getIdentifier("rs_stenciler_1", "raw", renderscript.getApplicationContext().getPackageName()));
    }

    private aw(RenderScript renderscript, Resources resources, int i)
    {
        super(renderscript, resources, i);
        b = Element.U8_4(renderscript);
        a = Element.U8(renderscript);
    }

    public final void a(int i, int j, float f, int k, float f1)
    {
    ...
    {
        if (!allocation.getType().getElement().isCompatible(b))
        {
