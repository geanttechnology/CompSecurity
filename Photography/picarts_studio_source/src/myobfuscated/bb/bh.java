...
public final class bh extends ScriptC
{

    private Element a;
    private Element b;

    public bh(RenderScript renderscript)
    {
        this(renderscript, renderscript.getApplicationContext().getResources(), renderscript.getApplicationContext().getResources().getIdentifier("rs_utils_convert_yuv_to_rgba", "raw", renderscript.getApplicationContext().getPackageName()));
    }

    private bh(RenderScript renderscript, Resources resources, int i)
    {
    ...
    public final void a(Allocation allocation, Allocation allocation1)
    {
        ...
        if (!allocation1.getType().getElement().isCompatible(b))
        {
