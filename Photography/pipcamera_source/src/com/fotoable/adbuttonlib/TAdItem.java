...
_L4:
        es.a(mContext, mdefaultSchemeURL);
        return;
    }
        ...
        } else
        {
            ...
            if (mImageByteArray != null)
            {
                ...
                if (mImageByteArray.length != 0)
                {
                    ...
                    try
                    {
                        obj = ev.a(mImageByteArray, mImageByteArray, true);
                    }
                    ...
                    if (obj != null)
                    {
                        ...
                        {
                            obj = new BitmapDrawable(mContext.getResources(), ev.a(((android.graphics.Bitmap) (obj))));
                        } else
                        {
                            obj = new BitmapDrawable(mContext.getResources(), ev.a(((android.graphics.Bitmap) (obj)), 5F));
                        }
...
_L3:
                Object obj = null;
                android.graphics.Bitmap bitmap = ev.a(mImageByteArray, mImageByteArray, true);
                obj = bitmap;
_L5:
                if (obj != null)
                {
...
_L2:
    ...
    {
        if (openIfExist && ew.a(mContext, mdefaultSchemeURL))
        {
            handOpenApp1();
            return;
        } else
...
_L3:
        ...
        startTime = (new Date()).getTime();
        mDownload.download(mimageURL, new TAdASyncImageRequest.DownloadCallback() {

            final TAdItem this$0;

            public void downLoaded(String s1, byte abyte0[])
            {
                Log.v("TAdItem ImagedownLoaded", "ImagedownLoaded");
                mImageByteArray = abyte0;
                s1 = getExtension(mimageURL);
                s1 = (new StringBuilder()).append(madId).append(".").append(s1).toString();
                if (madId.equalsIgnoreCase(TAdButton.TAG_Native))
                {
                    s1 = getNativeAdImageKey();
                }
                adimageCache.put(s1, abyte0);
                if (mitemCallbackListener != null)
                {
