    ...
    implements ICredentialsCallbacks
{
    ...
    public static ICredentialsCallbacks zzap(IBinder ibinder)
    {
        ...
        android.os.IInterface iinterface = ibinder.queryLocalInterface("com.google.android.gms.auth.api.credentials.internal.ICredentialsCallbacks");
        class zza
            implements ICredentialsCallbacks
        {
...
_L1:
        ...
        if (iinterface != null && (iinterface instanceof ICredentialsCallbacks))
        {
            return (ICredentialsCallbacks)iinterface;
        } else
        {
    ...
    public boolean onTransact(int i, Parcel parcel, Parcel parcel1, int j)
    {
        ...
        case 1: // '\001'
            ...
            if (parcel.readInt() != 0)
            {
                status = (Status)Status.CREATOR.romParcel(parcel);
            } else
            {
                status = null;
            }
            if (parcel.readInt() != 0)
            {
                parcel = (Credential)Credential.CREATOR.romParcel(parcel);
            } else
            {
                parcel = null;
            }
            onCredentialResult(status, parcel);
        ...
        if (parcel.readInt() != 0)
        {
            parcel = (Status)Status.CREATOR.romParcel(parcel);
        } else
        {
            parcel = null;
        }
        onStatusResult(parcel);
        parcel1.writeNoException();
        return true;
    }
