...
public class DependencyPriorityBlockingQueue extends PriorityBlockingQueue
{
    ...
    boolean canProcess(abo abo1)
    {
        return abo1.areDependenciesMet();
    }
...
_L1:
    ...
    abo get(int i, Long long1, TimeUnit timeunit)
    {
        do
        {
            abo abo1 = performOperation(i, long1, timeunit);
            if (abo1 == null || canProcess(abo1))
            {
                return abo1;
            }
            offerBlockedResult(i, abo1);
        } while (true);
    }

    boolean offerBlockedResult(int i, abo abo1)
    {
        lock.lock();
        if (i != 1)
        {
        ...
        super.remove(abo1);
        boolean flag = blockedQueue.offer(abo1);
        lock.unlock();
        return flag;
        abo1;
        lock.unlock();
    ...
    abo performOperation(int i, Long long1, TimeUnit timeunit)
    {
        switch (i)
        {
        default:
        ...
        case 0: // '\0'
            return (abo)super.take();

        case 1: // '\001'
            return (abo)super.peek();

        case 2: // '\002'
            return (abo)super.poll();

        case 3: // '\003'
            return (abo)super.poll(long1.longValue(), timeunit);
        }
    }
    ...
    public abo poll(long l, TimeUnit timeunit)
    {
