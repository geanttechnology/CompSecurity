...
public class MaskScrollImageViewTouch extends ImageViewTouch
{
    ...
    public Bitmap getDispalyImage(int i, int j)
    {
        ...
        matrix = new Matrix(getImageViewMatrix());
        matrix.postScale(f, f, getCenterRectF().left, getCenterRectF().top);
        canvas.drawARGB(0, 0, 0, 0);
        matrix1 = new Matrix();
        mBmpShader.getLocalMatrix(matrix1);
        if (mMask == null) goto _L2; else goto _L1
_L1:
...
_L3:
    ...
    public void renderInCanvas(Canvas canvas)
    {
        ...
        {
            Bitmap bitmap = ((ajb)getDrawable()).a();
            Paint paint = new Paint();
