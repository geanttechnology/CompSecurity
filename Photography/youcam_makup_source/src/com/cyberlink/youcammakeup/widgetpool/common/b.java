...
public class b extends android.widget.FrameLayout.LayoutParams
{
    ...
    public b(Context context, AttributeSet attributeset, int i, int k, boolean flag)
    {
        ...
        if (c != 0)
        {
            i = (int)((float)AutoSizeFrameLayout.c() / ((float)AutoSizeFrameLayout.d() / 160F));
            float f;
            float f1;
            if (!flag1)
            {
            ...
            if (d != 0)
            {
                i = (int)((float)AutoSizeFrameLayout.b() / ((float)AutoSizeFrameLayout.d() / 160F));
                float f2;
                if (!flag1)
                {
                    i = c(i);
    ...
    private int c(int i)
    {
        ...
        for (; k < b.length; k++)
        {
            int i1 = b[k] * (AutoSizeFrameLayout.b() / AutoSizeFrameLayout.c());
            if (i >= i1)
            {
                l = i1;
            }
