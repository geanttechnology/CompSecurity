...
//            Address, ConnectionSpec, CertificatePinner, Handshake, 
...
public final class Connection
{
    ...
    private void makeTunnel(Request request, int i, int j)
    {
        ...
        do
        {
            ...
            case 407: 
                request1 = OkHeaders.processAuthHeader(route.address.authenticator, request, route.proxy);
                request = request1;
                break;
            }
        } while (request1 != null);
    ...
    private void upgradeToTls(Request request, int i, int j)
    {
        ...
        route.address.certificatePinner.check(route.address.uriHost, request.getSession().getPeerCertificates());
        handshake = Handshake.get(request.getSession());
        if (route.connectionSpec.supportsTlsExtensions())
        {
            obj = ((Platform) (obj)).getSelectedProtocol(request);
            if (obj != null)
            {
                protocol = Protocol.get(((String) (obj)));
            }
        }
