...
public class AnimatorManager
{
    ...
    {
        mEffects.add(effectmanager);
        if (mAnimator != null)
        {
            effectmanager.setAnimator(mAnimator);
        }
    }
    ...
    public float getCurrentLinearFraction()
    {
        if (mAnimator != null)
        {
            return (float)(mAnimator.getCurrentPlayTime() - mAnimator.getStartDelay()) / (float)mAnimator.getDuration();
        } else
        {
            return 0.0F;
        }
    ...
    {
        mEffects.remove(effectmanager);
        if (mAnimator != null && effectmanager.getAnimator() == mAnimator)
        {
            effectmanager.detachFromAnimator();
        }
    }
