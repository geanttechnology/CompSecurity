    ...
    implements UploadIntentListener
{
    ...
    public void shutdown()
    {
        ...
        Iterator iterator = mBatchCreatorMap.values().iterator();
        while (iterator.hasNext()) 
        {
            BatchCreator batchcreator = (BatchCreator)iterator.next();
            try
            {
            ...
            {
                log.error("shutdown", "Exception enqueuing batches:", new Object[] {
                    exception.getMessage()
                });
            }
            batchcreator.shutdown();
        }
        for (Iterator iterator1 = mBatchTransmitterList.iterator(); iterator1.hasNext(); ((BatchTransmitter)iterator1.next()).shutdown()) { }
