    ...
    implements MetricsTransport, TransportStateNotifier
{
    ...
    private static final String SERVICE_NAME = "DeviceMetricsService";
    ...
    protected boolean isWanAvailable()
        throws IllegalStateException
    {
        boolean flag1 = mMetricsConfiguration.getNetworkConfiguration().getNetworkTypes().contains(NetworkType.WAN);
        String s = mDeviceUtil.fetchWANSupportedDeviceMode();
        boolean flag = flag1;
        if (s != null)
        {
            if (flag1 || s.equals(mDeviceUtil.fetchDeviceMode()))
            {
                flag = true;
            } else
            {
        ...
        return flag && NetworkManager.instance(mContext).isWanConnected();
    }
