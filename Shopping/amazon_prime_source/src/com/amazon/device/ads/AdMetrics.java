...
//            MobileAdsLoggerFactory, MetricsCollector, JSONUtils, WebUtils, 
//            MobileAdsInfoStore, RegistrationInfo, MobileAdsLogger, Version, 
//            WebRequest

class AdMetrics
{

    public static final String LOGTAG = com/amazon/device/ads/AdMetrics.getSimpleName();
...
_L2:
        ...
        while (i < k) 
        {
            ...
            if (obj1 instanceof MetricsCollector.MetricHitStopTime)
            {
                ...
                obj1 = (Long)((HashMap) (obj)).remove(((MetricsCollector.MetricHit) (obj1)).metric);
                if (obj1 != null)
                {
                    long l = JSONUtils.getLongFromJSON(jsonobject, metricscollector, 0L);
                    JSONUtils.put(jsonobject, metricscollector, (metrichitstoptime.stopTime + l) - ((Long) (obj1)).longValue());
                }
            } else
...
_L3:
    ...
    private String getAaxUrlAndResetAdMetrics()
    {
        ...
        s = (new StringBuilder()).append(submitter.getInstrumentationPixelUrl()).append(s).toString();
    ...
    {
        String s = submitter.getInstrumentationPixelUrl();
        if (s == null || s.equals(""))
        {
            return false;
        }
        s = MobileAdsInfoStore.getInstance().getRegistrationInfo().getAppKey();
        if (s == null || s.equals("123"))
        {
            logger.d("Not submitting metrics because the AppKey is either not set or set to a test key.");
            return false;
        } else
        {
            return true;
    ...
    protected String getAaxJson()
    {
        ...
        addMetricsToJSON(((JSONObject) (obj)), submitter.getMetricsCollector());
