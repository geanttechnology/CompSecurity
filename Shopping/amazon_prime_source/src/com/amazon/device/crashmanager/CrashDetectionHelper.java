...
//            OAuthRequestSigner, ThirdPartyDomainChooser, CrashDetailsAggregator, CrashDetailsCollector, 
//            AppFileArtifactSource, ArtifactUploader, DomainChooser, DetEndpointConfig, 
    ...
    implements Thread.UncaughtExceptionHandler
{
    ...
    CrashDetectionHelper(String s, String s1, OAuthHelper oauthhelper, MetricsFactory metricsfactory, AppFileArtifactSource appfileartifactsource, AmazonPackageLookup amazonpackagelookup, DomainChooser domainchooser, 
            NetworkManager networkmanager, CrashDescriptorDedupeUtil crashdescriptordedupeutil, CrashDetailsAggregator crashdetailsaggregator, boolean flag, boolean flag1)
    {
        ...
        mUploadCrashBeforeRestart = flag1;
        mNetworkManager = networkmanager;
        mCrashDescUtil = crashdescriptordedupeutil;
    ...
    public static CrashDetectionHelper setUpCrashDetection(String s, String s1, OAuthHelper oauthhelper, MetricsFactory metricsfactory, Context context)
    {
        if (context == null)
        {
        ...
        {
            return setUpCrashDetection(s, s1, oauthhelper, metricsfactory, ((DomainChooser) (new ThirdPartyDomainChooser(context.getApplicationInfo()))), context, true);
        }
    }
    ...
    {
        return setUpCrashDetection(s, s1, oauthhelper, metricsfactory, domainchooser, context, flag, false, false);
    }

    public static CrashDetectionHelper setUpCrashDetection(String s, String s1, OAuthHelper oauthhelper, MetricsFactory metricsfactory, DomainChooser domainchooser, Context context, boolean flag, boolean flag1, 
            boolean flag2)
    {
        if (ANDROID_API_VERSION < 9)
        {
    ...
    void uploadCrashReport()
    {
        ...
        if (!mNetworkManager.isWifiConnected() && !mNetworkManager.isEthernetConnected() && !mAllowWANUpload)
        {
        ...
        (new ArtifactUploader(DetEndpointConfig.getServiceEndpoint(mDomainChooser.chooseDomain()), mDeviceType, mDeviceId, linkedlist, linkedlist1, mCrashDescUtil, mStatusNotifier, mRequestSigner, mNetworkManager, mAllowWANUpload, detutil)).uploadArtifacts(metricevent);
    ...
    public void uploadCrashReportAsync()
    {
        try
        {
            SINGLE_THREADED_EXECUTOR.execute(new Runnable() {
                ...
                public void run()
                {
                    ...
                    {
                        Log.e(CrashDetectionHelper.TAG, "Failed to upload crash.", exception1);
                    }
                }
        ...
        {
            Log.e(TAG, "Failed to upload crash due to failure in accepting the task to execute", rejectedexecutionexception);
            return;
        }
        catch (Exception exception)
        {
            Log.e(TAG, "Failed to upload crash", exception);
        }
    }
