...
_L3:
    ...
    public void shutdown()
    {
        try
        {
            if (mFetchSessionIDThreadPool != null)
            {
                mFetchSessionIDThreadPool.shutdownNow();
            }
            if (mFetchCustomerIDThreadPool != null)
            {
                mFetchCustomerIDThreadPool.shutdownNow();
            }
            return;
        }
        catch (Exception exception)
        {
            Log.e("shutdown", "Failed to shutdown device util.", exception);
        }
    }


/*
    static String access$002(AbstractDeviceUtil abstractdeviceutil, String s)
    {
