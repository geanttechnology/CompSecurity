        ...
        implements AccountRegistrar.Listener
    {
        ...
        public void success(final String accountType, final String accountName, final Bundle extras)
        {
            ThreadUtils.submitToBackgroundThread(new Runnable() {
                ...
                public void run()
                {
                    AccountManagerLogic.access$500$70268062(_fld0, regType, regData, callback, accountName, extras);
                }
...
_L1:
    ...
    private void updateAccountCredentials(String s, String s1, Bundle bundle)
    {
        if (bundle != null)
        {
            ...
            String s2 = ((BackwardsCompatiableDataStorage) (obj)).getUserData(s, "com.amazon.dcp.sso.property.devicename");
            String s3 = ((BackwardsCompatiableDataStorage) (obj)).getUserData(s, "com.amazon.dcp.sso.property.deviceemail");
            if (s1 == null)
            {
                AccountTransaction accounttransaction = new AccountTransaction(s, null, null);
                String s4;
                for (Iterator iterator = bundle.keySet().iterator(); iterator.hasNext(); accounttransaction.setUserData(s4, bundle.getString(s4)))
                {
            ...
            } else
            {
                ...
                String s5;
                for (Iterator iterator1 = bundle.keySet().iterator(); iterator1.hasNext(); accounttransaction1.setUserData(StorageKeyUtils.getKeyWithPackageNamespace(s1, s5), bundle.getString(s5)))
                {
            ...
            if (!TextUtils.equals(s2, bundle) && DeviceTypeHelpers.isPackageUsingCentralDeviceType(mContext, s1))
            {
                UserProperties.storeDeviceName(mContext, s, bundle);
            }
            if (!TextUtils.equals(s3, ((CharSequence) (obj))) && DeviceTypeHelpers.arePackagesUsingTheSameDeviceType(mContext, s1, "com.amazon.kindle"))
            {
    ...
    public void authenticateAccountWithUI(Activity activity, SigninOption signinoption, Bundle bundle, Callback callback, Tracer tracer)
    {
        ...
        if (bundle1.getBoolean("set_cookie_for_authenticate_account_with_ui"))
        {
            ...
            bundle1.putStringArray("InjectCookiesToAuthPortalUIActivity", as);
        }
        obj = null;
        static class _cls11
        {
...
_L4:
        ...
        return;
_L2:
        bundle1.putString("requestType", com.amazon.identity.auth.device.endpoint.OpenIdRequest.REQUEST_TYPE.AUTHENTICATE.toString());
...
_L5:
    ...
    public MAPFuture renameDevice(final String directedId, final String newDeviceName, final Bundle options, Callback callback, final Tracer tracer)
    {
        ...
        ACCOUNT_AUTHENTICATOR_QUEUE.addAccountAuthenticatorTask(new AccountAuthenticatorQueue.AccountAuthenticatorTask() {
            ...
            public Bundle run(Callback callback1)
            {
                mRenameDevice.renameDevice$28cc6a5(directedId, newDeviceName, callback1, tracer);
                return null;
            }
    ...
    public MAPFuture updateAccountWithKey$5e2b1462(final String directedId, final KeyInfo key, Tracer tracer)
    {
        ...
        } else
        {
            AccountRegistrar.Listener listener = new AccountRegistrar.Listener() {
                ...
                {
                    updateAccountCredentials(directedId, key.getPackageName(), bundle);
                    s = new Bundle();
            ...
            mRegistrar.updateCredentials$11350a19(listener, directedId, key.getPackageName(), new Bundle(), tracer);
            return callbackFuture;
        }
    }
