...
public class Tracer
{
    ...
    public static Tracer createFromIntent(Intent intent, String s)
    {
        ...
        {
            (new StringBuilder("There is no tracer info in intent, creating tracer using new traceId and defaultApiName, traceId:")).append(i).append(" apiName:").append(s);
            return new Tracer(i, s, j);
        } else
        {
            (new StringBuilder("Creating Tracer from intent, traceId:")).append(i).append(" apiName:").append(s1);
    ...
    {
        (new StringBuilder("Incrementing counter. Name:")).append(s).append(" Count:").append(d);
