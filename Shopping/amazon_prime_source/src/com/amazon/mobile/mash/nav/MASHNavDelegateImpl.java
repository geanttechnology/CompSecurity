    ...
    implements MASHNavDelegate
{
    ...
    protected boolean handleThirdPartyPage(Context context, String s)
    {
        String s1 = Uri.parse(s).getScheme();
        if (!TextUtils.isEmpty(s1) && s1.startsWith("http") && !Util.isUrlFromAmazon(s))
        {
            Intent intent = new Intent(context, com/amazon/mobile/mash/embeddedbrowser/EmbeddedBrowserActivity);
    ...
    public boolean navigate(String s, Context context)
    {
        ...
        if (context != null)
        {
            ...
            } else
            {
                ...
                {
                    return handleAmznappScheme(context, uri);
                }
                if (handleThirdPartyPage(context, s))
                {
                    return true;
                }
                s = uri.getPath();
                if (!Util.isEmpty(s) && s.contains("/ap/signin"))
                {
                    return handleLogin(uri, context);
                }
                if ("modal".equalsIgnoreCase(uri.getQueryParameter("app-nav-type")))
                {
                    return handleModal(uri, context);
                }
                ...
                if (!Util.isEmpty(s))
                {
                    ...
                    try
                    {
                        flag1 = handleAppAction(Action.valueOf(s.toLowerCase(Locale.US).replaceAll("-", "_")), uri, context);
                    }
                    // Misplaced declaration of an exception variable
                    catch (String s)
                    {
                ...
                {
                    return handleDigitalStore(uri, context);
                }
            }
        }
        return false;
