...
public final class Util
{
    ...
    private static final Pattern HOST_PATTERN = Pattern.compile("(^|\\.)amazon\\.(com|cn|co\\.uk|co\\.jp|de|fr|ca|it|es|in|com\\.mx|com\\.br)$");
    ...
    {
        return "https".equalsIgnoreCase(Uri.parse(s).getScheme()) && isUrlFromAmazon(s);
    }

    public static boolean isUrlFromAmazon(String s)
    {
        if (!TextUtils.isEmpty(s))
        {
            ...
            {
                s = s.getHost();
                return HOST_PATTERN.matcher(s).find();
            }
        }
        return false;
    }
