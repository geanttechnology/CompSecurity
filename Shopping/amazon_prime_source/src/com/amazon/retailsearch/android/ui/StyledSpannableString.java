...
public class StyledSpannableString extends SpannableStringBuilder
{
    ...
    public StyledSpannableString(ResultLayoutType resultlayouttype, Context context1)
    {
        retailSearchLogger = RetailSearchLoggingProvider.getInstance().getRetailSearchLogger();
        RetailSearchDaggerGraphController.inject(this);
...
_L3:
    ...
    private void postAppendStyle(int i, int j, String s)
    {
        if (i > 0 && length() >= i)
        {
            ...
            styleattributesreader.readStyle(j, resultLayoutType);
...
_L2:
        ...
        obj = styledtext.getStyle();
        if (!"ASSET_ID_BADGE".equals(obj))
        {
        ...
        if (true) goto _L1; else goto _L3
_L3:
        if ("NEWLINE".equals(obj))
        {
            append(NEW_LINE_SEPARATOR, i);
            return;
        }
        append(s, i);
        obj = RetailSearchResultStyles.getStyledTextStyleId(((String) (obj)));
        if (obj != null)
        {
            postAppendStyle(s.length(), ((Integer) (obj)).intValue(), styledtext.getUrl());
            return;
        }
...
_L4:
    ...
    public void appendImage(Drawable drawable, String s, Integer integer, String s1)
    {
        ...
        {
            StyleAttributesReader styleattributesreader = new StyleAttributesReader(context);
            styleattributesreader.readStyle(integer.intValue(), resultLayoutType);
