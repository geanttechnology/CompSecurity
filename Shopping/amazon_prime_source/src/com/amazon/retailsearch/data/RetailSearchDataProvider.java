...
{
    private class RefinementLocationResultListener
        implements com.amazon.retailsearch.data.location.LocationLoader.LocationResultListener
    {

        private com.amazon.retailsearch.data.location.LocationLoader.LocationResultListener listener;
        final RetailSearchDataProvider this$0;
        ...
        {
            if (RetailSearchDataProvider.this.locations != null)
            {
                listener.locations(RetailSearchDataProvider.this.locations);
            }
        ...
        {
            RetailSearchDataProvider.this.locations = locations1;
            listener.locations(locations1);
        }

        public void onError(Exception exception)
        {
            RetailSearchDataProvider.log.error("Location request ended with an exception", exception);
            listener.onError(exception);
        }

        public RefinementLocationResultListener(com.amazon.retailsearch.data.location.LocationLoader.LocationResultListener locationresultlistener)
        {
            this$0 = RetailSearchDataProvider.this;
            super();
    ...
    private static final String DATASET_SUGGESTIONS = "suggestions";
    public static final String WORD_MATCHING_PATTERN = "(^|(?:\\s))";
    private static final MessageLogger log = AppLog.getLog(com/amazon/retailsearch/data/RetailSearchDataProvider);
...
_L6:
    ...
    public String getLatency(String s)
    {
        Map map = metricsLoader.getLatencyMap();
        if (map != null)
        {
    ...
    public void getLocation(com.amazon.retailsearch.data.location.LocationLoader.LocationResultListener locationresultlistener)
    {
        locationresultlistener = new LocationLoader(new RefinementLocationResultListener(locationresultlistener));
        if (locations == null)
        {
            locationresultlistener.getLocation(null);
            return;
        } else
        {
            locationresultlistener.getLocation("suggestions");
            return;
        }
    }
...
/*
    static Locations access$002(RetailSearchDataProvider retailsearchdataprovider, Locations locations1)
    {
        retailsearchdataprovider.locations = locations1;
        return locations1;
    }
