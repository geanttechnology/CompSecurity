    ...
    implements ContextualSerializer
{
    ...
    public void acceptJsonFormatVisitor(JsonFormatVisitorWrapper jsonformatvisitorwrapper, JavaType javatype)
    {
        ...
        if (flag)
        {
            jsonformatvisitorwrapper.expectNumberFormat(javatype).format(JsonValueFormat.UTC_MILLISEC);
            return;
        } else
        {
            jsonformatvisitorwrapper.expectStringFormat(javatype).format(JsonValueFormat.DATE_TIME);
            return;
        }
    }
            ...
            if (beanproperty != null)
            {
                ...
                } else
                {
                    ...
                    if (s.length() > 0)
                    {
                        ...
                        obj = new SimpleDateFormat(s, beanproperty);
                        beanproperty = timezone;
                        if (timezone == null)
                        {
                            beanproperty = serializerprovider.getTimeZone();
                        }
                        ((SimpleDateFormat) (obj)).setTimeZone(beanproperty);
                        return withFormat(false, ((DateFormat) (obj)));
                    }
                    obj = this;
                    if (timezone != null)
                    {
