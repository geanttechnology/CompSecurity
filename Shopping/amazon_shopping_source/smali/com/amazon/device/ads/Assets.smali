.class Lcom/amazon/device/ads/Assets;
.super Ljava/lang/Object;
.source "Assets.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/amazon/device/ads/Assets$AssetsBase64;,
        Lcom/amazon/device/ads/Assets$CircuitBreaker;
    }
.end annotation


# static fields
.field public static final CLOSE:Ljava/lang/String; = "amazon_ads_close.png"

.field public static final CLOSE_NORMAL:Ljava/lang/String; = "amazon_ads_close_button_normal.png"

.field public static final CLOSE_PRESSED:Ljava/lang/String; = "amazon_ads_close_button_pressed.png"

.field private static INSTANCE:Lcom/amazon/device/ads/Assets; = null

.field public static final INTERSITIAL_CLOSE_NORMAL:Ljava/lang/String; = "amazon_ads_interstitial_close_normal.png"

.field public static final INTERSITIAL_CLOSE_PRESSED:Ljava/lang/String; = "amazon_ads_interstitial_close_pressed.png"

.field public static final LEFT_ARROW:Ljava/lang/String; = "amazon_ads_leftarrow.png"

.field private static final LOG_TAG:Ljava/lang/String; = "Assets"

.field public static final OPEN_EXTERNAL_BROWSER:Ljava/lang/String; = "amazon_ads_open_external_browser.png"

.field public static final REFRESH:Ljava/lang/String; = "amazon_ads_refresh.png"

.field public static final RIGHT_ARROW:Ljava/lang/String; = "amazon_ads_rightarrow.png"

.field private static final VERSION:I = 0x4

.field private static final VERSION_SETTINGS_KEY:Ljava/lang/String; = "mraid-assets-version"


# instance fields
.field private baseFilePath:Ljava/lang/String;


# direct methods
.method static constructor <clinit>()V
    .locals 1

    .prologue
    .line 37
    new-instance v0, Lcom/amazon/device/ads/Assets;

    invoke-direct {v0}, Lcom/amazon/device/ads/Assets;-><init>()V

    sput-object v0, Lcom/amazon/device/ads/Assets;->INSTANCE:Lcom/amazon/device/ads/Assets;

    return-void
.end method

.method protected constructor <init>()V
    .locals 1

    .prologue
    .line 42
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 43
    invoke-virtual {p0}, Lcom/amazon/device/ads/Assets;->getBaseFilePath()Ljava/lang/String;

    move-result-object v0

    iput-object v0, p0, Lcom/amazon/device/ads/Assets;->baseFilePath:Ljava/lang/String;

    .line 44
    return-void
.end method

.method public static final getInstance()Lcom/amazon/device/ads/Assets;
    .locals 1

    .prologue
    .line 52
    sget-object v0, Lcom/amazon/device/ads/Assets;->INSTANCE:Lcom/amazon/device/ads/Assets;

    return-object v0
.end method

.method protected static newInstance()V
    .locals 1

    .prologue
    .line 56
    new-instance v0, Lcom/amazon/device/ads/Assets;

    invoke-direct {v0}, Lcom/amazon/device/ads/Assets;-><init>()V

    sput-object v0, Lcom/amazon/device/ads/Assets;->INSTANCE:Lcom/amazon/device/ads/Assets;

    .line 57
    return-void
.end method

.method protected static setInstance(Lcom/amazon/device/ads/Assets;)V
    .locals 0
    .param p0, "instance"    # Lcom/amazon/device/ads/Assets;

    .prologue
    .line 60
    sput-object p0, Lcom/amazon/device/ads/Assets;->INSTANCE:Lcom/amazon/device/ads/Assets;

    .line 61
    return-void
.end method

.method private writeAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;[BLjava/lang/String;)V
    .locals 7
    .param p1, "breaker"    # Lcom/amazon/device/ads/Assets$CircuitBreaker;
    .param p2, "bytes"    # [B
    .param p3, "filename"    # Ljava/lang/String;

    .prologue
    const/4 v5, 0x1

    const/4 v6, 0x0

    .line 151
    # invokes: Lcom/amazon/device/ads/Assets$CircuitBreaker;->isBroken()Z
    invoke-static {p1}, Lcom/amazon/device/ads/Assets$CircuitBreaker;->access$000(Lcom/amazon/device/ads/Assets$CircuitBreaker;)Z

    move-result v3

    if-eqz v3, :cond_0

    .line 189
    :goto_0
    return-void

    .line 155
    :cond_0
    const/4 v2, 0x0

    .line 158
    .local v2, "outputStream":Ljava/io/OutputStream;
    :try_start_0
    invoke-virtual {p0, p3}, Lcom/amazon/device/ads/Assets;->createOutputStream(Ljava/lang/String;)Ljava/io/OutputStream;
    :try_end_0
    .catch Ljava/io/IOException; {:try_start_0 .. :try_end_0} :catch_1

    move-result-object v2

    .line 168
    new-instance v0, Ljava/io/BufferedOutputStream;

    invoke-direct {v0, v2}, Ljava/io/BufferedOutputStream;-><init>(Ljava/io/OutputStream;)V

    .line 171
    .local v0, "bufferedOutputStream":Ljava/io/BufferedOutputStream;
    :try_start_1
    invoke-virtual {p0, v0, p2}, Lcom/amazon/device/ads/Assets;->writeBytesToOutputStream(Ljava/io/OutputStream;[B)V
    :try_end_1
    .catch Ljava/io/IOException; {:try_start_1 .. :try_end_1} :catch_2
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    .line 183
    :try_start_2
    invoke-virtual {v0}, Ljava/io/BufferedOutputStream;->flush()V

    .line 184
    invoke-virtual {v0}, Ljava/io/BufferedOutputStream;->close()V
    :try_end_2
    .catch Ljava/io/IOException; {:try_start_2 .. :try_end_2} :catch_0

    goto :goto_0

    .line 186
    :catch_0
    move-exception v3

    goto :goto_0

    .line 160
    .end local v0    # "bufferedOutputStream":Ljava/io/BufferedOutputStream;
    :catch_1
    move-exception v1

    .line 163
    .local v1, "e":Ljava/io/IOException;
    const-string/jumbo v3, "Assets"

    const-string/jumbo v4, "Could not write an asset to disk: %s"

    new-array v5, v5, [Ljava/lang/Object;

    aput-object p3, v5, v6

    invoke-static {v3, v4, v5}, Lcom/amazon/device/ads/Log;->e(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    .line 164
    # invokes: Lcom/amazon/device/ads/Assets$CircuitBreaker;->breakCircuit()V
    invoke-static {p1}, Lcom/amazon/device/ads/Assets$CircuitBreaker;->access$100(Lcom/amazon/device/ads/Assets$CircuitBreaker;)V

    goto :goto_0

    .line 173
    .end local v1    # "e":Ljava/io/IOException;
    .restart local v0    # "bufferedOutputStream":Ljava/io/BufferedOutputStream;
    :catch_2
    move-exception v1

    .line 175
    .restart local v1    # "e":Ljava/io/IOException;
    :try_start_3
    const-string/jumbo v3, "Assets"

    const-string/jumbo v4, "Caught IOException while writing asset to disk: %s"

    const/4 v5, 0x1

    new-array v5, v5, [Ljava/lang/Object;

    const/4 v6, 0x0

    aput-object p3, v5, v6

    invoke-static {v3, v4, v5}, Lcom/amazon/device/ads/Log;->w(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    .line 176
    # invokes: Lcom/amazon/device/ads/Assets$CircuitBreaker;->breakCircuit()V
    invoke-static {p1}, Lcom/amazon/device/ads/Assets$CircuitBreaker;->access$100(Lcom/amazon/device/ads/Assets$CircuitBreaker;)V
    :try_end_3
    .catchall {:try_start_3 .. :try_end_3} :catchall_0

    .line 183
    :try_start_4
    invoke-virtual {v0}, Ljava/io/BufferedOutputStream;->flush()V

    .line 184
    invoke-virtual {v0}, Ljava/io/BufferedOutputStream;->close()V
    :try_end_4
    .catch Ljava/io/IOException; {:try_start_4 .. :try_end_4} :catch_3

    goto :goto_0

    .line 186
    :catch_3
    move-exception v3

    goto :goto_0

    .line 181
    .end local v1    # "e":Ljava/io/IOException;
    :catchall_0
    move-exception v3

    .line 183
    :try_start_5
    invoke-virtual {v0}, Ljava/io/BufferedOutputStream;->flush()V

    .line 184
    invoke-virtual {v0}, Ljava/io/BufferedOutputStream;->close()V
    :try_end_5
    .catch Ljava/io/IOException; {:try_start_5 .. :try_end_5} :catch_4

    .line 187
    :goto_1
    throw v3

    .line 186
    :catch_4
    move-exception v4

    goto :goto_1
.end method

.method private writeAssetsToDisk()Z
    .locals 3

    .prologue
    .line 120
    new-instance v0, Lcom/amazon/device/ads/Assets$CircuitBreaker;

    invoke-direct {v0, p0}, Lcom/amazon/device/ads/Assets$CircuitBreaker;-><init>(Lcom/amazon/device/ads/Assets;)V

    .line 121
    .local v0, "breaker":Lcom/amazon/device/ads/Assets$CircuitBreaker;
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_close_button_normal.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 122
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_close_button_pressed.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 123
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_close.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 124
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_leftarrow.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 125
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_refresh.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 126
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_rightarrow.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 127
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_interstitial_close_normal.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 128
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_interstitial_close_pressed.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 129
    const-string/jumbo v1, ""

    const-string/jumbo v2, "amazon_ads_open_external_browser.png"

    invoke-virtual {p0, v2}, Lcom/amazon/device/ads/Assets;->getFilePath(Ljava/lang/String;)Ljava/lang/String;

    move-result-object v2

    invoke-direct {p0, v0, v1, v2}, Lcom/amazon/device/ads/Assets;->writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V

    .line 130
    # invokes: Lcom/amazon/device/ads/Assets$CircuitBreaker;->isBroken()Z
    invoke-static {v0}, Lcom/amazon/device/ads/Assets$CircuitBreaker;->access$000(Lcom/amazon/device/ads/Assets$CircuitBreaker;)Z

    move-result v1

    if-nez v1, :cond_0

    const/4 v1, 0x1

    :goto_0
    return v1

    :cond_0
    const/4 v1, 0x0

    goto :goto_0
.end method

.method private writeBase64EncodedAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;Ljava/lang/String;Ljava/lang/String;)V
    .locals 1
    .param p1, "breaker"    # Lcom/amazon/device/ads/Assets$CircuitBreaker;
    .param p2, "base64String"    # Ljava/lang/String;
    .param p3, "filename"    # Ljava/lang/String;

    .prologue
    .line 140
    invoke-static {p2}, Lcom/amazon/device/ads/Base64;->decode(Ljava/lang/String;)[B

    move-result-object v0

    .line 141
    .local v0, "decodedBytes":[B
    invoke-direct {p0, p1, v0, p3}, Lcom/amazon/device/ads/Assets;->writeAssetToDisk(Lcom/amazon/device/ads/Assets$CircuitBreaker;[BLjava/lang/String;)V

    .line 142
    return-void
.end method


# virtual methods
.method protected createOutputStream(Ljava/lang/String;)Ljava/io/OutputStream;
    .locals 1
    .param p1, "filename"    # Ljava/lang/String;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .prologue
    .line 193
    new-instance v0, Ljava/io/FileOutputStream;

    invoke-direct {v0, p1}, Ljava/io/FileOutputStream;-><init>(Ljava/lang/String;)V

    return-object v0
.end method

.method public declared-synchronized ensureAssetsCreated()Z
    .locals 12

    .prologue
    const/4 v6, 0x1

    const/4 v9, 0x4

    const/4 v5, 0x0

    .line 82
    monitor-enter p0

    :try_start_0
    iget-object v7, p0, Lcom/amazon/device/ads/Assets;->baseFilePath:Ljava/lang/String;

    if-nez v7, :cond_0

    .line 86
    invoke-virtual {p0}, Lcom/amazon/device/ads/Assets;->getBaseFilePath()Ljava/lang/String;

    move-result-object v7

    iput-object v7, p0, Lcom/amazon/device/ads/Assets;->baseFilePath:Ljava/lang/String;

    .line 88
    iget-object v7, p0, Lcom/amazon/device/ads/Assets;->baseFilePath:Ljava/lang/String;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    if-nez v7, :cond_0

    .line 115
    :goto_0
    monitor-exit p0

    return v5

    .line 94
    :cond_0
    :try_start_1
    invoke-static {}, Ljava/lang/System;->nanoTime()J

    move-result-wide v2

    .line 95
    .local v2, "time":J
    invoke-static {}, Lcom/amazon/device/ads/Settings;->getInstance()Lcom/amazon/device/ads/Settings;

    move-result-object v1

    .line 96
    .local v1, "settings":Lcom/amazon/device/ads/Settings;
    const-string/jumbo v7, "mraid-assets-version"

    const/4 v8, 0x0

    invoke-virtual {v1, v7, v8}, Lcom/amazon/device/ads/Settings;->getInt(Ljava/lang/String;I)I

    move-result v4

    .line 97
    .local v4, "writtenVersion":I
    const/4 v0, 0x0

    .line 98
    .local v0, "assetsCreated":Z
    if-ge v4, v9, :cond_1

    .line 100
    const-string/jumbo v7, "Assets"

    const-string/jumbo v8, "Writing assets to disk: version %d"

    const/4 v9, 0x1

    new-array v9, v9, [Ljava/lang/Object;

    const/4 v10, 0x0

    const/4 v11, 0x4

    invoke-static {v11}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v11

    aput-object v11, v9, v10

    invoke-static {v7, v8, v9}, Lcom/amazon/device/ads/Log;->d(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/Object;)V

    .line 101
    invoke-direct {p0}, Lcom/amazon/device/ads/Assets;->writeAssetsToDisk()Z

    move-result v7

    if-eqz v7, :cond_2

    .line 102
    const-string/jumbo v5, "mraid-assets-version"

    const/4 v7, 0x4

    invoke-virtual {v1, v5, v7}, Lcom/amazon/device/ads/Settings;->putInt(Ljava/lang/String;I)V

    .line 103
    const/4 v0, 0x1

    .line 111
    :cond_1
    invoke-static {}, Ljava/lang/System;->nanoTime()J

    move-result-wide v7

    sub-long/2addr v7, v2

    invoke-static {v7, v8}, Lcom/amazon/device/ads/Utils;->convertToMillisecondsFromNanoseconds(J)J

    move-result-wide v2

    .line 112
    invoke-static {}, Lcom/amazon/device/ads/Metrics;->getInstance()Lcom/amazon/device/ads/Metrics;

    move-result-object v5

    invoke-virtual {v5}, Lcom/amazon/device/ads/Metrics;->getMetricsCollector()Lcom/amazon/device/ads/MetricsCollector;

    move-result-object v7

    if-eqz v0, :cond_3

    sget-object v5, Lcom/amazon/device/ads/Metrics$MetricType;->ASSETS_CREATED_LATENCY:Lcom/amazon/device/ads/Metrics$MetricType;

    :goto_1
    invoke-virtual {v7, v5, v2, v3}, Lcom/amazon/device/ads/MetricsCollector;->publishMetricInMilliseconds(Lcom/amazon/device/ads/Metrics$MetricType;J)V

    move v5, v6

    .line 115
    goto :goto_0

    .line 106
    :cond_2
    invoke-static {}, Lcom/amazon/device/ads/Metrics;->getInstance()Lcom/amazon/device/ads/Metrics;

    move-result-object v6

    invoke-virtual {v6}, Lcom/amazon/device/ads/Metrics;->getMetricsCollector()Lcom/amazon/device/ads/MetricsCollector;

    move-result-object v6

    sget-object v7, Lcom/amazon/device/ads/Metrics$MetricType;->ASSETS_FAILED:Lcom/amazon/device/ads/Metrics$MetricType;

    invoke-virtual {v6, v7}, Lcom/amazon/device/ads/MetricsCollector;->incrementMetric(Lcom/amazon/device/ads/Metrics$MetricType;)V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    goto :goto_0

    .line 82
    .end local v0    # "assetsCreated":Z
    .end local v1    # "settings":Lcom/amazon/device/ads/Settings;
    .end local v2    # "time":J
    .end local v4    # "writtenVersion":I
    :catchall_0
    move-exception v5

    monitor-exit p0

    throw v5

    .line 112
    .restart local v0    # "assetsCreated":Z
    .restart local v1    # "settings":Lcom/amazon/device/ads/Settings;
    .restart local v2    # "time":J
    .restart local v4    # "writtenVersion":I
    :cond_3
    :try_start_2
    sget-object v5, Lcom/amazon/device/ads/Metrics$MetricType;->ASSETS_ENSURED_LATENCY:Lcom/amazon/device/ads/Metrics$MetricType;
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_0

    goto :goto_1
.end method

.method protected getBaseFilePath()Ljava/lang/String;
    .locals 2

    .prologue
    .line 69
    invoke-static {}, Lcom/amazon/device/ads/InternalAdRegistration;->getInstance()Lcom/amazon/device/ads/IInternalAdRegistration;

    move-result-object v0

    if-eqz v0, :cond_0

    invoke-static {}, Lcom/amazon/device/ads/InternalAdRegistration;->getInstance()Lcom/amazon/device/ads/IInternalAdRegistration;

    move-result-object v0

    invoke-interface {v0}, Lcom/amazon/device/ads/IInternalAdRegistration;->getFilesDir()Ljava/io/File;

    move-result-object v0

    if-eqz v0, :cond_0

    .line 71
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    invoke-static {}, Lcom/amazon/device/ads/InternalAdRegistration;->getInstance()Lcom/amazon/device/ads/IInternalAdRegistration;

    move-result-object v1

    invoke-interface {v1}, Lcom/amazon/device/ads/IInternalAdRegistration;->getFilesDir()Ljava/io/File;

    move-result-object v1

    invoke-virtual {v1}, Ljava/io/File;->getAbsolutePath()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    sget-object v1, Ljava/io/File;->separator:Ljava/lang/String;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    .line 73
    :goto_0
    return-object v0

    :cond_0
    const/4 v0, 0x0

    goto :goto_0
.end method

.method public getFilePath(Ljava/lang/String;)Ljava/lang/String;
    .locals 2
    .param p1, "file"    # Ljava/lang/String;

    .prologue
    .line 65
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    iget-object v1, p0, Lcom/amazon/device/ads/Assets;->baseFilePath:Ljava/lang/String;

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method protected writeBytesToOutputStream(Ljava/io/OutputStream;[B)V
    .locals 0
    .param p1, "outputStream"    # Ljava/io/OutputStream;
    .param p2, "bytes"    # [B
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/io/IOException;
        }
    .end annotation

    .prologue
    .line 198
    invoke-virtual {p1, p2}, Ljava/io/OutputStream;->write([B)V

    .line 199
    return-void
.end method
