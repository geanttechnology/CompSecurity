    ...
    implements android.os.lablePolicy._cls1
{
    ...
    public ParcelablePolicy createFromParcel(Parcel parcel)
    {
        ...
        if (i == 2)
        {
            ...
            return new ParcelablePolicy((new amazon.communication.connection.t>()).ompressionOption(compressionoption1).riority(priority1).sLowLatencyNecessary(flag1).sRequestResponseOnly(flag3).sClearText(flag5).sWiFiNecessary(flag7).sAnonymousCredentialsAllowed(flag9).sDedicated(flag11).urpose(new Purpose(s)).econnectOnFailure(flag12).eepAlive(parcel).d());
        } else
        {
