    ...
    private static class AuthenticateAccountAmazonWebserviceCallListener extends DefaultAmazonWebserviceCallListener
    {
        ...
        public void onResponseComplete(Object obj)
        {
            ...
            if (((PandaAuthenticateAccountResponse) (obj)).getError() != null)
            {
                ...
                switch (_cls1..SwitchMap.com.amazon.identity.kcpsdk.auth.AuthenticateAccountError[authenticateaccounterror.ordinal()])
                {
                default:
                    MAPLog.e(AccountAuthenticator.TAG, "Authentication Error: Unrecognized response.");
                    mCallback.onError(bundleWithErrorCode(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.UNRECOGNIZED, authenticateaccounterror.getErrorString()));
                    return;

                case 1: // '\001'
                    MAPLog.e(AccountAuthenticator.TAG, "Authentication Error: Crentials Invalid");
                    mCallback.onError(bundleWithErrorCode(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.CUSTOMER_NOT_FOUND, authenticateaccounterror.getErrorString()));
                    return;

                case 2: // '\002'
                    MAPLog.e(AccountAuthenticator.TAG, "Authentication Error: Invalid Value");
                    mCallback.onError(bundleWithErrorCode(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.BAD_REQUEST, authenticateaccounterror.getErrorString()));
                    return;
                ...
                Bundle bundle1 = bundleWithErrorCode(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.AUTHENTICATION_CHALLENGED, authenticateaccounterror.getErrorString());
...
_L1:
    ...
    public void authenticateAccount(Bundle bundle, Callback callback, Tracer tracer)
    {
        ...
        pandaauthenticateaccountrequest.setJwtSigner(AmazonJwtSigner.getSigner(PlatformSettings.getInstance(mContext), mDeviceInfo, tracer));
