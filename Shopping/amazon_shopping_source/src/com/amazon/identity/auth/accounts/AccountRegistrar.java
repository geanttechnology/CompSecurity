...
public class AccountRegistrar
{
    ...
    private class _cls3 extends RegisterDeviceAmazonWebserviceCallListener
    {
...
_L1:
                ...
                MAPLog.e(AccountRegistrar.TAG, (new StringBuilder("Error string: ")).append(((String) (obj2))).toString());
                static class _cls6
                {
                ...
                _cls6..SwitchMap.com.amazon.identity.kcpsdk.auth.RegisterDeviceErrorType[((RegisterDeviceError) (obj)).getType().ordinal()];
...
_L21:
                ...
                return;
_L4:
                MAPLog.e(AccountRegistrar.TAG, "Registration Error: Customer not found. The credential is invalid.");
                mListener.failure(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.CUSTOMER_NOT_FOUND, null);
...
_L7:
                ...
                } else
                {
                    ...
                    mListener.failure(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.AUTHENTICATION_CHALLENGED, ((Bundle) (obj2)));
                }
                continue; /* Loop/switch isn't completed */
_L8:
...
_L16:
                ...
                continue; /* Loop/switch isn't completed */
_L17:
                MAPLog.e(AccountRegistrar.TAG, (new StringBuilder("Registration Error: ")).append(((String) (obj2))).toString());
                mListener.failure(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.AUTHENTICATION_FAILED, null);
                continue; /* Loop/switch isn't completed */
_L18:
                MAPLog.e(AccountRegistrar.TAG, (new StringBuilder("Registration Error: ")).append(((String) (obj2))).toString());
                ...
                continue; /* Loop/switch isn't completed */
_L19:
                MAPLog.i(AccountRegistrar.TAG, (new StringBuilder("Registration Error: ")).append(((String) (obj2))).toString());
...
_L2:
        ...
        {
            AccountRegistrar.access$300(AccountRegistrar.this, registerdeviceresponse, accountRegAuth);
        ...
        _cls3(AccountRegistrarAuthenticator accountregistrarauthenticator)
        {
    ...
    public AccountRegistrar(Context context, FeatureSet featureset)
    {
        if (context == null)
        {
            throw new IllegalArgumentException("context is null");
        ...
        if (featureset == null)
        {
            throw new IllegalArgumentException("featureSet is null");
        } else
        {
            mContext = ServiceWrappingContext.create(context);
            mAmazonAccountManager = new AmazonAccountManager(mContext);
    ...
    AccountRegistrar(Context context, SSODeviceInfo ssodeviceinfo, TokenManagement tokenmanagement, MAPAccountManager mapaccountmanager, AmazonWebServiceCallerCreator amazonwebservicecallercreator, AuthenticateAccountAction authenticateaccountaction, AccountAuthenticator accountauthenticator, 
            OAuthTokenManager oauthtokenmanager, DelegatedAccountHelper delegatedaccounthelper, AmazonAccountManager amazonaccountmanager, FeatureSet featureset, DataStorage datastorage)
    {
        mContext = context;
        mDeviceInfo = ssodeviceinfo;
        mTokenManagement = tokenmanagement;
        mMAPAccountManager = mapaccountmanager;
        mAmazonWebServiceCallerCreator = amazonwebservicecallercreator;
        mAuthenticateAccountAction = authenticateaccountaction;
        mAccountAuthenticator = accountauthenticator;
            ...
            Tracer tracer)
    {
        ...
        String s2 = AmazonDomainHelper.getPartialAmazonDomainFromAPIBundle(bundle);
        registerdevicerequest.setPandaCookieDomain(AmazonDomainHelper.getDomainFromCompleteUrl(s2));
        registerdevicerequest.setNonAuthCookies((new MAPCookieManager(mContext)).getNonAuthCookies(AmazonDomainHelper.getCompleteCookieDomain(s2)));
        if (flag || mFeatureSet.hasFeature(Feature.PandaRegistration) && !Boolean.valueOf(flag1).booleanValue())
        {
            registerdevicerequest.setPandaEndPointDomain(s2);
            ...
            {
                String s3 = mAmazonAccountManager.getDevicePrimaryAmazonAccount();
                Object obj = KeyInfo.parseKey("com.amazon.dcp.sso.token.oauth.amazon.access_token");
                try
                {
                    s3 = mOAuthTokenManager.getAccessToken(s3, ((KeyInfo) (obj)), null, tracer);
                }
                ...
                registerdevicerequest.setPrimaryAccessToken(s3);
                s3 = bundle.getString("authAccount");
                obj = bundle.getString("password");
                if (!TextUtils.isEmpty(s3) && !TextUtils.isEmpty(((CharSequence) (obj))))
                {
                    registerdevicerequest.setSecondaryLogin(s3);
                    registerdevicerequest.setSecondaryPassword(((String) (obj)));
                }
            }
        }
        s3 = mDeviceInfo.getDeviceType();
        registerdevicerequest.setDeviceType(s3);
        obj = getDeviceSerialNumber();
        registerdevicerequest.setDeviceSerialNumber(((String) (obj)));
        registerdevicerequest.setDeviceSoftwareVersion(mDeviceInfo.getSoftwareVersion());
        registerdevicerequest.setSoftwareComponentId(getSoftwareComponentId(s3));
        bundle = bundle.getString("calling_package");
        if (bundle != null)
        {
            registerdevicerequest.setMetadataAppName(bundle);
            bundle = PackageUtils.getPackageVersion(mContext, bundle);
            if (bundle != null)
            {
                registerdevicerequest.setMetadataAppVersion(Integer.toString(bundle.intValue()));
            }
        }
        registerdevicerequest.setSecret(getDeviceSecret());
        registerdevicerequest.setLocale(InternationalizationUtil.fromAmazonDomainToLocale(s1));
        registerdevicerequest.setDeviceTypeToSoftwareVersionMapping(DeviceTypeHelpers.getAllMapDeviceTypesAndSoftwareVersion(mContext, false));
        s1 = tryRetrieveSelfGeneratedKeyPair(mContext);
        bundle = null;
        if (s1 == null)
        {
        ...
        } else
        {
            ...
            registerdevicerequest.setPublicKeyData(s1.getEncodedPublicKey());
            registerdevicerequest.setPublicKeyFormat(s1.getFormat());
            registerdevicerequest.setPublicKeyAlgorithm(s1.getAlgo());
            bundle = s1.getEncodedPrivateKey();
        }
        bundle = getRegisterListener(listener, accountregistrarauthenticator, s, s3, ((String) (obj)), bundle);
        registerdevicerequest.setTrustZone(mFeatureSet.hasFeature(Feature.TrustZone));
        s = registerdevicerequest.getWebRequest();
        if (s == null)
        {
        ...
        {
            invokeWebRequest(s, new RegisterDeviceResponseJsonParser(), bundle, tracer);
            return;
        } else
        {
            invokeRegisterWebRequest(registerdevicerequest, bundle, tracer);
            return;
        }
    }
    ...
    private void doUpdateCredentials(String s, String s1, String s2, final Listener final_listener, Tracer tracer)
    {
        ...
        ((UpdateDeviceCredentialsRequest) (obj)).setDeviceTypeToSoftwareVersionMapping(DeviceTypeHelpers.getAllMapDeviceTypesAndSoftwareVersion(mContext, false));
    ...
    private IAmazonWebserviceCallListener getRegisterListener(final Listener final_listener1, AccountRegistrarAuthenticator accountregistrarauthenticator, String s, String s1, String s2, String s3)
    {
        return new RegisterDeviceAmazonWebserviceCallListener(s2, s1) {
            ...
            protected RegisterInfo onRegisterResponseComplete(RegisterDeviceResponse registerdeviceresponse)
            {
                ...
                String s4 = registerdeviceresponse.getUserName();
                Object obj = s4;
                ...
                accountdatacollector.setDirectedId(registerdeviceresponse.getDirectedId());
                obj = getCookieJar(registerdeviceresponse);
    ...
    {
        s = DeviceTypeHelpers.getSoftwareComponentId(mContext, s, null);
    ...
    {
        return bundle.containsKey("com.amazon.dcp.sso.AddAccount.options.AddAsSecondary") && mAmazonAccountManager.hasDevicePrimaryAmazonAccount();
    }
            ...
            listener = new DefaultAmazonWebserviceCallListener() {
                ...
                public void onResponseComplete(Object obj)
                {
                    ...
                    if (((DeregisterDeviceResponse) (obj)).getError() != null)
                    {
                        ...
                        switch (_cls6..SwitchMap.com.amazon.identity.kcpsdk.auth.DeregisterDeviceErrorType[((DeregisterDeviceError) (obj)).getType().ordinal()])
                        {
                        default:
    ...
    public void deregister(Listener listener, String s, String s1, boolean flag, AccountRegistrarAuthenticator accountregistrarauthenticator, Tracer tracer)
    {
        deregister(listener, null, s1, null, flag, accountregistrarauthenticator, tracer);
    }

    public void deregister(Listener listener, String s, boolean flag, AccountRegistrarAuthenticator accountregistrarauthenticator, Tracer tracer)
    {
        deregister(listener, null, s, flag, accountregistrarauthenticator, tracer);
    }
    ...
    {
        return new CookieJar(registerdeviceresponse.getCookiesRetrievedFromFIRS());
    }
    ...
    protected void invokeRegisterWebRequest(RegisterRequest registerrequest, IAmazonWebserviceCallListener iamazonwebservicecalllistener, Tracer tracer)
    {
        ...
        try
        {
            obj = (RegisterDeviceResponse)iamazonwebservicecaller.call(registerrequest.getWebRequest(), new RegisterDeviceResponseParser(), iamazonwebservicecalllistener);
        }
        // Misplaced declaration of an exception variable
        ...
        {
            iamazonwebservicecalllistener.onParseError(registerrequest.getError());
            return;
        }
        // Misplaced declaration of an exception variable
        catch (RegisterRequest registerrequest)
        ...
        MetricsHelper.increasePeriodicCounter("ClockSkewHappened", new String[0]);
        registerrequest.setTimestamp(s);
        tracer = (RegisterDeviceResponse)iamazonwebservicecaller.call(registerrequest.getWebRequest(), new RegisterDeviceResponseParser(), iamazonwebservicecalllistener);
...
_L1:
        if (!bundle.containsKey("com.amazon.dcp.sso.AddAccount.options.AddAsSecondary"))
        {
            flag = false;
        } else
...
_L18:
        return;
_L2:
        _cls6..SwitchMap.com.amazon.identity.auth.device.api.RegistrationType[registrationtype.ordinal()];
        JVM INSTR tableswitch 1 8: default 240
    //                   1 260
    //                   2 509
    //                   3 808
    //                   4 855
...
_L5:
        ...
        obj = bundle.getString("password");
        if (registrationtype == null || obj == null)
        {
            MAPLog.e(TAG, "Must provide an Amazon login and password to register with it");
            listener.failure(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.BAD_REQUEST, null);
            return;
        }
        s5 = AmazonDomainHelper.getPartialAmazonDomainFromAPIBundle(bundle);
        if (listener == null)
        {
            throw new IllegalArgumentException("Listener is null");
        }
        if (TextUtils.isEmpty(registrationtype) || TextUtils.isEmpty(((CharSequence) (obj))))
        {
            listener.failure(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.CUSTOMER_NOT_FOUND, null);
            return;
        }
        if (isSecondaryAccountRegistration(bundle)) goto _L14; else goto _L13
_L13:
        if (!mFeatureSet.hasFeature(Feature.SplitRegistration)) goto _L16; else goto _L15
_L15:
        registrationtype = authenticateAccount(registrationtype, ((String) (obj)), s5, listener, bundle, tracer);
        if (registrationtype == null) goto _L18; else goto _L17
_L17:
...
_L16:
        ...
        registerdevicerequest.setPassword(((String) (obj)));
        commonRegisterDeviceRequest(registerdevicerequest, bundle, registrationtype, s5, accountregistrarauthenticator, listener, tracer);
        return;
_L14:
        obj = authenticateAccount(registrationtype, ((String) (obj)), s5, listener, bundle, tracer);
        if (obj == null) goto _L18; else goto _L19
_L19:
        registrationtype = ((RegistrationType) (obj));
        if (!mAmazonAccountManager.doesAccountExistAfterDeregisteringStateCleanup(((AuthenticateAccountAction.AuthenticatedAccountInfo) (obj)).directedId)) goto _L17; else goto _L20
_L20:
        MAPLog.e(TAG, "Secondary account already exists on the device");
        listener.accountAlreadyExists(((AuthenticateAccountAction.AuthenticatedAccountInfo) (obj)).directedId);
        return;
_L6:
        if (listener == null)
        {
        ...
        String s6 = mDeviceInfo.getDeviceType();
        ...
        bundle.setRadioId(getRadioId());
        bundle.setDeviceType(s6);
        bundle.setDeviceSerialNumber(s1);
        bundle.setVersionNumber(mDeviceInfo.getSoftwareVersion());
        bundle.setSoftwareComponentId(getSoftwareComponentId(s6));
        bundle.setReason("NoState");
        bundle.setLocale(LocaleUtil.getLocaleAsLanguageTag());
        bundle.setDeviceTypeToSoftwareVersionMapping(DeviceTypeHelpers.getAllMapDeviceTypesAndSoftwareVersion(mContext, false));
        ...
        registrationtype = getRegisterListener(listener, accountregistrarauthenticator, null, s6, s1, registrationtype);
        bundle.setTrustZone(mFeatureSet.hasFeature(Feature.TrustZone));
...
_L8:
        ...
        String s2 = bundle.getString("com.amazon.dcp.sso.AddAccount.options.AuthTokenClientContext");
        String s7 = AmazonDomainHelper.getPartialAmazonDomainFromAPIBundle(bundle);
        if (TextUtils.isEmpty(registrationtype) || TextUtils.isEmpty(s2) || TextUtils.isEmpty(s7))
        {
...
_L11:
        ...
        Object obj1 = bundle.getString("com.amazon.dcp.sso.property.account.delegateeaccount");
        String s3 = bundle.getString("com.amazon.dcp.sso.property.account.acctId");
        if (TextUtils.isEmpty(((CharSequence) (obj1))) || TextUtils.isEmpty(s3))
        {
        ...
        if (mAmazonAccountManager.doesAccountExistAfterDeregisteringStateCleanup(s3))
        {
            MAPLog.w(TAG, "The delegated account already exists on the device");
            listener.accountAlreadyExists(s3);
            return;
        }
        ...
        String s9 = mDelegatedAccountHelper.getDelegationDomainFromAPIBundle(bundle);
        registrationtype = null;
        if (TextUtils.isEmpty(s9) || "www.amazon.com".equals(s9))
        {
            registrationtype = "ATVPDKIKX0DER";
        } else
        if ("www.amazon.co.uk".equals(s9))
        {
            registrationtype = "A1F83G8C2ARO7P";
        } else
        {
            String s10 = TAG;
            (new StringBuilder("Can't use combined delegated registration for domain: ")).append(s9).toString();
        }
        if (registrationtype != null && mFeatureSet.hasFeature(Feature.CombinedDelegatedAuthenticationAndRegistration))
        {
            ...
            {
                obj1 = new RegisterDeviceRequest();
                ((RegisterDeviceRequest) (obj1)).setSecondaryDirectedId(s3);
                ((RegisterDeviceRequest) (obj1)).setAddAsChildAccount(true);
                ((RegisterDeviceRequest) (obj1)).setPfmOfDelegatee(registrationtype);
                ((RegisterDeviceRequest) (obj1)).setAddAsSecondaryAccount(isSecondaryAccountRegistration(bundle));
                commonRegisterDeviceRequest(((RegisterDeviceRequest) (obj1)), bundle, null, null, accountregistrarauthenticator, listener, tracer);
                return;
            }
        }
        registrationtype = getDelegatedAccessTokenOrCallBackError(((String) (obj1)), s3, s9, listener, tracer);
...
_L12:
        ...
        String s4 = bundle.getString("password");
        if (registrationtype == null || s4 == null)
        {
            MAPLog.e(TAG, "Must provide an Amazon directedId and password to register with it");
            listener.failure(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.BAD_REQUEST, null);
            return;
        }
        String s8 = AmazonDomainHelper.getPartialAmazonDomainFromAPIBundle(bundle);
        if (listener == null)
        {
            throw new IllegalArgumentException("Listener is null");
        }
        if (TextUtils.isEmpty(registrationtype) || TextUtils.isEmpty(s4))
        {
            listener.failure(com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.CUSTOMER_NOT_FOUND, null);
            return;
        }
        ...
        {
            registrationtype = authenticateAccountWithDirectedId(registrationtype, s4, s8, listener, bundle, tracer);
            if (registrationtype == null)
            {
        ...
        } else
        {
            ...
            registerdevicerequest1.setSecondaryPassword(s4);
            registerdevicerequest1.setAddAsSecondaryAccount(true);
            commonRegisterDeviceRequest(registerdevicerequest1, bundle, null, s8, accountregistrarauthenticator, listener, true, tracer);
            return;
        }
    }
    ...
    public void registerWithExplicitUrl(String s, AccountRegistrarAuthenticator accountregistrarauthenticator, Listener listener, Tracer tracer)
    {
        ...
        updatedevicecredentialsrequest.setDeviceTypeToSoftwareVersionMapping(DeviceTypeHelpers.getAllMapDeviceTypesAndSoftwareVersion(mContext, false));
    ...
    public void updateCredentials(Listener listener, String s, Bundle bundle, AccountRegistrarAuthenticator accountregistrarauthenticator, Tracer tracer)
    {
        if (listener == null)
        {
    ...
    public void updateCredentials(Listener listener, String s, String s1, Bundle bundle, AccountRegistrarAuthenticator accountregistrarauthenticator, Tracer tracer)
    {
        if (listener == null)
        {
            throw new IllegalArgumentException("No response set. Could not update credentials");
        ...
        if (s1 == null)
        {
            updateCredentials(listener, s, bundle, accountregistrarauthenticator, tracer);
            return;
        }
        if (bundle != null)
        {
...
/*
    static void access$200(AccountRegistrar accountregistrar, AccountRegistrarAuthenticator accountregistrarauthenticator, final Listener final_listener, Tracer tracer)
    {
        ...
        registerdevicewithsecretrequest.setRadioId(accountregistrar.getRadioId());
        ...
        registerdevicewithsecretrequest.setLocale(LocaleUtil.getLocaleAsLanguageTag());
        accountregistrarauthenticator = accountregistrar. new _cls3(accountregistrarauthenticator);
        registerdevicewithsecretrequest.setTrustZone(accountregistrar.mFeatureSet.hasFeature(Feature.TrustZone));
...
/*
    static void access$300(AccountRegistrar accountregistrar, RegisterDeviceResponse registerdeviceresponse, AccountRegistrarAuthenticator accountregistrarauthenticator)
    {
        if (registerdeviceresponse.getDeviceInfo() != null)
        {
            for (registerdeviceresponse = registerdeviceresponse.getDeviceInfo().entrySet().iterator(); registerdeviceresponse.hasNext(); accountregistrar.mDataStorage.setDeviceData("device.metadata", (String)accountregistrarauthenticator.getKey(), (String)accountregistrarauthenticator.getValue()))
            {
                accountregistrarauthenticator = (java.util.Map.Entry)registerdeviceresponse.next();
                MAPLog.i(TAG, (new StringBuilder("device attribute received: ")).append((String)accountregistrarauthenticator.getKey()).append(" value: ").append((String)accountregistrarauthenticator.getValue()).toString());
            }
