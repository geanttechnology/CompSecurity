...
_L1:
        ...
        throws NotChildApplicationException
    {
        ...
        Bundle bundle1 = BundleUtils.constructEmptyBundleIfNull(bundle);
        String s2 = bundle1.getString("override_dsn");
        ...
        DMSSubAuthenticator.constructDMSSubAuthForDeviceType(mContext, s1, s2, integer, s3).registerChildApplication(callback, s, bundle1, tracer);
    }

    public void updateChildApplicationCredentials(String s, String s1, Bundle bundle, Callback callback, Tracer tracer)
    {
        callback = mPendingUpdateRequestCallbackHolder.waitForInflightRequestOrMakeRequest(s1, callback);
        if (callback == null)
        {
            MAPLog.i(TAG, String.format("Update child app request already in flight for device type %s", new Object[] {
                s1
            }));
            return;
        }
        ...
        MAPLog.i(s4, String.format("Updating child application with device type %s, version %s, and component id %s", new Object[] {
            s1, bundle, s3
        }));
        DMSSubAuthenticator.constructDMSSubAuthForDeviceType(mContext, s1, s2, integer, s3).updateCredentials(callback, s, bundle1, tracer);
    }
