    ...
    implements ADPCorpusSigningAuthenticationMethod
{
    ...
    private static final String TAG = com/amazon/identity/auth/device/api/InProcessAdpAuthenticationMethod.getName();
    ...
    private byte[] getCorpus$3335907d(Uri uri, String s, byte abyte0[], String s1)
    {
...
_L3:
        ...
        return null;
_L11:
        if ("BustedIdentityADPAuthenticator".equals(getAuthenticationType()))
        {
            uri = uri.toString();
        ...
        s3 = uri.getEncodedQuery();
        uri = s2;
        if (!TextUtils.isEmpty(s3))
        {
            uri = (new StringBuilder()).append(s2).append("?").append(s3).toString();
        }
        continue; /* Loop/switch isn't completed */
_L4:
        if (!"BustedIdentityADPAuthenticator".equals(getAuthenticationType())) goto _L6; else goto _L5
_L5:
        abyte0 = new byte[0];
_L8:
        s2 = mTokenManagement.getValue(mDirectedId, TokenKeys.getAdpTokenKeyForPackage(mPackageName), getOptions(), GET_CENTRAL_CREDENTIAL_TIME_OUT_MS);
        if (s2 != null)
        {
...
_L9:
        ...
        MAPLog.e(TAG, (new StringBuilder("Getting ADP Token during corpus generation failed because of callback error. Error Bundle: ")).append(BundleUtils.toString(uri)).toString());
        continue; /* Loop/switch isn't completed */
        uri;
        MAPLog.e(TAG, (new StringBuilder("Getting ADP Token during corpus generation failed because of InterruptedException: ")).append(uri.getMessage()).toString());
        continue; /* Loop/switch isn't completed */
        uri;
        MAPLog.e(TAG, (new StringBuilder("Getting ADP Token during corpus generation failed because of ExecutionException: ")).append(uri.getMessage()).toString());
        continue; /* Loop/switch isn't completed */
        uri;
        MAPLog.e(TAG, (new StringBuilder("Getting ADP Token during corpus generation failed because of TimeoutException: ")).append(uri.getMessage()).toString());
...
_L12:
    ...
    private PrivateKey getPrivateKey()
    {
        ...
        Object obj = mTokenManagement.getValue(mDirectedId, TokenKeys.getPrivateKeyKeyForPackage(mPackageName), getOptions(), GET_CENTRAL_CREDENTIAL_TIME_OUT_MS);
        if (obj == null)
        {
            return null;
        }
        KeyFactory keyfactory = KeyFactoryUtils.getKeyFactoryForKey(((String) (obj)));
        if (keyfactory == null)
        {
            return null;
        }
        obj = keyfactory.generatePrivate(new PKCS8EncodedKeySpec(Base64.decode(KeyFactoryUtils.replacePrivateKeyHeaderAndFooter(((String) (obj))).getBytes(), 0)));
        privatekey = ((PrivateKey) (obj));
_L2:
        return privatekey;
        Object obj1;
        obj1;
        MAPLog.e(TAG, "Could not sign request because there was an invalid key", ((Throwable) (obj1)));
        continue; /* Loop/switch isn't completed */
        obj1;
        Bundle bundle = ((MAPCallbackErrorException) (obj1)).getErrorBundle();
...
_L1:
    ...
    private String signBufferAsBase64(byte abyte0[])
    {
        String s = null;
        ...
        throws IOException
    {
        ...
        AuthenticationMethod.callbackError(callbackfuture, 2, "Could not sign request because we could not get an ADP Signature");
        MAPLog.e(TAG, "Could not sign request because we could not get an ADP Signature");
        return callbackfuture;
        String s1;
        s1 = getCurrentTimestamp();
        uri = getCorpus$3335907d(uri, s, abyte0, s1);
...
_L1:
        ...
        uri = null;
          goto _L3
_L6:
        AuthenticationMethod.callbackError(callbackfuture, 3, "Could not sign request because we could not generate an ADP signature, please check your parameters!");
...
_L7:
        ...
        return callbackfuture;
_L3:
        if (uri != null && map != null) goto _L7; else goto _L6
    }
