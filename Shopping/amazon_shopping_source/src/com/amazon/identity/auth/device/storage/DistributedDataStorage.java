...
public class DistributedDataStorage extends DataStorage
{
    ...
    public DistributedDataStorage(Context context, LocalDataStorage localdatastorage, RetrySyncDirtyDataScheduler retrysyncdirtydatascheduler, LambortishClock lambortishclock)
    {
        mContext = context;
        mLocalDataStorage = localdatastorage;
        mRetryDirtyDataScheduler = retrysyncdirtydatascheduler;
        mLambortishClock = lambortishclock;
    }
    ...
    private RemoteMapInfo getLatestInitializedMapApplication()
    {
        Object obj = MAPApplicationInformationQueryer.getInstance(mContext).getSortedByLatestMapApplications();
...
_L3:
        ...
        remotemapinfo = (RemoteMapInfo)((Iterator) (obj)).next();
        if (remotemapinfo.isSelf())
        {
            continue; /* Loop/switch isn't completed */
        }
        Integer integer = remotemapinfo.fetchMAPInitVersion();
        if (integer == null)
        {
            continue; /* Loop/switch isn't completed */
        }
...
_L2:
        MAPLog.e(TAG, "Found no initialized map application");
        return null;
    }
    ...
    private boolean propagateDataChangeToAllMapApplications(RemoteDataStorageAction remotedatastorageaction)
    {
        ...
        boolean flag1;
        flag1 = true;
        iterator = MAPApplicationInformationQueryer.getInstance(mContext).getAllMapApplication().iterator();
_L5:
        RemoteMapInfo remotemapinfo;
        Object obj;
        int i;
        ...
        obj = new RemoteAmazonDataStorage(mContext, remotemapinfo);
        flag2 = false;
        flag = false;
        i = 0;
_L2:
        int j = i + 1;
        boolean flag3 = remotedatastorageaction.performAction(((RemoteAmazonDataStorage) (obj)));
        flag = true;
_L3:
        if (flag)
        {
...
_L4:
    ...
    {
        if (!context.getSharedPreferences("distributed.datastore.info.store", 0).edit().putBoolean("distributed.datastore.init.key", true).commit())
        {
...
_L3:
    ...
    {
        throw new UnsupportedOperationException("It is not supported on DistributedDataStorage!");
    }
...
_L1:
        ...
        return;
_L2:
        Object obj = getLatestInitializedMapApplication();
        if (obj != null)
        {
            break; /* Loop/switch isn't completed */
        }
...
_L4:
        ...
        throw obj;
_L3:
        MAPLog.i(TAG, "Initializing data storage from application %s", new Object[] {
            ((RemoteMapInfo) (obj)).toString()
        });
        obj = (new RemoteAmazonDataStorage(mContext, ((RemoteMapInfo) (obj)))).getAllData();
...
_L3:
    ...
    public void setTokens(final String directedId, final Map tokens)
    {
        this;
        JVM INSTR monitorenter ;
        final Date timestamp;
        initialize();
        timestamp = mLambortishClock.getNewTimestamp();
        if (mLocalDataStorage.setTokens(new AccountTransaction(directedId, null, tokens), timestamp, false)) goto _L2; else goto _L1
_L1:
        MAPLog.e(TAG, "Setting the token was not successful");
_L4:
        this;
        ...
        propagateDataChangeInTheBackground(new RemoteDataStorageAction() {
            ...
            public boolean performAction(RemoteAmazonDataStorage remoteamazondatastorage)
            {
                for (Iterator iterator = tokens.entrySet().iterator(); iterator.hasNext();)
                {
                    ...
                    if (!remoteamazondatastorage.setToken(directedId, (String)entry.getKey(), (String)entry.getValue(), timestamp))
                    {
                        return false;
                    }
                }
...
_L3:
    ...
    public void setUserData(final String directedId, final Map userData)
    {
        this;
        JVM INSTR monitorenter ;
        final Date timestamp;
        initialize();
        timestamp = mLambortishClock.getNewTimestamp();
        if (mLocalDataStorage.setUserData(new AccountTransaction(directedId, userData, null), timestamp, false)) goto _L2; else goto _L1
_L1:
        MAPLog.e(TAG, "Setting the userdata was not successful");
_L4:
        this;
...
_L2:
        propagateDataChangeInTheBackground(new RemoteDataStorageAction() {
            ...
            public boolean performAction(RemoteAmazonDataStorage remoteamazondatastorage)
            {
                for (Iterator iterator = userData.entrySet().iterator(); iterator.hasNext();)
                {
                    java.util.Map.Entry entry = (java.util.Map.Entry)iterator.next();
                    if (!remoteamazondatastorage.setUserData(directedId, (String)entry.getKey(), (String)entry.getValue(), timestamp))
                    {
                        return false;
                    }
                }
            ...
            {
                String s;
                for (Iterator iterator = userData.keySet().iterator(); iterator.hasNext(); mLocalDataStorage.setNotDirtySetUserData(directedId, s, timestamp))
                {
                    s = (String)iterator.next();
                }
...
_L3:
    ...
    public void setup()
    {
        this;
        JVM INSTR monitorenter ;
        RemoteMapInfo remotemapinfo = MAPApplicationInformationQueryer.getInstance(mContext).getLatestMapApp();
...
_L4:
    ...
    public void syncDirtyData()
    {
        ...
        initialize();
        timestamp = mLambortishClock.getNewTimestamp();
        allCurrentDirtyData = mLocalDataStorage.getAllDirtyData(((Date) (timestamp)));
        if (allCurrentDirtyData.size() != 0) goto _L2; else goto _L1
_L1:
        timestamp = TAG;
_L4:
        this;
