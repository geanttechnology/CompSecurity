...
//            CredentialStatusChecker, CredentialStatus
...
public class AuthenticatedResponseHandler extends WebResponseParser
{
    ...
    {
        return CredentialStatusChecker.getErrorText(getWebResponseHeaders());
    }

    public CredentialStatus getCredentialStatus()
    {
        return CredentialStatusChecker.checkResponse(getWebResponseHeaders());
    }

    public String getCredentialWarningText()
    {
        return CredentialStatusChecker.getWarningText(getWebResponseHeaders());
    }
    ...
    protected boolean internalBeginParse(WebResponseHeaders webresponseheaders)
    {
        ...
        {
            MAPLog.i(TAG, "AuthenticatedResponseHandler: beginParse: Response could be an authentication error. Will try to parse as an authentication error. Status Code: %d Content-Type: %s", new Object[] {
...
_L8:
    ...
    {
        if (mOuterParseError == ParseError.ParseErrorNoError && mShouldTryParsing && !mParser.parseChunk(abyte0, l))
        {
            mOuterParseError = ParseError.ParseErrorMalformedBody;
        }
        if (mInnerParseError == ParseError.ParseErrorNoError && mNextParser != null && mNextParser.shouldParseBody())
        {
