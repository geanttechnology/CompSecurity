...
public class WebResponseHeaders
{
    ...
    public String getHeaderName(long l)
    {
        if (l >= getNumHeaders())
        {
            MAPLog.e(TAG, "WebResponseHeaders: getHeaderName: index out of range");
            return null;
        } else
        {
            return (String)mHeaderNames.get((int)l);
        }
    }

    public String getHeaderValue(long l)
    {
        if (l >= getNumHeaders())
        {
            MAPLog.e(TAG, "WebResponseHeaders: getHeaderValue: index out of range");
            return null;
        } else
        {
            String s = (String)mHeaderNames.get((int)l);
            return (String)mHeaders.get(s.toLowerCase(Locale.US));
        }
    }
