...
//            PlatformLoggingMetricsCollector, PlatformDCPMetricsCollector, PlatformMetricsTimer, PlatformMetricsCollector
...
_L3:
    ...
    public static PlatformMetricsTimer getTimer(String s, String s1)
    {
        s = (new StringBuilder()).append(s).append("_").append(s1).toString();
        return PlatformMetricsTimer.createPeriodicTimer(getMetricsCollectorInstance(sContext), s);
    }

    public static transient void increasePeriodicCounter(String s, String as[])
    {
        ...
        {
            platformmetricscollector.increasePeriodicCounter(s, as);
            return;
        }
    }
    ...
    {
        s = (new StringBuilder()).append(s).append("_").append(s1).toString();
        s = PlatformMetricsTimer.createPeriodicTimer(getMetricsCollectorInstance(sContext), s);
        s.start();
        return s;
    }
...
_L1:
    ...
    public static Callback wrapRegistrationCallbackWithMetrics(Callback callback, Tracer tracer, String s, com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError registrationerror)
    {
        return new Callback(tracer, callback, s, registrationerror) {

            final Callback val$callback;
            final com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError val$defaultError;
            final String val$errorCodeKey;
            final Tracer val$tracer;

            public void onError(Bundle bundle)
            {
                String s1 = com.amazon.identity.auth.device.api.MAPAccountManager.RegistrationError.fromValue(bundle.getInt(errorCodeKey, defaultError.value()), defaultError).getName();
                if (tracer != null)
                {
                    tracer.incrementCounter((new StringBuilder("Error:")).append(s1).toString());
                    tracer.finishTrace();
                }
                callback.onError(bundle);
            }
            ...
            
            {
                ...
                callback = callback1;
                errorCodeKey = s;
                defaultError = registrationerror;
                super();
            }
        };
    }
