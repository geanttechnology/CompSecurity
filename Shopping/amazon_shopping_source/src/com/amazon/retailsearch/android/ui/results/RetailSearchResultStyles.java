...
public class RetailSearchResultStyles
{
    ...
    public static Integer getStyleId(int i, ViewType viewtype, Resources resources)
    {
        if (styles == null)
        {
            initStyles(resources);
        }
        if (viewtype == null || resources == null)
        {
        ...
        viewtype = (new StringBuilder()).append(resources).append(".").append(viewtype.getName()).toString();
        if (styles.get(viewtype) != null)
        {
            i = ((Integer)styles.get(viewtype)).intValue();
        }
        return Integer.valueOf(i);
    }
