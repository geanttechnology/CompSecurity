...
public abstract class BaseAbstractDCMLogger
{
    ...
    private void addDeviceInfo(MetricEvent metricevent)
    {
        ...
        String s1 = DeviceProfile.getTelephonyDeviceId(context);
        if (!TextUtils.isEmpty(s1))
        {
            metricevent.addString(StringName.TelephonyDeviceId.name(), s1);
        }
        s1 = DeviceProfile.getNetworkOperatorName(context);
        if (!TextUtils.isEmpty(s1))
        {
...
_L3:
    ...
    {
        metricsFactory = (MetricsFactory)context1.getSystemService("com.amazon.dp.metrics.MetricsFactory");
...
_L1:
        return;
_L2:
        Object obj = servicecall.getMethod();
        if (obj == null)
        {
