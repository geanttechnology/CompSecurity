...
class val.inFinished extends NamedRunnable
{
    ...
    public void execute()
    {
        boolean flag = SpdyConnection.access$2600(SpdyConnection.this).onData(val$streamId, val$buffer, val$byteCount, val$inFinished);
        if (!flag)
        {
        ...
        {
            SpdyConnection.access$2700(SpdyConnection.this).remove(Integer.valueOf(val$streamId));
        }
        return;
        exception;
        spdyconnection;
