...
public class Base64 extends BaseNCodec
{
    ...
    public Base64(int i, byte abyte0[], boolean flag)
    {
        ...
        if (abyte0 != null)
        {
            if (containsAlphabetOrPad(abyte0))
            {
                ...
                throw new IllegalArgumentException("lineSeparator must not contain base64 characters: [" + abyte0 + "]");
            }
            if (i > 0)
            {
    ...
    public static byte[] encodeBase64(byte abyte0[], boolean flag, boolean flag1, int i)
    {
        ...
        if (l > (long)i)
        {
            throw new IllegalArgumentException("Input array too big, the output array would be bigger (" + l + ") than the specified maximum size of " + i);
        } else
        {
