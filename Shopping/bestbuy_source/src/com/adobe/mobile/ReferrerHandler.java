...
_L3:
    ...
    protected static void handleV1Acquisition(HashMap hashmap)
    {
        if (Lifecycle.lifecycleHasRun)
        {
            ...
            {
                final HashMap referrerLifecycleContextData = new HashMap();
                referrerLifecycleContextData.put("a.referrer.campaign.source", hashmap.get("utm_source"));
                referrerLifecycleContextData.put("a.referrer.campaign.medium", hashmap.get("utm_medium"));
                referrerLifecycleContextData.put("a.referrer.campaign.term", hashmap.get("utm_term"));
                referrerLifecycleContextData.put("a.referrer.campaign.content", hashmap.get("utm_content"));
                referrerLifecycleContextData.put("a.referrer.campaign.name", hashmap.get("utm_campaign"));
                referrerLifecycleContextData.put("a.referrer.campaign.trackingcode", hashmap.get("trackingcode"));
...
_L1:
    ...
    protected static void handleV3Acquisition(final HashMap contextData)
    {
        contextData = getReferrerDataFromV3Server((String)contextData.get("utm_content"), StaticMethods.getAdvertisingIdentifier());
        if (Lifecycle.lifecycleHasRun)
        {
            contextData = parseV3Response(contextData);
            StaticMethods.getAnalyticsExecutor().execute(new Runnable() {
        ...
        try
        {
            ...
            editor.putString("ADMS_Referrer_ContextData_Json_String", contextData);
            editor.commit();
        }
        // Misplaced declaration of an exception variable
        catch (final HashMap contextData)
        {
            StaticMethods.logErrorFormat("Analytics - Error persisting referrer data (%s)", new Object[] {
                contextData.getMessage()
            });
        }
        _referrerProcessed = true;
    }
