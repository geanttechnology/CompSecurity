...
label3:
                ...
                return detectionresultrowindicatorcolumn.getBoundingBox().addMissingRows(l, k, detectionresultrowindicatorcolumn.isLeft());
            }
        }
        return null;
    }
    ...
    private static int adjustCodewordStartColumn(BitMatrix bitmatrix, int i, int j, boolean flag, int k, int l)
    {
        ...
        while (j1 < 2) 
        {
            for (; (flag && i1 >= i || !flag && i1 < j) && flag == bitmatrix.get(i1, l); i1 += k1)
            {
                if (Math.abs(k - i1) > 2)
                {
                    return k;
    ...
    private static DecoderResult createDecoderResult(DetectionResult detectionresult)
    {
        ...
        for (int j = ((flag) ? 1 : 0); j < aobj.length; j++)
        {
...
label0:
            ...
            j2 = detectionresult.getBarcodeColumnCount() + 1;
            detectionresult.setDetectionResultColumn(0, resultpoint5);
            detectionresult.setDetectionResultColumn(j2, resultpoint4);
...
_L2:
        ...
        if (k1 == 0 || k1 == j2)
        {
            ...
            resultpoint = new DetectionResultRowIndicatorColumn(boundingbox, flag1);
        } else
        {
...
_L3:
        i2 = getStartColumn(detectionresult, k1, l1, flag);
        if (i2 >= 0)
        {
            j1 = i2;
...
_L6:
        j1 = i;
        resultpoint1 = detectCodeword(bitmatrix, boundingbox.getMinX(), boundingbox.getMaxX(), flag, j1, l1, i1, l);
        if (resultpoint1 != null)
        {
            resultpoint.setCodeword(l1, resultpoint1);
            i1 = Math.min(i1, resultpoint1.getWidth());
            l = Math.max(l, resultpoint1.getWidth());
            i = j1;
        }
...
_L7:
    ...
    private static Codeword detectCodeword(BitMatrix bitmatrix, int i, int j, boolean flag, int k, int l, int i1, int j1)
    {
        ...
        {
            return new Codeword(i, k, getCodewordBucketNumber(j), l);
        }
    }
...
_L6:
    ...
    private static int[] getModuleBitCount(BitMatrix bitmatrix, int i, int j, boolean flag, int k, int l)
    {
        ...
        flag1 = flag;
        for (j1 = 0; (flag && k < j || !flag && k >= i) && j1 < ai.length;)
        {
            if (bitmatrix.get(k, l) == flag1)
            {
                ai[j1] = ai[j1] + 1;
                k += i1;
            } else
        ...
        if (j1 == ai.length || (flag && k == j || !flag && k == i) && j1 == ai.length - 1)
        {
            return ai;
        } else
        {
        ...
        for (; isValidBarcodeColumn(detectionresult, j - k); j = l)
        {
            ...
            for (j = 0; j < i1; j++)
            {
                ...
                if (codeword1 != null)
                {
                    ...
                    return k * i * (codeword1.getEndX() - codeword1.getStartX()) + j;
                }
            }
