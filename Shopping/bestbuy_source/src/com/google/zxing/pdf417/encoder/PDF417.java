...
//            Compaction, BarcodeRow, BarcodeMatrix, PDF417ErrorCorrection, 
...
public final class PDF417
{
    ...
    private static final float PREFERRED_RATIO = 3F;
    ...
    private static int calculateNumberOfRows(int i, int j, int k)
    {
        int i1 = (i + 1 + j) / k + 1;
        int l = i1;
        if (k * i1 >= i + 1 + j + k)
    ...
    private int[] determineDimensions(int i, int j)
    {
        float f;
        int ai[];
        int k;
...
label0:
        {
            ...
            {
                ai1 = ai;
                if (calculateNumberOfRows(i, j, minCols) < minRows)
                {
                    ai1 = new int[2];
                    ai1[0] = minCols;
                    ai1[1] = minRows;
...
_L1:
        f2 = ((float)(k * 17 + 69) * 0.357F) / ((float)l * 2.0F);
        if (ai == null) goto _L4; else goto _L3
_L3:
        f1 = f;
        if (Math.abs(f2 - 3F) > Math.abs(f - 3F)) goto _L5; else goto _L4
_L4:
        ai = new int[2];
        ai[0] = k;
        ai[1] = l;
    ...
    private static void encodeChar(int i, int j, BarcodeRow barcoderow)
    {
        int l = 1 << j - 1;
        int k;
        int i1;
    ...
    private void encodeLowLevel(CharSequence charsequence, int i, int j, int k, BarcodeMatrix barcodematrix)
    {
        int k1 = 0;
        int j1 = 0;
        while (j1 < j) 
        {
            ...
            encodeChar(0x1fea8, 17, barcodematrix.getCurrentRow());
            int l;
            int i1;
            if (l1 == 0)
            {
            ...
            encodeChar(CODEWORD_TABLE[l1][i1], 17, barcodematrix.getCurrentRow());
            for (i1 = 0; i1 < i; i1++)
            {
                encodeChar(CODEWORD_TABLE[l1][charsequence.charAt(k1)], 17, barcodematrix.getCurrentRow());
                k1++;
            }

            if (compact)
            {
                encodeChar(0x3fa29, 1, barcodematrix.getCurrentRow());
            } else
            {
                encodeChar(CODEWORD_TABLE[l1][l], 17, barcodematrix.getCurrentRow());
                encodeChar(0x3fa29, 18, barcodematrix.getCurrentRow());
            }
            j1++;
        }
    }

    private static int getNumberOfPadCodewords(int i, int j, int k, int l)
    {
        j = k * l - j;
        if (j > i + 1)
        {
    ...
    public void generateBarcodeLogic(String s, int i)
    {
        int j = 0;
        int j1 = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(i);
        String s1 = PDF417HighLevelEncoder.encodeHighLevel(s, compaction);
        int k1 = s1.length();
        int ai[] = determineDimensions(k1, j1);
        int k = ai[0];
        int l = ai[1];
        int i1 = getNumberOfPadCodewords(k1, j1, k, l);
        if (j1 + k1 + 1 > 929)
        {
        ...
        s = s.toString();
        s1 = PDF417ErrorCorrection.generateErrorCorrection(s, i);
        s = (new StringBuilder()).append(s).append(s1).toString();
        barcodeMatrix = new BarcodeMatrix(l, k);
        encodeLowLevel(s, k, l, i, barcodeMatrix);
    }
    ...
    public void setCompaction(Compaction compaction1)
    {
