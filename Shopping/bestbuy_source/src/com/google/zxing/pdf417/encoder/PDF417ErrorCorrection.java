...
final class PDF417ErrorCorrection
{
    ...
    {
        int l = getErrorCorrectionCodewordCount(i);
        char ac[] = new char[l];
        int i1 = charsequence.length();
        for (int j = 0; j < i1; j++)
        {
            int j1 = (charsequence.charAt(j) + ac[ac.length - 1]) % 929;
            for (int k = l - 1; k >= 1; k--)
            {
                ac[k] = (char)(((929 - (EC_COEFFICIENTS[i][k] * j1) % 929) + ac[k - 1]) % 929);
            }

            ac[0] = (char)((929 - (EC_COEFFICIENTS[i][0] * j1) % 929) % 929);
        }

        charsequence = new StringBuilder(l);
        for (i = l - 1; i >= 0; i--)
        {
            if (ac[i] != 0)
            {
