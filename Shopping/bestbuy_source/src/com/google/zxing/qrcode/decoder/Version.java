...
public final class Version
{
    ...
    private transient Version(int i, int ai[], ECBlocks aecblocks[])
    {
        ...
        ecBlocks = aecblocks;
        int l = aecblocks[0].getECCodewordsPerBlock();
        ...
        {
            aecblocks = ai[i];
            int k = aecblocks.getCount();
            j += (aecblocks.getDataCodewords() + l) * k;
        }
    ...
    static Version decodeVersionInformation(int i)
    {
        ...
        int k = 0x7fffffff;
        int i1 = 0;
        while (j < VERSION_DECODE_INFO.length) 
        {
            int l = VERSION_DECODE_INFO[j];
            if (l == i)
            {
                return getVersionForNumber(j + 7);
            }
            int j1 = FormatInformation.numBitsDiffering(i, l);
            l = k;
            if (j1 < k)
            {
                i1 = j + 7;
