    ...
    implements Runnable
{
    ...
    static Bitmap applyCustomTransformations(List list, final Bitmap builder)
    {
        int j = list.size();
        for (int i = 0; i < j;)
        {
            final Transformation transformation = (Transformation)list.get(i);
            Bitmap bitmap = transformation.transform(builder);
            if (bitmap == null)
            {
                ...
                for (list = list.iterator(); list.hasNext(); builder.append(((Transformation)list.next()).key()).append('\n')) { }
