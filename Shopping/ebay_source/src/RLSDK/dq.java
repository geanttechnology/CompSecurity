...
_L2:
    ...
    private boolean b()
    {
        ...
        for (int j = 0; j < l;)
        {
            float f3 = Math.abs(((dp)b.elementAt(j)).c() - f2);
            j++;
            f1 = f3 + f1;
        }
    ...
    final dr a(Hashtable hashtable)
        throws v
    {
        int ai[];
        int i;
...
_L6:
        ...
        c = true;
        i = (int)(Math.abs(hashtable.a() - dp1.a()) - Math.abs(hashtable.b() - dp1.b())) / 2;
_L9:
        if (i > ai[2])
        {
            i = k + (i - ai[2] - 2);
...
_L2:
        ...
        if (j > 3)
        {
            ...
            ak.a(b, new b(f1, (byte)0));
            f = Math.max(0.2F * f1, f);
            for (i = 0; i < b.size() && b.size() > 3; i = j + 1)
            {
                j = i;
                if (Math.abs(((dp)b.elementAt(i)).c() - f1) > f)
                {
                    b.removeElementAt(i);
                    j = i - 1;
                }
        ...
        if (b.size() > 3)
        {
            ...
            for (i = 0; i < b.size(); i++)
            {
                f += ((dp)b.elementAt(i)).c();
            }

            f /= b.size();
            ak.a(b, new a(f, (byte)0));
            b.setSize(3);
        }
        hashtable = new dp[3];
        hashtable[0] = (dp)b.elementAt(0);
        hashtable[1] = (dp)b.elementAt(1);
        hashtable[2] = (dp)b.elementAt(2);
        aa.a(hashtable);
        return new dr(hashtable);
    }
}
