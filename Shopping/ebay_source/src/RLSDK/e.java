    ...
    implements android.hardware.Camera.AutoFocusCallback, android.hardware.Camera.ErrorCallback, android.hardware.Camera.PictureCallback, android.hardware.Camera.PreviewCallback, android.view.SurfaceHolder.Callback
{
    ...
    protected static final String TAG = RLSDK/e.getSimpleName();
    protected static e cameraManager;
    protected Camera camera;
    protected Matrix cameraToPreviewMatrix;
    protected final d configManager = new d();
...
_L1:
        Class class1 = Class.forName("RLSDK.h");
_L3:
        cameraManager = (e)class1.getDeclaredConstructors()[0].newInstance(new Object[] {
            application
        });
        return;
_L2:
        ...
        class1 = Class.forName("RLSDK.f");
          goto _L3
        class1 = Class.forName("RLSDK.e");
          goto _L3
        application;
        application = TAG;
        return;
    ...
    protected Point calculatePreviewSurfaceSize()
    {
        ...
        float f1 = mPreviewParentView.getWidth();
        float f2 = mPreviewParentView.getHeight();
        Point point = configManager.a();
        int j = relativeCameraOrientation;
        float f;
        float f3;
        float f4;
        if (j == 90 || j == 270)
        ...
        cameraToPreviewMatrix.postRotate(j);
        cameraToPreviewMatrix.postScale(f, f);
        cameraToPreviewMatrix.postTranslate(f1 / 2.0F, f2 / 2.0F);
        if (((String) (obj)).equalsIgnoreCase("Nexus 7") && (j == 90 || j == 270))
        {
    ...
    public void onAutoFocus(boolean flag, Camera camera1)
    {
        ...
        if (torchSetting != getTorch())
        {
            setTorch(torchSetting);
        }
        if (focusStyle == 2)
        {
...
_L3:
    ...
    public void onTapEvent(int j, int k)
    {
        ...
        } else
        {
            ...
            if (camera == null)
            {
                try
                {
                    ...
                    mPreviewSurface = new i(mContext);
    ...
    public void setPreviewFrameHandler(b b1)
    {
        frameEngine = b1;
    }
