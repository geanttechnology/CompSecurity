...
label0:
    ...
    private void pullGlows(int i, int j, int k, int l)
    {
        boolean flag = false;
        if (j < 0)
        {
            ensureLeftGlow();
            if (mLeftGlow.onPull((float)(-j) / (float)getWidth(), 1.0F - (float)k / (float)getHeight()) || false)
            {
                flag = true;
            } else
            {
        ...
        {
            ensureRightGlow();
            if (mRightGlow.onPull((float)j / (float)getWidth(), (float)k / (float)getHeight()) || false)
            {
                flag = true;
            } else
            {
        ...
        {
            ensureTopGlow();
            if (mTopGlow.onPull((float)(-l) / (float)getHeight(), (float)i / (float)getWidth()) || flag)
            {
                k = 1;
            } else
            {
        ...
        } else
        {
            ...
            {
                ensureBottomGlow();
                if (mBottomGlow.onPull((float)l / (float)getHeight(), 1.0F - (float)i / (float)getWidth()) || flag)
                {
                    k = 1;
                } else
                {
...
_L4:
        ...
        if (mScrollState == 1)
        {
            ...
            if (scrollByInternal(j, j1, true, k1, l1))
            {
                getParent().requestDisallowInterceptTouchEvent(true);
            }
        }
    ...
    public void scrollBy(int i, int j)
    {
        ...
        } else
        {
            ...
            if (flag || flag1)
            {
                ...
                scrollByInternal(i, j, false, 0, 0);
                return;
            }
        }
    }

    boolean scrollByInternal(int i, int j, boolean flag, int k, int l)
    {
        int j1 = 0;
        boolean flag3 = false;
        int k1 = 0;
        ...
        if (mAdapter != null)
        {
            ...
            if (j != 0)
            {
                l1 = mLayout.scrollVerticallyBy(j, mRecycler, mState);
                k1 = j - l1;
            }
            TraceCompat.endSection();
            if (supportsChangeAnimations())
            {
                int j2 = mChildHelper.getChildCount();
                int i2 = 0;
                while (i2 < j2) 
                {
                    View view = mChildHelper.getChildAt(i2);
                    Object obj = getChildViewHolder(view);
                    if (obj == null || ((ViewHolder) (obj)).mShadowingHolder == null)
                    {
                        continue;
