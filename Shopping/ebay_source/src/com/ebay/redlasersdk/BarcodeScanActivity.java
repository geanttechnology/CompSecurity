...
public abstract class BarcodeScanActivity extends Activity
{
    ...
    private final Runnable statusUpdateRunnable = new Runnable() {
        ...
        public void run()
        {
            if (RLSDK.a.a.b == RLSDK.a.a.c)
            {
                denyDecode("Unregistered SDK Limit Reached");
            } else
            if (RLSDK.a.a.b == RLSDK.a.a.e)
            {
                denyDecode("RedLaser Error");
            } else
            {
                java.util.HashMap hashmap = barcodeEngine.b();
                Set set = (Set)hashmap.get("FoundBarcodes");
                Matrix matrix = e.get().cameraToPreviewMatrix();
...
// JavaClassFileOutputException: get_constant: invalid tag
    ...
    private final void initCamera(FrameLayout framelayout)
    {
        try
        {
            e.get().setSelectedCameraIndex(mRequestedCameraIndex);
            e.get().openDriver(this, framelayout);
            e.get().setPreviewFrameHandler(barcodeEngine);
            return;
        }
    ...
    protected void doneScanning()
    {
        ...
        if (RLSDK.a.a.b == RLSDK.a.a.c || RLSDK.a.a.b == RLSDK.a.a.e)
        {
            setResult(0, intent);
        } else
        {
            intent.putParcelableArrayListExtra("com.ebay.redlasersdk.results", new ArrayList(set));
            setResult(-1, intent);
        }
        finish();
    }
    ...
    {
        return e.get().getTorch();
    }

    protected boolean isTorchAvailable()
    {
        return e.get().isTorchAvailable();
    }
    ...
    public void onCreate(Bundle bundle)
    {
        ...
        barcodeEngine = new b(enabledTypes, this);
    ...
    protected void onPause()
    {
        ...
        if (e.get() != null)
        {
            e.get().stopPreview(this);
            e.get().closeDriver(this);
        }
    }
...
label0:
    ...
    protected void setActiveRect(Rect rect)
    {
        ...
        Matrix matrix = e.get().previewToCameraMatix();
        if (matrix != null)
        {
            rect = new RectF(rect);
            matrix.mapRect(rect);
