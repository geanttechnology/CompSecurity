...
class KatanaProxyLoginMethodHandler extends LoginMethodHandler
{
    ...
    private LoginClient.Result handleResultOk(LoginClient.Request request, Intent intent)
    {
        ...
        if (intent == null && s2 == null && s == null)
        {
            try
            {
                result = LoginClient.Result.createTokenResult(request, createAccessTokenFromWebBundle(request.getPermissions(), bundle, AccessTokenSource.FACEBOOK_APPLICATION_WEB, request.getApplicationId()));
            }
