...
_L3:
    ...
    private static ClassLoader getClassLoader(Class class1, ClassLoader classloader)
    {
        ...
        if (Visibility.forType(class1) == Visibility.PUBLIC)
        {
            ...
            {
                return (ClassLoader)CLASS_LOADER_CACHE.getUnchecked(classloader);
            } else
            {
    ...
    static 
    {
        ...
        CacheBuilder cachebuilder = CacheBuilder.newBuilder().weakKeys().weakValues();
        if (!CUSTOM_LOADER_ENABLED)
        {
            cachebuilder.maximumSize(0L);
        }
        CLASS_LOADER_CACHE = cachebuilder.build(new CacheLoader() {
