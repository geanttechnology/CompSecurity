...
public class Draft_76 extends Draft_75
{
    ...
    private static String generateKey()
    {
        ...
        for (int i = 0; i < k + 1; i++)
        {
            ...
            s = (new StringBuilder(s)).insert(i1, c).toString();
        }
    ...
    {
        if (framedata.getOpcode() == org.java_websocket.framing.Framedata.Opcode.CLOSING)
        {
        ...
        throws InvalidHandshakeException
    {
        ...
        serverhandshakebuilder.put("Sec-WebSocket-Location", (new StringBuilder()).append("ws://").append(clienthandshake.getFieldValue("Host")).append(clienthandshake.getResourceDescriptor()).toString());
        ...
        throws InvalidHandshakeException
    {
        ...
        if ((handshakebuilder.hasFieldValue("Sec-WebSocket-Key1") || role == org.java_websocket.WebSocket.Role.CLIENT) && !handshakebuilder.hasFieldValue("Sec-WebSocket-Version"))
        {
            byte abyte0[];
            byte byte0;
            if (role == org.java_websocket.WebSocket.Role.SERVER)
            {
                byte0 = 8;
            } else
            {
