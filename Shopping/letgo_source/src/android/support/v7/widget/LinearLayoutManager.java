...
_L2:
        ...
        public RecyclerView.ViewHolder nextViewHolderInLimitedList(RecyclerView.ViewHolder viewholder)
        {
            ...
            while (j < l) 
            {
                RecyclerView.ViewHolder viewholder2 = (RecyclerView.ViewHolder)mScrapList.get(j);
                if (viewholder2 != viewholder && !viewholder2.isRemoved())
                {
                    int k = (viewholder2.getLayoutPosition() - mCurrentPosition) * mItemDirection;
                    if (k >= 0 && k < i)
                    {
                        if (k == 0)
                        {
...
_L1:
        ...
        if (!((RecyclerView.LayoutParams)view2.getLayoutParams()).isItemRemoved()) goto _L4; else goto _L3
_L3:
        if (view != null)
        {
            break MISSING_BLOCK_LABEL_187;
...
_L4:
        if (mOrientationHelper.getDecoratedStart(view2) >= j1) goto _L6; else goto _L5
_L5:
        View view4 = view2;
        if (mOrientationHelper.getDecoratedEnd(view2) >= i1) goto _L7; else goto _L6
_L6:
        if (view1 != null)
        {
            break MISSING_BLOCK_LABEL_187;
...
_L3:
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent accessibilityevent)
    {
        ...
        if (getChildCount() > 0)
        {
            accessibilityevent = bc.a(accessibilityevent);
