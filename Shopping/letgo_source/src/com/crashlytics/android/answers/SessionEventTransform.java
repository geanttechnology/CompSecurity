    ...
    implements anw
{
    ...
    public byte[] toBytes(SessionEvent sessionevent)
        throws IOException
    {
        sessionevent = buildJsonForEvent(sessionevent);
        if (!(sessionevent instanceof JSONObject))
        {
            sessionevent = sessionevent.toString();
        } else
        {
            sessionevent = JSONObjectInstrumentation.toString((JSONObject)sessionevent);
        }
        return sessionevent.getBytes("UTF-8");
    }
