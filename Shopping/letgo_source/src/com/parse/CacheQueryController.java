...
class CacheQueryController extends AbstractQueryController
{
    private static interface CommandDelegate
    {
        ...
        public abstract cu runFromCacheAsync();

        public abstract cu runOnNetworkAsync(boolean flag);
    }
    ...
    private cu countFromCacheAsync(ParseQuery.State state, String s)
    {
    ...
    private cu findFromCacheAsync(ParseQuery.State state, String s)
    {
        return cu.a(new Callable() {
    ...
    private cu runCommandWithPolicyAsync(final CommandDelegate c, ParseQuery.CachePolicy cachepolicy)
    {
        static class _cls7
        {
        ...
        case 4: // '\004'
            return c.runFromCacheAsync().b(new ct() {
        ...
        case 5: // '\005'
            return c.runOnNetworkAsync(false).b(new ct() {
    ...
    public cu countAsync(final ParseQuery.State state, final ParseUser sessionToken, final cu cancellationToken)
    {
    ...
    public cu findAsync(final ParseQuery.State state, final ParseUser sessionToken, final cu cancellationToken)
    {
        if (sessionToken != null)
        {
            sessionToken = sessionToken.getSessionToken();
