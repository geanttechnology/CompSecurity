...
class ParseOperationSet extends HashMap
{
    ...
    public ParseOperationSet(ParseOperationSet parseoperationset)
    {
        ...
        isSaveEventually = parseoperationset.isSaveEventually;
    }
    ...
    public void mergeFrom(ParseOperationSet parseoperationset)
    {
        ...
        for (Iterator iterator = parseoperationset.keySet().iterator(); iterator.hasNext(); put(s, parsefieldoperation))
        {
            ...
            ParseFieldOperation parsefieldoperation1 = (ParseFieldOperation)parseoperationset.get(s);
            ParseFieldOperation parsefieldoperation2 = (ParseFieldOperation)get(s);
            parsefieldoperation = parsefieldoperation1;
            if (parsefieldoperation2 != null)
            {
                parsefieldoperation = parsefieldoperation2.mergeWithPrevious(parsefieldoperation1);
            }
        }
        ...
        throws JSONException
    {
        ...
        String s;
        for (Iterator iterator = keySet().iterator(); iterator.hasNext(); jsonobject.put(s, ((ParseFieldOperation)get(s)).encode(parseencoder)))
        {
            s = (String)iterator.next();
        }
