...
class ServiceUtils
{
    ...
    public static boolean runIntentInService(Context context, Intent intent, Class class1)
    {
        ...
        if (intent != null)
        {
            ...
            if (!flag)
            {
                PLog.e("com.parse.ServiceUtils", (new StringBuilder()).append("Could not start the service. Make sure that the XML tag <service android:name=\"").append(class1.toString()).append("\" /> is in your ").append("AndroidManifest.xml as a child of the <application> element.").toString());
                flag1 = flag;
            }
        }
        return flag1;
