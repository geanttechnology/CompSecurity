...
public class RealmQuery
{
    ...
    RealmQuery(Realm realm1, LinkView linkview, Class class1)
    {
        ...
        columns = realm1.columnIndices.getClassFields(class1);
    }
    ...
    public RealmQuery(RealmResults realmresults, Class class1)
    {
        ...
        columns = realm.columnIndices.getClassFields(class1);
    }
    ...
    private long[] getColumnIndices(String s, ColumnType columntype)
    {
        ...
        if (containsDot(s))
        {
            ...
            while (i < as.length - 1) 
            {
                ...
                if (l < 0L)
                {
                    throw new IllegalArgumentException((new StringBuilder()).append("Invalid query: ").append(as[i]).append(" does not refer to a class.").toString());
                }
    ...
    public RealmQuery isNotNull(String s)
    {
        return beginGroup().not().isNull(s).endGroup();
    }

    public RealmQuery isNull(String s)
    {
        if (containsDot(s))
        {
        ...
        {
            query.isNull(((Long)columns.get(s)).longValue());
            return this;
        }
    }
