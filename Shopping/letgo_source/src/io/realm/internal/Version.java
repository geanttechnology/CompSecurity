...
public class Version
{
    ...
    public static boolean coreLibVersionCompatible(boolean flag)
    {
        ...
        {
            String s = (new StringBuilder()).append("Version mismatch between realm.jar (0.1.6) and native core library (").append(getCoreVersion()).append(")").toString();
            if (flag)
            {
        ...
        } else
        {
            ...
            if (!flag1)
            {
                String s1 = (new StringBuilder()).append("Native lib API is version ").append(nativeGetAPIVersion()).append(" != ").append(23).append(" which is expected by the jar.").toString();
                if (flag)
                {
