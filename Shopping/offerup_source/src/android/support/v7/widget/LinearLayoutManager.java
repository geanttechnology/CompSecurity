...
_L6:
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler, RecyclerView.State state, int i, int j)
    {
        ...
        int i2 = list.size();
        int j2 = getPosition(getChildAt(0));
        int i1 = 0;
        while (i1 < i2) 
        {
            ...
            if (!viewholder.isRemoved())
            {
                int j1;
                boolean flag;
                if (viewholder.getLayoutPosition() < j2)
                {
                    flag = true;
                } else
                {
                ...
                {
                    int k1 = mOrientationHelper.getDecoratedMeasurement(viewholder.itemView) + l;
                    l = k;
                    k = k1;
                }
            } else
