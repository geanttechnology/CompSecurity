...
label0:
    ...
    {
        inputstream = createLightweightScaledBitmapFromStream(inputstream, i, j, config);
        int l = inputstream.getWidth();
        int k = inputstream.getHeight();
        ...
        if (f < 1.0F)
        {
            ...
            config = Bitmap.createScaledBitmap(inputstream, i, j, true);
            Log.d("Final bitmap dimensions: %d x %d", new Object[] {
                Integer.valueOf(config.getWidth()), Integer.valueOf(config.getHeight())
            });
            inputstream.recycle();
            return config;
        } else
