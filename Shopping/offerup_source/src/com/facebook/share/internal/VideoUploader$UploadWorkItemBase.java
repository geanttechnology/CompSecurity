...
abstract class completedRetries
    implements Runnable
{
    ...
    private boolean attemptRetry(int i)
    {
        if (completedRetries < 2 && getTransientErrorCodes().contains(Integer.valueOf(i)))
        {
            ...
            VideoUploader.access$800().postDelayed(new _cls1(), i * 5000);
            return true;
        } else
        {
            return false;
    ...
    protected void issueResponseOnMainThread(final FacebookException error, final String videoId)
    {
        ...
        VideoUploader.access$800().post(new _cls2());
    }
    ...
    protected _cls2(_cls2 _pcls2, int i)
    {
        uploadContext = _pcls2;
        completedRetries = i;
    }
}
