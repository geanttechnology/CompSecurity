...
class EventDataManager extends DataManager
{
    ...
    boolean deleteEventType(String s)
    {
    ...
    boolean deleteEvents(Set set)
    {
        ...
        int i = set.size();
        String s = repeat("?", i, ", ");
        return delete("events", (new StringBuilder("event_id IN ( ")).append(s).append(" )").toString(), (String[])set.toArray(new String[i])) > 0;
    }

    boolean deleteSession(String s)
    {
        ...
        if (i > 0)
        {
            Logger.debug((new StringBuilder("EventDataManager - Deleted ")).append(i).append(" rows with session ID ").append(s).toString());
            return true;
        } else
        {
            return false;
    ...
    long insertEvent(String s, String s1, String s2, String s3, String s4)
    {
    ...
    {
        Logger.debug((new StringBuilder("EventDataManager - Upgrading analytics database from version ")).append(i).append(" to ").append(j).append(", which will destroy all old data").toString());
        sqlitedatabase.execSQL("DROP TABLE IF EXISTS events");
        onCreate(sqlitedatabase);
    }
}
