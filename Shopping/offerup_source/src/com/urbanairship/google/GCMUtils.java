...
public class GCMUtils
{
    ...
    public static void validateManifest(AirshipConfigOptions airshipconfigoptions)
    {
        ...
        if (airshipconfigoptions.minSdkVersion < 16 || applicationinfo != null && applicationinfo.targetSdkVersion < 16 || android.os.Build.VERSION.SDK_INT < 16)
        {
            ...
            {
                Logger.error((new StringBuilder("Required permission ")).append(airshipconfigoptions).append(" is unknown to PackageManager.").toString());
            }
        }
        if (PlayServicesUtils.isGoogleCloudMessagingDependencyAvailable())
        {
            if (ManifestUtils.getReceiverInfo(com/urbanairship/push/GCMPushReceiver) != null)
            {
                ...
                {
                    Logger.error((new StringBuilder("AndroidManifest.xml's ")).append(com/urbanairship/push/GCMPushReceiver.getCanonicalName()).append(" declaration missing required ").append(airshipconfigoptions.getAction()).append(" filter with category = ").append(s).toString());
                }
            } else
            {
                Logger.error((new StringBuilder("AndroidManifest.xml missing required receiver: ")).append(com/urbanairship/push/GCMPushReceiver.getCanonicalName()).toString());
            }
            try
            {
            ...
            {
                Logger.error((new StringBuilder("Google Play services developer error: ")).append(airshipconfigoptions.getMessage()).toString());
            }
            return;
        } else
        {
