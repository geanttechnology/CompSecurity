    ...
    implements Encoder
{
        ...
        throws IOException
    {
        ...
        do
        {
            if (i <= 0 || !ignore(s.charAt(i - 1)))
            {
                int l = i - 4;
                int j1;
                for (int k = nextI(s, 0, l); k < l; k = nextI(s, j1 + 1, l))
                {
        ...
        return j + decodeLastBlock(outputstream, s.charAt(i - 4), s.charAt(i - 3), s.charAt(i - 2), s.charAt(i - 1));
    }
        ...
        throws IOException
    {
        ...
        do
        {
            ...
            {
                int i1 = j - 4;
                int l = nextI(abyte0, i, i1);
                i = ((flag) ? 1 : 0);
                int k1;
                for (int k = l; k < i1; k = nextI(abyte0, k1 + 1, i1))
                {
                    byte byte0 = decodingTable[abyte0[k]];
                    int j1 = nextI(abyte0, k + 1, i1);
                    k = decodingTable[abyte0[j1]];
                    k1 = nextI(abyte0, j1 + 1, i1);
                    j1 = decodingTable[abyte0[k1]];
                    k1 = nextI(abyte0, k1 + 1, i1);
                    byte byte1 = decodingTable[abyte0[k1]];
                    outputstream.write(byte0 << 2 | k >> 4);
                    outputstream.write(k << 4 | j1 >> 2);
                    outputstream.write(j1 << 6 | byte1);
                    i += 3;
                }
        ...
        return i + decodeLastBlock(outputstream, (char)abyte0[j - 4], (char)abyte0[j - 3], (char)abyte0[j - 2], (char)abyte0[j - 1]);
    }
