...
_L4:
        ...
        byte byte0 = decodingTable[s.charAt(i)];
        for (i++; i < j && ignore(s.charAt(i)); i++) { }
        byte abyte0[] = decodingTable;
        int l = i + 1;
        outputstream.write(byte0 << 4 | abyte0[s.charAt(i)]);
        k++;
        i = l;
...
_L1:
        ...
        byte byte0 = decodingTable[abyte0[i]];
        for (i++; i < j && ignore((char)abyte0[i]); i++) { }
        byte abyte1[] = decodingTable;
        int l = i + 1;
        outputstream.write(byte0 << 4 | abyte1[abyte0[i]]);
        k++;
        i = l;
        continue; /* Loop/switch isn't completed */
_L2:
