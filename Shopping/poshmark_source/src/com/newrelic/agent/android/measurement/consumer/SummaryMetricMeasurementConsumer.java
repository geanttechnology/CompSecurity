    ...
    implements TraceLifecycleAware
{
    ...
    private void summarizeActivityMetrics(ActivityTrace activitytrace)
    {
        ...
        {
            d += ((Metric)iterator.next()).getExclusive();
        }

        double d3 = (double)(trace.exitTimestamp - trace.entryTimestamp) / 1000D;
        Metric metric;
        for (obj = ((Map) (obj)).values().iterator(); ((Iterator) (obj)).hasNext(); Harvest.addMetric(metric))
        {
            metric = (Metric)((Iterator) (obj)).next();
            double d2 = 0.0D;
            double d1 = d2;
            if (metric.getExclusive() != 0.0D)
            {
                d1 = d2;
                if (d != 0.0D)
                {
