...
label0:
    ...
    private JsonArray traceToTree(Trace trace)
    {
        ...
        jsonarray1.add(SafeJsonPrimitive.factory(Long.valueOf(trace.threadId)));
        jsonarray1.add(SafeJsonPrimitive.factory(trace.threadName));
    ...
    public void addCompletedTrace(Trace trace)
    {
        ...
        if (traceCount > 2000)
        {
            log.verbose((new StringBuilder()).append("Maximum trace limit reached, discarding trace ").append(trace.myUUID).toString());
            return;
        }
        ...
        if (log.getLevel() == 4)
        {
            log.verbose((new StringBuilder()).append("Added trace ").append(trace.myUUID.toString()).append(" missing children: ").append(missingChildren.size()).toString());
        }
    ...
    public JsonArray asJsonArray()
    {
        ...
        if (!complete)
        {
            log.verbose((new StringBuilder()).append("Attempted to serialize trace ").append(rootTrace.myUUID.toString()).append(" but it has yet to be finalized").toString());
            return null;
        }
        jsonarray.add((new Gson()).toJsonTree(params, GSON_STRING_MAP_TYPE));
        jsonarray.add(SafeJsonPrimitive.factory(Long.valueOf(rootTrace.entryTimestamp)));
        jsonarray.add(SafeJsonPrimitive.factory(Long.valueOf(rootTrace.exitTimestamp)));
        jsonarray.add(SafeJsonPrimitive.factory(rootTrace.displayName));
        JsonArray jsonarray1 = new JsonArray();
