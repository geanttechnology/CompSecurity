...
_L4:
        if (log.getLevel() == 4)
        {
            log.verbose((new StringBuilder()).append("Trace ").append(trace1.myUUID.toString()).append(" is now active").toString());
            return;
        }
...
_L3:
        ...
        throws TracingInactiveException
    {
        ...
        if (log.getLevel() == 4)
        {
            log.verbose((new StringBuilder()).append("Registering trace of ").append(s).append(" with parent ").append(trace.displayName).toString());
        }
...
_L3:
    ...
    public static void unloadTraceContext(Object obj)
    {
        ...
        {
            log.verbose((new StringBuilder()).append("Trace ").append(((Trace)threadLocalTrace.get()).myUUID.toString()).append(" is now inactive").toString());
        }
        threadLocalTrace.remove();
        threadLocalTraceStack.remove();
        try
