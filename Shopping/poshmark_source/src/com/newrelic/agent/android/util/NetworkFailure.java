...
public final class NetworkFailure extends Enum
{
    ...
    {
        log.debug((new StringBuilder()).append("fromErrorCode invoked with errorCode: ").append(i).toString());
        NetworkFailure anetworkfailure[] = values();
        int k = anetworkfailure.length;
        for (int j = 0; j < k; j++)
        {
            NetworkFailure networkfailure = anetworkfailure[j];
            if (networkfailure.getErrorCode() == i)
            {
                log.debug((new StringBuilder()).append("fromErrorCode found matching failure: ").append(networkfailure).toString());
                return networkfailure;
            }
        }
