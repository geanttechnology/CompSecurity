...
public class FbDialogStateMachine extends PM_State_Machine
    implements StateCompletionListener
{

    ShareManager shareManager;

    public FbDialogStateMachine(ShareManager sharemanager)
    {
        shareManager = sharemanager;
        transitions.addStateObject(new FbDialogShare(this, sharemanager));
        ...
        transitions.addTransition(GetServerContent.id, FbDialogShare.id, FSMTransition.CONDITION.OK);
        transitions.addTransition(GetServerContent.id, GetLocalContent.id, FSMTransition.CONDITION.FAIL);
        transitions.addTransition(GetLocalContent.id, FbDialogShare.id, FSMTransition.CONDITION.OK);
        transitions.addTransition(FbDialogShare.id, Finish.id, FSMTransition.CONDITION.OK);
