...
import com.poshmark.utils.sharing.share_states.FbMessengerShare;
...
public class FbMessengerStateMachine extends PM_State_Machine
    implements StateCompletionListener
{

    ShareManager shareManager;

    public FbMessengerStateMachine(ShareManager sharemanager)
    {
        shareManager = sharemanager;
        transitions.addStateObject(new FbMessengerShare(this, sharemanager));
        ...
        transitions.addTransition(GetServerContent.id, FbMessengerShare.id, FSMTransition.CONDITION.OK);
        transitions.addTransition(GetServerContent.id, GetLocalContent.id, FSMTransition.CONDITION.FAIL);
        transitions.addTransition(GetLocalContent.id, FbMessengerShare.id, FSMTransition.CONDITION.OK);
        transitions.addTransition(FbMessengerShare.id, Finish.id, FSMTransition.CONDITION.OK);
