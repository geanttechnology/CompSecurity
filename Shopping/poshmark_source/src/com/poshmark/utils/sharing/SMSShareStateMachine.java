...
import com.poshmark.utils.sharing.share_states.SMSShare;
...
public class SMSShareStateMachine extends PM_State_Machine
    implements StateCompletionListener
{

    ShareManager shareManager;

    public SMSShareStateMachine(ShareManager sharemanager)
    {
        shareManager = sharemanager;
        transitions.addStateObject(new SMSShare(this, sharemanager));
        ...
        transitions.addTransition(GetServerContent.id, SMSShare.id, FSMTransition.CONDITION.OK);
        transitions.addTransition(GetServerContent.id, GetLocalContent.id, FSMTransition.CONDITION.FAIL);
        transitions.addTransition(GetLocalContent.id, SMSShare.id, FSMTransition.CONDITION.OK);
        transitions.addTransition(SMSShare.id, Finish.id, FSMTransition.CONDITION.OK);
