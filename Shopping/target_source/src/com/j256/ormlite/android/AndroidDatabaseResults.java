    ...
    implements DatabaseResults
{
        ...
        throws SQLException
    {
        ...
        if (i < 0)
        {
            ...
            int j = lookupColumn(stringbuilder.toString());
            i = j;
            if (j < 0)
            {
                String as[] = cursor.getColumnNames();
                throw new SQLException((new StringBuilder()).append("Unknown field '").append(s).append("' from the Android sqlite cursor, not in:").append(Arrays.toString(as)).toString());
            }
        }
        return i;
    }
