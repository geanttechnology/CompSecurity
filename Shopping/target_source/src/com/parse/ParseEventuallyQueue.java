...
//            ParseRESTCommand, ParseObject, ParseOperationSet, EventuallyPin
...
abstract class ParseEventuallyQueue
{
    public static class TestHelper
    {
        ...
        public void clear()
        {
            ...
            events.put(8, new Semaphore(1000));
            for (int i = 0; i < events.size(); i++)
            {
                int k = events.keyAt(i);
                ((Semaphore)events.get(k)).acquireUninterruptibly(1000);
            }
        ...
        public List getUnexpectedEvents()
        {
            ...
            {
                int k = events.keyAt(i);
                if (((Semaphore)events.get(k)).availablePermits() > 0)
                {
                    arraylist.add(getEventString(k));
                }
            }
        ...
        public int unexpectedEvents()
        {
            ...
            for (; i < events.size(); i++)
            {
                int l = events.keyAt(i);
                k += ((Semaphore)events.get(l)).availablePermits();
            }
        ...
        public boolean waitFor(int i, int k)
        {
            boolean flag;
            try
            {
                flag = ((Semaphore)events.get(i)).tryAcquire(k, 10L, TimeUnit.SECONDS);
            }
    ...
    public abstract j enqueueEventuallyAsync(ParseRESTCommand parserestcommand, ParseObject parseobject);
    ...
    protected void notifyTestHelper(int i, Throwable throwable)
    {
        if (testHelper != null)
        {
            testHelper.notify(i, throwable);
        }
    }
    ...
    j waitForOperationSetAndEventuallyPin(ParseOperationSet parseoperationset, EventuallyPin eventuallypin)
    {
        return j.a(null);
    }
}
