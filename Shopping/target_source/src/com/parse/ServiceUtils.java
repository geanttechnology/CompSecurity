...
final class ServiceUtils
{
    ...
    public static void completeWakefulIntent(Intent intent)
    {
        if (intent != null && intent.hasExtra("parseWakeLockId"))
        {
            ...
            PLog.e("com.parse.ServiceUtils", (new StringBuilder()).append("Got wake lock id of ").append(i).append(" in intent, but no such lock found in ").append("global map. Was completeWakefulIntent called twice for the same intent?").toString());
        }
        return;
        obj;
        intent;
    ...
    public static boolean runIntentInService(Context context, Intent intent, Class class1)
    {
        ...
        if (intent != null)
        {
            ...
            if (!flag)
            {
                PLog.e("com.parse.ServiceUtils", (new StringBuilder()).append("Could not start the service. Make sure that the XML tag <service android:name=\"").append(class1).append("\" /> is in your ").append("AndroidManifest.xml as a child of the <application> element.").toString());
                flag1 = flag;
            }
        }
        return flag1;
    ...
    public static boolean runWakefulIntentInService(Context context, Intent intent, Class class1)
    {
        boolean flag = false;
        if (intent != null)
        {
