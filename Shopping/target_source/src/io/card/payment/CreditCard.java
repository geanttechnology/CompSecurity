    ...
    implements Parcelable
{
    ...
    public String getRedactedCardNumber()
    {
        if (cardNumber != null)
        {
            ...
            {
                s = (new StringBuilder()).append("").append(String.format((new StringBuilder()).append("%").append(cardNumber.length() - 4).append("s").toString(), new Object[] {
                    ""
                }).replace(' ', '\u2022')).toString();
            }
...
label0:
        {
            String s = (new StringBuilder()).append("{").append(getCardType()).append(": ").append(getRedactedCardNumber()).toString();
            if (expiryMonth <= 0)
            {
                obj = s;
                if (expiryYear <= 0)
